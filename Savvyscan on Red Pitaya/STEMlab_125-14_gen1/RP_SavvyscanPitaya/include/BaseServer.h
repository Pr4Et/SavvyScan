...
// Required global functions (provided by top-level module)
...
// Linux socket typedefs matching WinSock names where used
using SOCKET = int;
constexpr int INVALID_SOCKET = -1;
constexpr int SOCKET_ERROR   = -1;

// 32-bit BOOL to match Windows BOOL over the wire
using BOOL32 = int32_t; // must be 4 bytes

#define MAX_LONG_ARGS 16
#define MAX_DBL_ARGS  8
#define MAX_BOOL_ARGS 8
#define ARGS_BUFFER_CHUNK 1024
#define SELECT_TIMEOUT 50   // milliseconds
...
protected:
    ...
    // IMPORTANT: use fixed 32-bit for protocol packing, not 'long'
    static int32_t *mLongArray[MAX_SOCK_CHAN];
    static int32_t  mLongArgs[MAX_SOCK_CHAN][MAX_LONG_ARGS];
    static double   mDoubleArgs[MAX_SOCK_CHAN][MAX_DBL_ARGS];
    static BOOL32   mBoolArgs[MAX_SOCK_CHAN][MAX_BOOL_ARGS];

    static char *mArgsBuffer[MAX_SOCK_CHAN];
    static int  mNumBytesSend[MAX_SOCK_CHAN];
    static int  mArgBufSize[MAX_SOCK_CHAN];
    static BOOL32 mSendLongArray[MAX_SOCK_CHAN];

    static bool mInitialized[MAX_SOCK_CHAN];
    static unsigned short mPort[MAX_SOCK_CHAN];
    static std::thread *mSocketThread[MAX_SOCK_CHAN];

    static int  mStartupError[MAX_SOCK_CHAN];
    static int  mLastWSAerror[MAX_SOCK_CHAN]; // keep name for log compatibility
...
public:
    ...
    // Linux entry point (std::thread) – we use void(void) signature
    static void SocketProc(int sockInd);
    ...
    // Default index-0 shims (API parity with original)
    static int  FinishGettingBuffer(int numReceived, int numExpected) {return FinishGettingBuffer(0, numReceived, numExpected);}    
    static void CloseClient() { CloseClient(0); }
    static int  SendBuffer(char *buffer, int numBytes) { return SendBuffer(0, buffer, numBytes);} 
    static void ReportErrorAndClose(int retval, const char *message) { ReportErrorAndClose(0, retval, message);} 
    ...
    static void SendImageBack(int retval, short *imArray, int bytesPerPixel) { SendImageBack(0, retval, imArray, bytesPerPixel);} 
