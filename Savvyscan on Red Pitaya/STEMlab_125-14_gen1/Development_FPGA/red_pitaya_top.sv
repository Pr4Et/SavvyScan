...
// Modified by Shahar Seifer & CoPilot
////////////////////////////////////////////////////////////////////////////////
...
// Native BRAM Port-B signals coming from ps
wire [31:0] bram_b_addr;
wire        bram_b_clk;
wire [31:0] bram_b_din;
...
// Drive Port-B for read-only operation in adc_clk domain
assign bram_b_clk = adc_clk;
assign bram_b_en  = 1'b1;
assign bram_b_rst = 1'b0;
assign bram_b_we  = 4'b0000;
assign bram_b_din = 32'h00000000;



wire [14:0] rp_idx;      // index from rp_gate
wire [15:0] rp_data16;   // 16-bit value delivered to rp_gate

// 32-bit BRAM ? 16-bit entry extraction
wire [13:0] word_idx_b = rp_idx[14:1];
wire        half_sel_b = rp_idx[0];

wire [31:0] x_addr;
wire [31:0] y_addr;


// Address is 32 bits on wrapper : zero-extend
// ---------------------
// Region detectors
// ---------------------
wire use_x = (x_addr[31:16] == 16'h0001);
wire use_y = (y_addr[31:16] == 16'h0002);
// ---------------------
// Port-B address mux
// ---------------------
assign bram_b_addr =
...
// BRAM Port-B addr bus is 32 bits in wrapper - zero extend word index
//assign bram_b_addr = {17'd0, word_idx_b, 1'b0}; // {zeros, [13:0], 0}; the LSB is often ignored by BMG native addressing but adding 0 keeps byte alignment explicit

// Half-word slicing
assign rp_data16 = half_sel_b ? bram_b_dout[31:16]
                              : bram_b_dout[15:0];

// DMA AXIS stream wires from the Processing System (PS)
wire [31:0] M_AXIS_MM2S_0_tdata;
wire [3:0]  M_AXIS_MM2S_0_tkeep;   // Vivado BD defines width (typical 4 for 32-bit AXIS)
...
red_pitaya_ps ps (
  ...
  // System clocks / resets out of PS
  .fclk_clk_o       (fclk),
  .fclk_rstn_o      (frstn),
...
// ===== DMA AXIS (MM2S) stream ports =====
  ...
  // AXI HP masters (to scope/asg DMA fabric)
  .axi0_sys         (axi0_sys),
  .axi1_sys         (axi1_sys),
  .axi2_sys         (axi2_sys),
  .axi3_sys         (axi3_sys),
...
// BRAM Port-B between PS and top
...
// New nets from rp_gate block
wire rp_dio0_p, rp_dio3_p;
wire signed [15:0] dac_x;
wire signed [15:0] dac_y;

localparam int GATE_SLOT =6; // sysbus at 0x40600000

rp_gate_sys_extmem #(.ENTRY_AW(15)) i_rpgate (
  .clk            (adc_clk),
  .rstn           (adc_rstn),

  // sysbus at 0x40006000
  .sys_addr       (sys[GATE_SLOT].addr), //sys[6]
  .sys_wdata      (sys[GATE_SLOT].wdata),
  .sys_wen        (sys[GATE_SLOT].wen),
  .sys_ren        (sys[GATE_SLOT].ren),
  .sys_rdata      (sys[GATE_SLOT].rdata),
  .sys_err        (sys[GATE_SLOT].err),
  .sys_ack        (sys[GATE_SLOT].ack),

  // ADC clock domain
  .adc_clk        (adc_clk),

  // Pattern memory from BRAM Port-B
  .pattern_idx_r  (rp_idx),
  .pattern_rdata  (rp_data16),

  // Gate pulses
  .rp_dio0_p      (rp_dio0_p),
  .rp_dio3_p      (rp_dio3_p),
  
  // ==== DMA X/Y stream from DDR ====
  .s_axis_xy_tdata   (M_AXIS_MM2S_0_tdata),
  .s_axis_xy_tvalid  (M_AXIS_MM2S_0_tvalid),
  .s_axis_xy_tready  (M_AXIS_MM2S_0_tready),
  .s_axis_xy_tlast   (M_AXIS_MM2S_0_tlast),
  
  .dac_x      (dac_x),      // int16 to DAC
  .dac_y      (dac_y)       // int16 to DAC

);


//---------------------------------------------
// Wires carrying scan-deflection DAC signals
// from rp_gate_sys_extmem (fpga-generated scan)
//---------------------------------------------


assign dac_x = i_rpgate.dac_x;
assign dac_y = i_rpgate.dac_y;
...
sys_bus_interconnect #(
  .SN (8),
  .SW (20)   //It means that each slot as a space of x100000
) sys_bus_interconnect (
...
//assign dac_a_sum = dac_x;// + pid_dat[0];
//assign dac_b_sum = dac_y;// + pid_dat[1];

// saturation
//assign dac_a = (^dac_a_sum[15-1:15-2]) ? {dac_a_sum[15-1], {13{~dac_a_sum[15-1]}}} : dac_a_sum[14-1:0];
//assign dac_b = (^dac_b_sum[15-1:15-2]) ? {dac_b_sum[15-1], {13{~dac_b_sum[15-1]}}} : dac_b_sum[14-1:0];

// Shift signed 16-bit samples left by 2 to align with the 14-bit DAC input
wire signed [15:0] dac_x_shifted = dac_x <<< 2;
wire signed [15:0] dac_y_shifted = dac_y <<< 2;
// Take the upper 14 bits as the DAC input
assign dac_a = dac_x_shifted[15:2];
assign dac_b = dac_y_shifted[15:2];
...
wire [DWE-1:0] exp_p_alt,  exp_n_alt;
wire [DWE-1:0] exp_p_altr, exp_n_altr;
wire [DWE-1:0] exp_p_altd, exp_n_altd;
...
// -----------------------------------------------------------------------------
// Pick two pins for rp_gate pulses on the P-side connector
localparam int DIO0_EXP_P_IDX = 0; // exp_p_io[0]  -> DIO0_P
localparam int DIO3_EXP_P_IDX = 3; // exp_p_io[3]  -> DIO3_P

// Defaults on the P-side (no ALT logic except rp_gate)
wire [DWE-1:0] exp_p_alt_def  = {DWE{1'b0}};
wire [DWE-1:0] exp_p_altr_def = {DWE{1'b0}};
wire [DWE-1:0] exp_p_altd_def = {DWE{1'b0}};


// One-hot mask for the two pins we override (tool-friendly form)
wire [DWE-1:0] rp_mask = 
    (({{(DWE-1){1'b0}},1'b1}) << DIO0_EXP_P_IDX) |
    (({{(DWE-1){1'b0}},1'b1}) << DIO3_EXP_P_IDX);

// Final single-driver assignments
assign exp_p_alt  = exp_p_alt_def  | rp_mask;  // select ALT mux on those pins
assign exp_p_altd = exp_p_altd_def | rp_mask;  // set them as outputs
assign exp_p_altr = exp_p_altr_def
                  | (rp_dio0_p ? (({{(DWE-1){1'b0}},1'b1}) << DIO0_EXP_P_IDX) : {DWE{1'b0}})
                  | (rp_dio3_p ? (({{(DWE-1){1'b0}},1'b1}) << DIO3_EXP_P_IDX) : {DWE{1'b0}});

// N-side ALT assignments
...
(
  ...
  .sys_clk_i       (  adc_clk                    ),  // clock
  .sys_rstn_i      (  adc_rstn                   ),  // reset - active low
  .sys_addr_i      (  sys[7].addr                ),// changed from sys[6] to sys[7]
  .sys_sel_i       (                             ),
  ...
  `else
  //sys_bus_stub sys_bus_stub_6 (sys[6]);   //we use this section for rp_gate
  `endif
...
for (genvar i=2; i<7; i++) begin: for_sys2
   if (i != GATE_SLOT) begin
      sys_bus_stub u_sys_stub (sys[i]);
    end
end: for_sys2
endgenerate
