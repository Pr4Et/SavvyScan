...
bool bSpcMInitCardByName (ST_SPCM_CARDINFO *pstCardInfo, char* szDrvName)
    ...
    switch (pstCardInfo->eCardFunction)
        ...
        case AnalogIn:
            ...
            pstCardInfo->uCfg.stAI.bACCouplingAvailable =(lAIFeatures & SPCM_AI_ACCOUPLING) != 0;
            pstCardInfo->uCfg.stAI.bBWLimitAvailable =   (lAIFeatures & SPCM_AI_LOWPASS) != 0;
...
bool bSpcMSetupPathInputCh (ST_SPCM_CARDINFO *pstCardInfo, int32 lChannel, int32 lPath, int32 lInputRange, bool bTerm, bool bACCoupling, bool bBWLimit, bool bDiffInput)
    {
    if (!pstCardInfo)
        return false;
    ...
    if (pstCardInfo->uCfg.stAI.bDiffModeAvailable)
        if (!dwError) dwError = spcm_dwSetParam_i32 (pstCardInfo->hDrv, SPC_DIFF0 +   lChannel * (SPC_DIFF1   - SPC_DIFF0),    bDiffInput ?  1 : 0);
    if (pstCardInfo->uCfg.stAI.bACCouplingAvailable)
    ...
    if (pstCardInfo->uCfg.stAI.bBWLimitAvailable)
