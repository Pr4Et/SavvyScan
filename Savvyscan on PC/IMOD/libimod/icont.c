...
int imodContoursDeleteToEnd(Iobj *obj, int keep)
{
  ...
  if (obj->contsize){
    newCont = (Icont *)realloc(obj->cont, obj->contsize * sizeof(Icont));
    if (!newCont)
      return(-1);
    obj->cont = newCont;
 ...
 * [cont] can be a scan contour; if not, a temporary scan contour is made.  Calls
 * @@imodel_contour_centroid@.
 */
int imodContourCenterOfMass(Icont *cont, Ipoint *rpt)
{
  ...
  saveFlags = cont->flags;
  if (cont->flags & ICONT_TEMPUSE)
    cont->flags |= ~ICONT_TEMPUSE;
  err = imodel_contour_centroid(cont, rpt, &weights);
  cont->flags = saveFlags;
  if (err)
    return err;

  if (fabs(weights) > 1.e-20) {
    rpt->x /= weights;
    rpt->y /= weights;
    rpt->z /= weights;
  }
 ...
 * pixel sum, or area, in [rtw].  The input contour can be a scan contour; if not, a 
 * temporary scan contour is made. If ICONT_TEMPUSE is set, it treats it as 
                            ...
                            double *rtw)
{
  ...
  *rtw = 0;
  ...
  rcp->z = icont->pts[0].z;
  cont = makeOrReturnScanContour(icont);
  if (!cont)
    return (-1);
 ...
 * meshing, which is why the [fill] value prevents the separation.  If fill is any 
 * non-zero value besides 1 or -1, it will not separate the connecting lines, which is
 * useful for polygon fill drawing.
 */
...
{
  Ipoint point;
  int dirFac = (direc >= 0 ? 1 : cfrom->psize - 1);
...
void imodContourReduce(Icont *cont, float tol)
{
  ...
  if (minseg[0] + 1 != npts) {
    ...
    if (cont->store) {
      ...
      /* Otherwise just repack the retained points into the contour and adjust size */
      npo = 0;
      while (nextpt[ipo] < npts) {
        ipo = nextpt[ipo];
        npo++;
        cont->pts[npo] = cont->pts[ipo];
      }
      cont->psize = npo + 1;
    }
    /* if (minseg[0] + 1 != npo)
...
/* Frees all arrays and contours except the one being returned on success */
#define CLEANUP_SCAN(e)                         \
  B3DFREE(aet->pts);                            \
 ...
 * by increasing X at each Y level.  Z coordinates are ignored, except that the Z 
 * coordinate of the first point on [incont] is transferred to the first point of the 
 * scan contour.
 */
Icont *imodel_contour_scan(Icont *incont)
{
  ...
  for(i = 0; i <=  ymax - ymin; i++){
    ...
    for (j = 0; j < aet->psize; j++){
      ...
      if (cont->psize == allocsize) {
        allocsize += chunksize;
        newPts = (Ipoint *)realloc(cont->pts,
  ...
  newPts = (Ipoint *)realloc(cont->pts, cont->psize * sizeof(Ipoint));
  if (newPts == NULL) {
    CLEANUP_SCAN(1);
    return(NULL);
  }
 ...
 * [contzp] = pointer to array for Z values, which is allocated for the number of 
 * contours ^
 ...
 * The arrays for [zlist], [numatzp], and [contatzp] are allocated to have zmax + 2 - zmin
 * elements and would be indexed by z - zmin for z between zmin and zmax, exclusive.
 */
                           ...
                           int *zlsizep, int *nummaxp)
{
  ...
  for (i = 0; i < zmax + 1 - zmin; i++) {
    contatz[i] = (int *)malloc((numatz[i] + 1) * sizeof(int));
    if (!contatz[i]) {
      imodContourFreeZTables(numatz, contatz, contz, zlist, zmin, zmax);
      return -1.;
    }
 ...
 * [co], [eco] = contour numbers that are indexes into arrays here, not the original 
 * contour numbers in the object  ^
 ...
 * [nests] = pointer to array of nesting structures, which should start out NULL and will
 * be built up over repeated calls ^
 * [nestind] = array for index from contours to nests, which must be allocated for the 
 * maximum contour number that this is called with plus 1 ^
...
{
  float frac1, frac2;
  int inco, outco, nind, needWarn = 0;
  Nesting *nest, *newNests;
  int *newOutside, *newInside;
  ...
  if (frac1 > 0.99998 && frac2 > 0.99998) { 
    if (*numwarn >= 0)
      printf("WARNING: Contours %d and %d are duplicates\n", co + 1, eco + 1);
    needWarn = 1;
    ...
    if (nind < 0) {
      if (*numnests)
        newNests = (Nesting *)realloc(*nests, (*numnests + 1 )* sizeof(Nesting));
      else
        newNests = (Nesting *)malloc(sizeof(Nesting));
      if (!newNests)
        return -1;
      *nests = newNests;
      nind = (*numnests)++;
    ...
    if (nest->noutside)
      newOutside = (int *)realloc(nest->outside,
                                           (nest->noutside + 1) * sizeof(int));
    else
      newOutside = (int *)malloc(sizeof(int));
    if (!newOutside)
      return -1;
    nest->outside = newOutside;
    ...
    if (nest->ninside)
      newInside = (int *)realloc(nest->inside,
                                    (nest->ninside + 1) * sizeof(int));
    else
      newInside = (int *)malloc(sizeof(int));
    if (!newInside)
      return -1;
    nest->inside = newInside;
