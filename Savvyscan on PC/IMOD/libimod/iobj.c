...
void imodObjectDefault(Iobj *obj)
{
  ...
  for(i = 0; i < IOBJ_EXSIZE; i++)
    obj->extra[i] = 0;
  obj->contsize   = 0;
  obj->flags      = IMOD_OBJFLAG_DRAW_LABEL | IMOD_OBJFLAG_SCALE_WDTH;
  obj->axis       = 0;
  obj->drawmode   = 1;
  obj->red        = 0.5f;
  obj->green      = 0.5f;
...
double imodObjectChecksum(Iobj *obj, int obNum)
{
  ...
  for(co = 0; co < obj->contsize; co++){
    ...
    for(pt = 0; pt < cont->psize; pt++){
      psum += cont->pts[pt].x * ((pt % 7) + 1);
      psum += cont->pts[pt].y * ((pt % 5) + 1);
      psum += cont->pts[pt].z;
    }
...
int imodObjectAddMesh(Iobj *inObject, Imesh *inMesh)
{
  if (!inObject || !inMesh) 
    return(-1);
  inObject->mesh = imodel_mesh_add(inMesh, inObject->mesh, &(inObject->meshsize));
  if (!inObject->mesh)
    return -1;
  inObject->surfsize = B3DMAX(inObject->surfsize, inMesh->surf);
...
void imodObjectCleanSurf(Iobj *obj)
{
  ...
  obj->surfsize = 0;
  for (co = 0; co < obj->contsize; co++)
    ACCUM_MAX(obj->surfsize, obj->cont[co].surf);
  for (co = 0; co < obj->meshsize; co++)
    ACCUM_MAX(obj->surfsize, obj->mesh[co].surf);
 ...
 * contours.  Returns 1 if there is no mesh information, 2 for memory error, or 3 for
 * an unsupported feature in a contourless mesh (see @@imesh.html#imeshSortSurfaces@.)
 */
int imodObjectSortSurf(Iobj *obj)
{
  int npoly, me, i, j, poly, ninpoly, iwork, nwork, scan, found, lastSurf, numNewMesh;
  int refvert, work, nsurfs, co, pt, ind, resol, vertBase, normAdd, newSurfs, error;
  int *listp, *surfs, *meshes, *nverts, *towork, *refp, *scanp, *lincs;
  int **starts;
  float *zmins, *zmaxs;
  Ipoint *vertp;
  Imesh *surfMeshes, *temp, *newMeshes = NULL;
  ...
  if (!obj->contsize) {
    ...
    for (me = 0; me < obj->meshsize; me++) {
      ...
      if (imeshResol(obj->mesh[me].flag) == resol) {
        if (obj->mesh[me].vsize && obj->mesh[me].lsize) {

          /* For non-empty mesh, sort and adjust surface numbers up */
          surfMeshes = imeshSortSurfaces(&obj->mesh[me], &newSurfs, &error);

          if (!surfMeshes)
            return error;
          for (i = 0; i < newSurfs; i++)
            surfMeshes[i].surf += lastSurf;
          lastSurf += newSurfs;
        }
      } else {

        /* Just copy a different resolution mesh */
        surfMeshes = imodMeshDup(&obj->mesh[me]);
        if (!surfMeshes)
          return 2;
        newSurfs = 1;
      }
      ...
      for (i = 0; i < newSurfs; i++) {
        temp = newMeshes;
        newMeshes = imodel_mesh_add(&surfMeshes[i], newMeshes, &numNewMesh);
        if (!newMeshes)
          return 2;
      }
      free(surfMeshes);
    ...
    obj->mesh = newMeshes;
    obj->meshsize = numNewMesh;
    obj->surfsize = lastSurf;
    return 0;
  }
...
/* Removed Unused and suspect imodObjectClip 2/5/18 */
 ...
 * instead.  Returns -1 for an error (including no non-empty contours or meshes), 0 for
...
/*!
 * Sets the color of object [inObject] to the given values, which should be between 0 
 * and 1.
 */
