 ...
 *
 *  Copyright (C) 1995-2005 by the Regents of the University of 
...
#define CHECK_OBJ_IND                              \
  if (objInd < 0 || objInd >= imod->objsize) {    \
    error = IMOD_ERROR_CORRUPT;                   \
    break;                                        \
  }
#define CHECK_CONT_IND                                         \
  if (objInd < 0 || objInd >= imod->objsize ||                \
      contInd < 0 || contInd >= imod->obj[objInd].contsize) { \
    error = IMOD_ERROR_CORRUPT;                               \
    break;                                                    \
  }
...
{
  FILE  *fin;
  int objInd = -1, contInd = -1, meshInd = -1;
  int error;
  int ieof = FALSE;
  int id;
#ifdef IMODEL_FILES_DEBUG
...
#ifdef IMODEL_FILES_DEBUG
  printf("imodel_read_v02: input ok.\n");
#endif
  ...
  if (id != ID_IMOD)
    return(IMOD_ERROR_FORMAT);
#ifdef IMODEL_FILES_DEBUG
  printf("imodel_read_v02: format ok.\n");
#endif     
...
#ifdef IMODEL_FILES_DEBUG
  printf("imodel_read_v02: header ok.\n");
#endif
  ...
  while((!feof(fin)) && (!ieof)){
    ...
    /* Set error to 0, break on all errors and test it outside the switch
       This allows consistent return on error in case additional cleanup is needed */
    error = 0;
    ...
    case ID_OBJT:
      objInd++;
      CHECK_OBJ_IND;
      if ((error = imodel_read_object(&imod->obj[objInd], imod->file))) {
        break;
      }
      if (imod->obj[objInd].contsize) {
        imod->obj[objInd].cont = imodContoursNew(imod->obj[objInd].contsize);
        if (!imod->obj[objInd].cont) {
          error = IMOD_ERROR_MEMORY;
          break;
        }
      } else {
        imod->obj[objInd].cont = NULL;
      }  
      contInd = -1;
      if (imod->obj[objInd].meshsize) {
        imod->obj[objInd].mesh = imodMeshesNew(imod->obj[objInd].meshsize);
        if (!imod->obj[objInd].mesh) {
          error = IMOD_ERROR_MEMORY;
          break;
        }
      }
      meshInd = -1;
#ifdef IMODEL_FILES_DEBUG
      printf("imodel_read: obj ok %x c = %d, m = %d.\n",
              id, imod->obj[objInd].contsize, imod->obj[objInd].meshsize);
#endif
      break;
	       
    case ID_OLBL:
      CHECK_OBJ_IND;
      imod->obj[objInd].label = imodLabelRead(imod->file, &error);
      break;

    case ID_CONT:
      contInd++;
      CHECK_CONT_IND;
      if ((error = imodel_read_contour(&imod->obj[objInd].cont[contInd], imod->file)))
        break;
#ifdef IMODEL_FILES_DEBUG
      printf("imodel_read: cont ok. %x\n", id);
#endif
      break;

    case ID_LABL:
      CHECK_CONT_IND;
      imod->obj[objInd].cont[contInd].label = imodLabelRead(imod->file, &error);
      break;

    case ID_SIZE:
      CHECK_CONT_IND;
      error = imodel_read_ptsizes(&imod->obj[objInd].cont[contInd], imod->file);
      break;

    case ID_MESH:
      meshInd++;
      if ((error = imodel_read_mesh(&imod->obj[objInd].mesh[meshInd], imod->file))) {
#ifdef IMODEL_FILES_DEBUG
        printf("imodel_read: mesh error %d.\n", error);
#endif
        return(error);
      }
#ifdef IMODEL_FILES_DEBUG
      printf("imodel_read: mesh ok.\n");
#endif
      break;
    ...
    case ID_IEOF:
      fflush(fin);
#ifdef IMODEL_FILES_DEBUG
      printf("imodel_read_v02: exit chunk %x.\n", id);
#endif
      ieof = TRUE;
      break;
    ...
    case ID_CLIP:
#ifdef IMODEL_FILES_DEBUG
      printf("imodel_read_v02: clip %x.\n", id);
#endif
      CHECK_OBJ_IND;
      imodel_read_clip(&imod->obj[objInd], imod->file, imod->flags); 
      break; 

      /* DNM 9/4/02: pass flags so mat1 & mat3 can be read two ways */
    case ID_IMAT:
      CHECK_OBJ_IND;
      imodel_read_imat(&imod->obj[objInd], imod->file, imod->flags); 
      break; 

    case ID_VIEW:
#ifdef IMODEL_FILES_DEBUG
      printf("imodel_read_v02: view %x.\n", id);
#endif
    ...
    case ID_OBST: /* Object general storage */
      CHECK_OBJ_IND;
      imod->obj[objInd].store = imodReadStore(fin, &error);
      break;

    case ID_COST: /* Contour general storage */
      CHECK_CONT_IND;
      imod->obj[objInd].cont[contInd].store = imodReadStore(fin, &error);
      break;

    case ID_MEST: /* Mesh general storage */
      CHECK_OBJ_IND;
      if (meshInd < 0 || meshInd >= imod->obj[objInd].meshsize) {
        error = IMOD_ERROR_CORRUPT;
        break;
      }
      imod->obj[objInd].mesh[meshInd].store = imodReadStore(fin, &error);
      break;

    case ID_MEPA:
      CHECK_OBJ_IND;
      error = imodel_read_meshparm(&imod->obj[objInd], fin);
      break;

    case ID_SKLI:
      CHECK_OBJ_IND;
      error = imodel_read_meshskip(&imod->obj[objInd], fin);
      break;

    case ID_SLAN:
      error = imodel_read_sliceang(imod, fin);
      break;

    case ID_OGRP:
      error = objGroupRead(&imod->groupList, fin);
      break;
    ...
    default:
      ...
      *badp = id;
      badid[4] = 0;
      printf("imodel_read: unknown data. %x, %f\n%s\n", 
              id, (float)id,badid);
#endif
      id = imodGetInt(fin);
...
#ifdef IMODEL_FILES_DEBUG
  printf("imodel_read_v02: model ok.\n");
#endif
...
static int imodel_read_contour(Icont *cont, FILE *fin)
{
  ...
  if (ferror(fin)){
    cont->psize = 0;
#ifdef IMODEL_FILES_DEBUG
    printf("imodel_read_contour: error reading cont head.\n");
#endif
    return(error);
  }
...
#ifdef IMODEL_FILES_DEBUG
      printf("#imodReadAscii: read object %d %d %d\n",
              ob, conts, meshes);
#endif
