...
Islice *sliceBox(Islice *sl, int llx, int lly, int urx, int ury)
{
  ...
  if (llx >= 0 && lly >= 0 && urx <= sl->xsize && ury <= sl->ysize &&
      !mrc_getdcsize(sl->mode, &csize, &dsize)) {
    pixSize = csize * dsize;
    for (j = lly, y = 0; y < ny; y++, j++) {
      inPtr = sl->data.b + (j * sl->xsize + llx) * pixSize;
      outPtr = sout->data.b + y * nx * pixSize;
      memcpy(outPtr, inPtr, pixSize * nx);
    }
    return(sout);
  }
...
int sliceBoxIn(Islice *sl, int llx, int lly, int urx, int ury)
{
  ...
  if (!llx && !lly && urx == sl->xsize && ury == sl->ysize)
    return 0;
 ...
 * header, or an error from @@mrcfiles.html#mrc_write_slice@ if there is an error 
 * writing the data. Unused as of 2/3/07, but does work.
 */
int sliceWriteMRCfile(const char *filename, Islice *slice)
{
  ...
  error = mrc_write_slice(slice->data.b, fp, &hout, 0, 'z');
  fclose(fp);
  return(error);
}
 ...
 * plane is [secNum] along the axis given by [axis], which must be one of x, X, y,
                      ...
                      int xsize, int ysize, int xcen, int ycen)
{
  ...
  /* Get the full size of the input file in the given axis */
  switch(axis) {
  case 'x':
  case 'X':
    nx = hin->ny;
  ...
  case 'Z':
    ...
    break;
  default:
    b3dError(stderr, "ERROR: sliceReadSubm - Illegal axis argument %c\n", axis);
    return(NULL);
  }

  /* Get the coordinate range, plus 1 on upper right */
  llx = xcen - (xsize / 2);
  lly = ycen - (ysize / 2);
  urx = llx + xsize;
  ury = lly + ysize;

  /* If it is all contained within a Y or Z slice, set up for direct read into slice */
  if ((axis == 'z' || axis == 'Z' || axis == 'y' || axis == 'Y') && 
      llx >= 0 && lly >= 0 && urx <= nx && ury <= ny) {
    mrc_init_li(&li, NULL);
    mrc_init_li(&li, hin);
    li.xmin = llx;
    li.xmax = urx - 1;
    ...
    slice = sliceCreate(urx - llx, ury - lly, hin->mode);
    if (!slice)
      return(NULL);
    slice->mean = hin->amean;
    if (!mrcReadSection(hin, &li, slice->data.b, secNum))
      return(slice);
    sliceFree(slice);
    return(NULL);
  }    

  /* Or, if boxing is needed, get slice structure, read data and get a buffer from that */
  slice = (Islice *)malloc(sizeof(Islice));
  if (!slice)
    return(NULL);
  slice->mean = hin->amean;

  buf = mrc_mread_slice(hin->fp, hin, secNum, axis);
  if (!buf) {
    free(slice);
    return(NULL);
  }

  /* Assign data to slice and box it */
