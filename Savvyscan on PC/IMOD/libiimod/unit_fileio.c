...
int iiuOpen(int iunit, const char *name, const char *attribute)
{
  ...
  if (mode == 3) {
    ...
    /* For TIFF file, an MRC header was created and filled automatically */
...
{
  Unit *u; 
  int trial, delay = 500, numTry = 10, unit = iunit - 1;
...
{
  Unit *u = lookupUnit(iunit, "iiuGetIIFile", 1, 0);
  return u->iiFile;
}
...
static int setupCurrentLines(Unit *u, int numLines)
{
  ...
  if (u->iiFile->ury >= u->iiFile->ny) {
    b3dError(stdout, "\nERROR: iiuRead/WriteLines - lines go past end of current "
             "section  (cur line %d  #l %d  to %d  ny %d).\n", u->currentLine, numLines,
             u->iiFile->ury, u->iiFile->ny);
    return(1);
  }
 ...
 * of the conversion option; if checkRW is 2 it tests whether writing is possible; in
...
/*!
 * Fixes the pointer to the header for unit [iunit] if it is an HDF file and the
 * the underlying ImodImageFile has been reopened, since a new header structure is created
 * on the reopen.  Exits upon error; the file must be an HDF file.
 */
void iiuReassignHeaderPtr(int iunit)
{
  Unit *u = lookupUnit(iunit, "iiuReassignHeaderPtr", 1, 0);
  if (u->iiFile->file != IIFILE_HDF) {
    printf("ERROR: iiuReassignHeaderPtr - File on unit %d is not HDF\n", iunit);
    exit(1);
  }
  u->header = (MrcHeader *)u->iiFile->header;
}
 ...
 * IIFILE_HDF (5).  Fortran wrapper iiufiletype.
 */
...
int iiuBufBytesPerPixel(int iunit)
{
  ...
  Unit *u = lookupUnit(iunit, "iiuFileType", 1, 0);
  if (u->noConvert) {
    mrc_getdcsize(u->iiFile->mode, &dsize, &csize);
    return dsize * csize;
  }
  return 4;
}
