...
void MultiSlider::setDecimals(int slider, int decimals)
{
  ...
  B3DCLAMP(decimals, 0, MAX_DECIMALS);
  mDecimals[slider] = decimals;
}

/*!
 * Sets a scaling to divide integer slider values by to get the number to display.
 * When no scaling is set and decimals are specified, slider values are divided by
 * 10 to the number of decimals, and single steps correspond to single digit changes.
...
void MultiSlider::displayValue(int slider, int value)
{
  ...
  if (dec && mScaling[slider] > 0.)
    str.sprintf(deciFormats[dec], value / mScaling[slider]);
  else if (dec)
    str.sprintf(deciFormats[dec], value / pow(10., (double)dec));
  else if (mScaling[slider] > 0)
    str.sprintf("%d", B3DNINT(value / mScaling[slider]));
  else
    str.sprintf("%d", value);
