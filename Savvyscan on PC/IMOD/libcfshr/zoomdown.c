...
static void setFilterStatics(int type, double zoom, double *scale, double *support,
                             int *width, int *retWidth);
...
int selectZoomFilter(int type, double zoom, int *outWidth)
{
  ...
  setFilterStatics(type, zoom, &sXscale, &sXsupport, &sXwidth, outWidth);
  setFilterStatics(type, zoom, &sYscale, &sYsupport, &sYwidth, outWidth);
  if (type == 3)
    mitchell_init(1./3., 1./3.);
  return 0;
}

/* For the filter type and zoom, set the variables describing the filter */
static void setFilterStatics(int type, double zoom, double *scale, double *support,
                             int *width, int *retWidth)
{
  *scale = 1. / zoom;
                       ...
                       int *outWidthY)
{
  ...
  int err = selectZoomFilter(type, B3DMIN(xzoom, yzoom), outWidthX);
  if (err)
    return err;
  if (yzoom >= xzoom) {
    setFilterStatics(type, yzoom, &sYscale, &sYsupport, &sYwidth, outWidthY);
  } else {
    *outWidthY = *outWidthX;
    setFilterStatics(type, xzoom, &sXscale, &sXsupport, &sXwidth, outWidthX);
  }
  return 0;
}
