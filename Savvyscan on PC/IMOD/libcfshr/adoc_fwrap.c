...
#define adocxmlreadstatus ADOCXMLREADSTATUS
...
#define adocsetwriteasxml ADOCSETWRITEASXML
#define adocgetwriteasxml ADOCGETWRITEASXML
#define adocgetxmlrootelement ADOCGETXMLROOTELEMENT
#define adocsetxmlrootelement ADOCSETXMLROOTELEMENT
...
#define adocgetdouble ADOCGETDOUBLE
...
#define adocsetdouble ADOCSETDOUBLE
...
#define adocxmlreadstatus adocxmlreadstatus_
...
#define adocsetwriteasxml adocsetwriteasxml_
#define adocgetwriteasxml adocgetwriteasxml_
#define adocgetxmlrootelement adocgetxmlrootelement_
#define adocsetxmlrootelement adocsetxmlrootelement_
...
#define adocgetdouble adocgetdouble_
...
#define adocsetdouble adocsetdouble_
...
static int twof2cstr(const char *collName, const char *key, int collSize, int keySize, 
                     char **cStr, char **kStr);
static char *adocf2cstr(const char *str, int strSize);
...
{
  return AdocXmlReadStatus(sectNotElem, sectNoName, childNotElem,
                           childAttribs, valueNotText, multipleChilds);
}
...
{
  return AdocGetWriteAsXML();
}

int adocgetxmlrootelement(char *element, int elemSize)
{
  char *strPtr = NULL;
  int err;
  err = AdocGetXmlRootElement(&strPtr);
  if (!err && !strPtr)
    c2fString(" ", element, elemSize);
  else if (!err && c2fString(strPtr, element, elemSize)) {
    PipSetError("In AdocGetXmlRootElement, string is too long for character variable");
    err = -1;
  }
...
int adocsetxmlrootelement(char *element, int elemSize)
{
  char *cStr;
  int err;
  if (!(cStr = adocf2cstr(element, elemSize)))
    return -1;
  err = AdocSetXmlRootElement(cStr);
  free(cStr);
  return err;
}
...
int adocsetdouble(char *collName, int *sectInd, char *key, double *val,
                 int collSize, int keySize)
{
  char *cStr, *kStr;
  int err;
  if (twof2cstr(collName, key, collSize, keySize, &cStr, &kStr))
    return -1;
  err = AdocSetDouble(cStr, *sectInd - 1, kStr, *val);
  free(cStr);
  free(kStr);
  return err;
}
...
int adocgetdouble(char *collName, int *sectInd, char *key, double *val1,
                 int collSize, int keySize)
{
  char *cStr, *kStr;
  int err;
  if (twof2cstr(collName, key, collSize, keySize, &cStr, &kStr))
    return -1;
  err = AdocGetDouble(cStr, *sectInd - 1, kStr, val1);
  free(cStr);
  free(kStr);
  return err;
}
