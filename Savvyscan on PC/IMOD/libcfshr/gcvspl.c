/*
 * gvcspl.c - spline smoothing with cross-validation
 * Based on this file from netlib with test program removed:
 * gcvspl.f -- translated by f2c (version 20100827).
 *
 * $Id$
 */

/* Include copy of f2c when translation was done 
   Do not include anything that will pull in stdlib, it will conflict */
#include "f2c-gcvspl.h"
#include "imodconfig.h"

/* The functions themselves are in Fortran calling convention, so define their names
   to work everywhere */
#ifdef F77FUNCAP
#define gcvspl_ GCVSPL
#define splder_ SPLDER
#define bandet_ BANDET
#define bansol_ BANSOL
#define basis_ BASIS
#define prep_ PREP
#define search_ SEARCH
#define splc_ SPLC
#define trinv_ TRINV
#endif

/* Prototypes so the functions have the right return value in the wrappers*/
int gcvspl_(double *x, double *y, int *ny, double *wx, double *wy, int *m, int *n,
            int *k, int *md, double *val, double *c__, int *nc, double *wk, int *ier);
double splder_(int *ider, int *m, int *n, double *t, double *x, double *c__, int *l,
               double *q);

/* C wrappers to the two functions of interest */
int gcvspl(double *x, double *y, int yDim, double *wgtx, double *wgty, int mOrder,
           ...
           double *work, int *ier)
{
  return gcvspl_(x, y, &yDim, wgtx, wgty, &mOrder, &numVal, &numYcol, &mode, &val, coeff,
                 &coeffDim, work, ier);
}
...
{
  return splder_(&derivOrder, &mOrder, &numVal, &tVal, x, coeff, nearInd, work);
}
                             ...
                             doublereal *wk, integer *ier)
{
  ...
  *ier = 0;
  ...
  if (abs(*md) > 4 || *md == 0 || (abs(*md) == 1 && *val < 0.) || 
      (abs(*md) == 3 && *val < 0.) || 
      (abs(*md) == 4 && (*val < 0. || *val > (doublereal) (*n - *m)))) {
    *ier = 3;
    /* Wrong mode value */
    return 0;
  }
 ...
 L80:
  if (gf3 <= gf4) {
    ...
    r3 = r2 - alpha;
    gf3 = splc_(m, n, k, &y[y_offset], ny, &wx[1], &wy[1], md, val, &r3, &
                c_b6, &c__[c_offset], nc, &wk[1], &wk[ib], &wk[iwe], &el, &wk[
                                                                              7]);
  } else {
    r1 = r3;
    gf1 = gf3;
