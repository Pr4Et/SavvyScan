...
#define sumstoavgsdalldbl SUMSTOAVGSDALLDBL
...
#define sumstoavgsdalldbl sumstoavgsdalldbl_
#define lsfit lsfit_
...
{
  double avg8, sd8;
  sumsToAvgSDallDbl(sx8, sxsq8, n1, n2, &avg8, &sd8);
  *avg = (float)avg8;
  *sd = (float)sd8;
}
...
void sumstoavgsdalldbl(double *sx8, double *sxsq8, int *n1, int *n2, double *avg,
                    double *sd)
{
  sumsToAvgSDallDbl(*sx8, *sxsq8, *n1, *n2, avg, sd);
}
                       ...
                       float *avg, float *SD)
{
  float den;
  float *arrTmp;
  double sumTmp, sumTmpSq, roughMean, avg8, sd8;
  int ix, iy, xDiv, yDiv, jx, jy, nsum;
  int nxArea = ix1 + 1 - ix0;
  int nyArea = iy1 + 1 - iy0;
  ...
  *dmax = -1.e37;
  roughMean = 0.;
  nsum = 0;
  xDiv = B3DMAX(2, B3DMIN(8, nx -2));
  yDiv = B3DMAX(2, B3DMIN(8, ny -2));
  for (jy = 1; jy < yDiv; jy++) {
    iy = iy0 + (jy * nyArea) / yDiv;
    for (jx = 1; jx < xDiv; jx++) {
      ix = ix0 + (jx * nxArea) / xDiv;
      roughMean += array[(size_t)iy * nx + ix];
      nsum++;
    }
  }
  roughMean /= nsum;
  ...
  sumsToAvgSDallDbl(*sumDbl, *sumSqDbl, nxArea, nyArea, &avg8, &sd8);
  avg8 += roughMean;
  *sumDbl = nxArea * (nyArea * avg8);
  *sumSqDbl = ((double)nxArea * nyArea - 1.) * sd8 * sd8 + *sumDbl * avg8;
  *avg = (float)avg8;
  *SD = (float)sd8;
}
               ...
               float xpred, float *ypred, float *prederr)
{
  ...
  double sxy, sysq, setmp, xp, yp, dslope, dbint;
  int i;
  *slope = 1.;
  *bint = *ro = *sa = *sb = *se = *ypred = *prederr = 0.;
  ...
  dslope = (sxyp / sxpsq);
  *slope = (float)dslope;
  dbint = ((ybar * sxpsq - xbar * sxyp) / sxpsq);
  *bint = (float)dbint;
...
void lsFit2Pred(float *x1, float *x2, float *y, int n, float *a, float *b, 
                ...
                float *prederr)
{
  double x1s, x2s, ys, x1m, x2m, ym, x1sqs, x2sqs, x1x2s, x1ys, x2ys, ysqs;
  int i;
  double denom, anum, bnum, x1p, x2p, yp, absdenom, absanum, absbnum;
  double c11, c22, c12, predsq, devss, dbla, dblb, dblc;
  x1s = 0.;
  x2s = 0.;
  ys = 0.;
  for (i = 0; i < n; i++) {
  ...
  *c = dblc;
  *ypred = dbla * x1pred + dblb * x2pred + dblc;
  c11 = x2sqs / denom;
  c22 = x1sqs / denom;
  c12 = -x1x2s / denom;
  devss = ysqs - dbla * x1ys - dblb * x2ys;
