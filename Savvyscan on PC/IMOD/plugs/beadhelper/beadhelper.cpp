...
static const char *DEGREE_SIGN = " deg";      // degree sign
...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Bead Helper", "plughelp/beadhelper.html#TOP", '6',
                              Qt::Window);
  plug.window  = new BeadHelper(sTopWin,"Bead Helper");
...
BeadHelper::BeadHelper(DockingDialog *parent, const char *name) :
  DialogFrame(parent, 0, NULL, NULL, true, "Bead Helper", "", name)
{
...
void BeadHelper::loadSettings()
{
  ...
  if(nvals!=NUM_SAVED_VALS)
  {
    ...
    QMessageBox::about( this, "-- Documentation --",
                        ...
                        "right, watch the video and read the documentation ! \n"
                        "                                   -- Andrew Noske" );
    return;
  }
...
void BeadHelper::moreSettings()
{
  ...
  ds.addLineEditF( "tilt axis angle:           ", INT_MIN, INT_MAX, &newTiltAxisAngle, 5,
                  "The angle (in degrees) counterclockwise from "
...
bool BeadHelper::openTiltAngleFile()
{
  ...
  qname  = diaOpenFileName(this, "Select Tilt or Raw Tilt File", 1, filter);
  if (qname.isEmpty())
    return false;
  ...
  for (int i=0; i<(int)plug.tiltAngles.size(); i++ )
  {
    ...
    if( i>0 )
    {
      ...
      if( (tiltIncrementApprox > 0 && tiltIncrease < tiltIncrementApprox/2.0f) ||
          (tiltIncrementApprox < 0 && tiltIncrease > tiltIncrementApprox/2.0f) )
        wprint("  <-- PROBLEM?");
    }
    wprint("\n");
  }
  
  wprint("\n > average tilt increment = %f%s\n", tiltIncrementApprox, DEGREE_SIGN);
...
void BeadHelper::topCloseEvent ( QCloseEvent * e )
{
...
void BeadHelper::topChangeEvent(QEvent *e)
{
...
float bead_estimateTiltAngle()
{
  ...
  wprint("\nAverage angle over %d contours = %f%s", totalConts, avgAngle, DEGREE_SIGN);
  return (avgAngle);
}
