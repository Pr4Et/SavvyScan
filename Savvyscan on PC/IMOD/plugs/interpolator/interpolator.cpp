...
int imodPlugKeys(ImodView *vw, QKeyEvent *event)
{
	...
	switch(keysym)
  {
    ...
    case Qt::Key_Semicolon: 
      plug.window->changeInterpContRange();
      break;
...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Interpolator", "plughelp/interpolator.html#TOP", '5',
                              Qt::Window);
	plug.window	= new Interpolator(sTopWin, "Interpolator");
	sTopWin->setWindowTitle("Interpolator (*by SLASH*)");		// to help with our grant
...
Interpolator::Interpolator(DockingDialog *parent, const char *name)
	: DialogFrame(parent, 0, NULL, NULL, true, "Interpolator", "", name)
{
  ...
  applyInterpolationButton = new QPushButton("Interpolate Contour [enter]",
	...
	changeMarkingButton = new QPushButton("Change Contour Marking", grpActions);
	changeMarkingButton->setFocusPolicy(Qt::NoFocus);
	connect(changeMarkingButton, SIGNAL(clicked()), this,
          SLOT(changeInterpContRange()));
	changeMarkingButton->setToolTip("Lets you mark a range of contours within the current "
                                  "object as\nkey contours or interpolated contours"
                                  " (hot key ; (semicolon))");
	vboxLayout1->addWidget(changeMarkingButton);
...
void Interpolator::moreActions()
{
  ...
  {
    case(0):      // find isolated contours
    {
      findIsolatedContours();
    } break;
      
    case(1):      // print model interpolation info
    {
      printModelInterpInfo();
    } break;
    
    case(2):      // print number contours each slice
    {
...
//------------------------
//-- Callback for the buttons at the bottom (no more)
...
void Interpolator::topCloseEvent ( QCloseEvent * e )
{
	imodDialogManager.remove((QWidget *)sTopWin);
...
void Interpolator::topChangeEvent(QEvent *e)
{
...
{
  int numDeleted = 0;
	vector<int> needDeletion;

	//## SEARCH DOWN AND FLAG MATCHING INTERPOLATED CONTOURS FOR DELETION:
  // DNM 11/19/19: Need to build a list of ones to mark because contoursSameSurf uses 
  // isEmpty which considers contour empty if it is marked for deletion
  ...
  for (int i = 0; i < numDeleted; i++)
    setDeleteFlag( getC(needDeletion[i]), 1 );
...
//## TO FORM "SMOOTH" INTERPOLATED CONTOURS
  ...
  // DNM 11/19/19: Need to delete all the existing contours in range
  deleteAllSameSurfInterpContsInZRange(getZ(contL), getZ(contU), clIdx);
