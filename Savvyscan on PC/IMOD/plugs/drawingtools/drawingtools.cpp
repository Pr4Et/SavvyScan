...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  if (!ivwGrayScaleImageLoaded(inImodView)) {
    wprint("\aDrawing Tools will not work on RGB or blank data\n");
    return;
  }
  
  ivwSetMovieModelMode( inImodView, IMOD_MMODEL );
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Drawing Tools", "plughelp/drawingtools.html#TOP", '4',
                              Qt::Window);
  plug.window  = new DrawingTools(sTopWin,"Drawing Tools");
  sTopWin->setWindowTitle("Drawing Tools (*by SQUIRE*)");		// to help with our grant
  sTopWin->addWidgetToStack(plug.window);

  adjustGeometryAndShow((QWidget *)sTopWin, IMOD_DIALOG );
  sTopWin->resizeToHintWidthAfterShow();
  imodDialogManager.add((QWidget *)sTopWin, IMOD_DIALOG, DOCKING_DIALOG_TYPE);

  plug.window->checkForNamelessObjects(false);    // check for objects without nam  
  //plug.window->show();
}
                  ...
                  int but1, int but2, int but3)
{
  ...
  // DNM: former logic was to assign press or release when the new value of butN matched
  // the old butNdown, then to take the butN as the new butNdown
  // This did not set release until the first mouse move after true release, because
  // butN is set on its release event, and screwed up livewire click at last point
  // This is changed to the true press and release signals, and butNdown is set only
  // during the press and move and NOT during the release, replicating its old setting
  plug.but1Pressed  = (but1 == 1) && (event->type() == QEvent::MouseButtonPress);
  plug.but2Pressed  = (but2 == 1) && (event->type() == QEvent::MouseButtonPress);
  plug.but3Pressed  = (but3 == 1) && (event->type() == QEvent::MouseButtonPress);
  
  plug.but1Released = (but1 == 1) && (event->type() == QEvent::MouseButtonRelease);
  plug.but2Released = (but2 == 1) && (event->type() == QEvent::MouseButtonRelease);
  plug.but3Released = (but3 == 1) && (event->type() == QEvent::MouseButtonRelease);
  plug.but1Down = plug.but1Released ? 0 : but1;  // right mouse   (using my preferred settings)
  plug.but2Down =  plug.but2Released ? 0 : but2; // left mouse    (using my preferred settings)
  plug.but3Down =  plug.but3Released ? 0 : but3; // middle mouse  (using my preferred settings)
...
DrawingTools::DrawingTools(DockingDialog *parent, const char *name) :
  DialogFrame(parent, 0, NULL, NULL, true, "Drawing Tools", "", name)
{
...
bool DrawingTools::drawExtraObjectLivewire( bool redraw )
{
  ...
  float sc = fDiv( 1.0f, zapZoom);			// tomogram distance for one screen pixel 
	...
	if( addLivewireSegment )
	{
      ...
      // DNM: Scale to screen pixels
			if( distFirstLWPt <= LW_SNAP_DIST * sc || distLastLWPt <= LW_SNAP_DIST * sc)
				finishContour = true;
		}
		...
		if( plug.lwUseWrap && plug.livewireF!=NULL && lastPtSelected )
		{
        ...
        // DNM: Scale to screen pixels
				bool removePts = (closestDist <= LW_SNAP_DIST * sc) && abs(closestPtIdx-ptIdx) >= 3;
...
void DrawingTools::loadSettings()
{
  ...
  if(nvals!=NUM_SAVED_VALS )
  {
    ...
    QMessageBox::about( this, "-- Documentation --",
                        ...
                        "we HIGHLY recommended you click '?' in the top \n"
                        " right and watch the video tutorial! \n\n"
                        "                                   -- Andrew Noske" );
    return;
  }
  ...
  plug.useArrowKeys               = savedValues[33];
	...
	//plug.lwAreaSize                 = savedValues[47]; // no longer used
...
void DrawingTools::saveSettings()
{
  ...
  saveValues[33]  = plug.useArrowKeys;
	...
	//saveValues[47]  = plug.lwAreaSize; // no longer used
...
{
	initLivewire();				// if not already, make sure livewire variables are initialized
	...
	ds.addRadioGrp( "what are you tracing?",
							    "(a) the middle of a dark region|"
							    "(b) the middle of a light region|"
							    "(c) the edge of a region|",
							    &plug.lwOpt,
								  ".....",
								  "choose this if want to draw contours through dark pixels |"
								  "choose this if want to draw contours through light pixels |"
								  "choose this if want to draw contours between light and dark pixels" );
	...
	ds.addComboBox( "binning:",
								 ...
								 "by averaging pixels in an area. Increasing this will reduce \n"
								 "noise in the image but also possibly reduce signal." );
	
	ds.addComboBox( "noise reduction:",
								 "median|"
								 "mean|"
								 "gaussian", &plug.lwNoiseRed,
								 "The method used to reduce noise either. If not binning a 3x3 \n"
								 "filter is used. If binning then this method is used to bin the \n"
								 "data. We recommend 'median'." );
	...
	ds.addHtmlLabel( "<font size='2.5'>This algorithm was developed by "
									 "<b>Jeffrey Bush</b>.<br>"
									 "<a href='https://cellsegmentation.org/download-tools/semi-automated-tools-for-manual-segmentation-and-data-annotation/livewire'>Click here</a> "
...
//-- "plug.lwBinning" and "plug.lwNoiseRed".
...
void DrawingTools::setupLivewireOptions()
{
	...
	Livewire::Weights::Settings     set(plug.lwSettings);
	// TODO: should this use HSV for RGB images?
	set.Method = Livewire::Weights::LumaSMPTE; // NOTE: if data is grayscale this setting is ignore
	set.EdgeDetection = (plug.lwOpt == LW_EDGE) ? Livewire::Weights::Sobel5 : Livewire::Weights::NoEdgeDetection;
	set.Accentuation  = Livewire::Weights::Sigmoid;
	bool isImageInverted = false; //TODO: ivwGetContrastReversed(plug.view);	// true if user toggle F11
	set.Invert = (plug.lwOpt == LW_LIGHT) ? !isImageInverted : isImageInverted;
		
	//** NOISE-REDUCTION / BINNING METHOD:
	
	set.NoiseReduction = Livewire::Weights::NoNoiseReduction;
	
	if( plug.lwBinning==0 )				// "none":
	{
		set.PixelReduction = Livewire::Weights::NoPixelReduction;
		if     ( plug.lwNoiseRed==0 )
			set.NoiseReduction = Livewire::Weights::MedianFilter3pxWindow;
		else if( plug.lwNoiseRed==1 )
			set.NoiseReduction = Livewire::Weights::MeanFilter3pxWindow;
		else if( plug.lwNoiseRed==2 )
			set.NoiseReduction = Livewire::Weights::GaussianFilter3pxWindow;
	}
	else if( plug.lwBinning==1 )		// "bin by 2":
	{
		if     ( plug.lwNoiseRed==0 )
			set.PixelReduction = Livewire::Weights::Median2pxWindow;
		else if( plug.lwNoiseRed==1 )
			set.PixelReduction = Livewire::Weights::Mean2pxWindow;
		else if( plug.lwNoiseRed==2 )
			set.PixelReduction = Livewire::Weights::Mean2pxWindow;
	}
	else if( plug.lwBinning==2 )		// "bin by 3":
	{
		if     ( plug.lwNoiseRed==0 )
			set.PixelReduction = Livewire::Weights::Median3pxWindow;
		else if( plug.lwNoiseRed==1 )
			set.PixelReduction = Livewire::Weights::Mean3pxWindow;
		else if( plug.lwNoiseRed==2 )
			set.PixelReduction = Livewire::Weights::Gaussian3pxWindow;
	}
	else if( plug.lwBinning==3 )		// "bin by 4":
	{
		if     ( plug.lwNoiseRed==0 )
			set.PixelReduction = Livewire::Weights::Median4pxWindow;
		else if( plug.lwNoiseRed==1 )
			set.PixelReduction = Livewire::Weights::Mean4pxWindow;
		else if( plug.lwNoiseRed==2 )
			set.PixelReduction = Livewire::Weights::Gaussian4pxWindow;
	}
	else if( plug.lwBinning==4 )		// "bin by 5":
	{
		if     ( plug.lwNoiseRed==0 )
			set.PixelReduction = Livewire::Weights::Median5pxWindow;
		else if( plug.lwNoiseRed==1 )
			set.PixelReduction = Livewire::Weights::Mean5pxWindow;
		else if( plug.lwNoiseRed==2 )
			set.PixelReduction = Livewire::Weights::Gaussian5pxWindow;
	}
	
	
	//** UPDATE THE SETTINGS OF "plug.weights" WITH NEW VALUES:
	
	if( plug.lwSettings != set )
	{
		plug.lwSettings = set;
		plug.weights->SetSettings(set);
	}
...
void DrawingTools::showWandOptions()
{
  //## GET USER INPUT FROM CUSTOM DIALOG:
	...
	ds.addHtmlLabel( "<font size='2.5'>This algorithm was based on code by <br>"
									...
									"<a href='http://cellsegmentation.org/'>Click here</a> "
									"for video demo & source code.</fontsize>","" );
	ds.setStylePrev( "background-color: rgb(100, 255, 100);");			// light green
...
void DrawingTools::initLivewire( )
{
	if( plug.lwInit == true )
		return;
	...
	if( plug.weights == NULL )
	{
		plug.weights = new Livewire::Weights();
	}
	if( plug.livewire == NULL )
	{
		plug.livewire = new Livewire::LivewireCalculator(plug.weights);
		plug.window->connect(plug.livewire, SIGNAL(finished()), plug.window, SLOT(livewireFinished()));
	}
	if( plug.livewireF == NULL )
	{
		plug.livewireF = new Livewire::LivewireCalculator(plug.weights);
		plug.window->connect(plug.livewireF, SIGNAL(finished()), plug.window, SLOT(livewireFinished()));
	}
	if( plug.lwPts == NULL )
	{
...
//------------------------
//-- Callback for the buttons at the bottom  (no more buttons for now)
...
void DrawingTools::topCloseEvent ( QCloseEvent * e )
{
  imodDialogManager.remove((QWidget *)mTopWin);
  clearExtraObj();
  ...
  ivwFreeTileCachedSection(plug.view);
	...
	if( plug.lwPts!=NULL )		{ imodContourDelete(plug.lwPts); plug.lwPts = NULL; }
	Livewire::PointPriorityQueue::EntryClear();
	Livewire::BlockPool::Clear();
...
void DrawingTools::topChangeEvent(QEvent *e)
{
  mRoundedStyle = ivwGetRoundedStyle();
...
void  edit_executeSculptPush( Ipoint center, float radius )
{
  ...
  if( isContValid(cont) && imodContourZValue(cont) == center.z )
  {
    ...
    // add extra points nearby to ensure circle does not lie "between points"
    cont_addPtsNearby( cont, radius/3.0f, isContClosed(obj,cont), center, radius * 3.f );
...
//------------------------
//-- Returns whether the distance from the bounding box to the center point is less
//-- than the radius

bool edit_doesBBoxTouchCircle( Ipoint *ll, Ipoint *ur, Ipoint *center, float radius )
{
  int izCen = B3DNINT(center->z);
  if( izCen < B3DNINT(ll->z) || izCen > B3DNINT(ur->z) )
    return false;
...
void edit_executeLivewireClick()
{
  ...
  if( startNew )
    ...
    imodSetIndex(imod, objIdx, newContPos, 0);
		...
		edit_startLivewireFromPt( mX, mY, mZ, plug.lwUseWrap );
		return;
	}
	...
	bool lastPtSelected     = (ptIdx >= psize(cont)-1);
	int ptsAdded = 0;
  float distLastLWPt = 0.;
  float distFirstLWPt = 0.;
	QPoint qptEnd = QPoint( mX, mY );
	
	if( numLivewirePts >= 3 && lastPtSelected )
	{
		distFirstLWPt = line_distBetweenPts2D( getFirstPt(plug.lwPts), &plug.mouse);
		distLastLWPt  = line_distBetweenPts2D( getLastPt(plug.lwPts),  &plug.mouse);

    // DNM: Scale to screen pixels
		if( distFirstLWPt <= LW_SNAP_DIST / zapZoom || distLastLWPt <= LW_SNAP_DIST / zapZoom )
			finishContour = true;
	}
	...
	if(lastPtSelected)
	{
    // DNM: When finishing with a click on last point, it needs to not add the livewire
    // line up to the point; this either gives spurious lines or removes the contour
    if (!(plug.lwUseWrap && finishContour && distLastLWPt < 2.))
      ptsAdded += edit_addLivewirePtsToCont( cont, -1,
        (plug.lwUseWrap && firstSegment) ? plug.livewireF : plug.livewire, qptEnd, mZ, true, true );
    // DNM: And when finishing with a click on the first point, it needs to not add the
    // livewire line from there to the end, which generally removes the contour
		if( plug.lwUseWrap && finishContour && distFirstLWPt >= 2.)
		{
			ptsAdded += edit_addLivewirePtsToCont( cont, -1, plug.livewireF, qptEnd, mZ, true, false );
		}
	}
	...
	else
	{
      ...
      // DNM: Scale to screen pixels
      bool removePts = (closestDist <= LW_SNAP_DIST / zapZoom) && abs(ptDiff) >= 3;
...
bool edit_startLivewireFromPt( int x, int y, int z, bool useLivewireF )
{
	...
	if( useLivewireF )
	{
		plug.livewireF->Start(x,y, LW_AREA);
	}
	else
	{
		plug.livewire->Start(x,y, LW_AREA);
	}
...
{	
	...
	Livewire::Weights::DataFormat format;
	if     (mode==0)				format = Livewire::Weights::GrayscaleByte;
	else if(mode==MRC_MODE_USHORT)	format = Livewire::Weights::GrayscaleUShort;
	else							format = Livewire::Weights::RGB;
	
	uint stride = data[1] - data[0];
	plug.weights->SetImage(data[0], plug.xsize, plug.ysize, format, stride);
}
