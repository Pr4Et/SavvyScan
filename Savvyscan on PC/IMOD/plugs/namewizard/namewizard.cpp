...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  plug.window->setWindowTitle("Name Wizard (*by SQUIRE*)");		// to help with our grant
...
static const char *buttonTips[]   = {(char*)"Close this plugin window",
	                             (char*)"See SQUIRE help videos showing \n"
...
void NameWizard::buttonPressed(int which)
{
  if      (which==0)
    close();
		...
		openUrl( "https://www.youtube.com/watch?v=9wno30ACni4" );
  // openUrl("https://cellsegmentation.org/download-tools/semi-automated-tools-for-manual-segmentation-and-data-annotation/imod-plugins/name-wizard-plugin");
	else if (which==2)
    helpPluginHelp();
}
...
void NameWizard::changeEvent(QEvent *e)
{
