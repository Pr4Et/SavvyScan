...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  plug.window  = new Stereology(imodDialogManager.parent(IMOD_DIALOG),"Stereology");
	plug.window->setWindowTitle("Stereology (*by SQUIRE*)");		// to help with our grant
...
static const char *buttonLabels[] = {(char*)"Done", (char*)"Intro", 
                                     (char *)"Help"};
static const char *buttonTips[]   = {(char*)"Close this plugin window",
	                             (char*)"See SQUIRE help page with pictures showing\n"
...
void Stereology::buttonPressed(int which)
{
  if      (which==0)
    close();
		...
		openUrl( "https://cellsegmentation.org/download-tools/tools-for-image-interpretation/stereology-plugin-introduction" );
	else if (which==2)
    helpPluginHelp();
}
...
void Stereology::changeEvent(QEvent *e)
{
