...
unsigned char *tiff_read_section(FILE *fp, Tf_info *tiff, int section)
{
  ...
  if (tiff->iifile) {
    if (tiff->iifile->file == IIFILE_RAW) {
      if (iiReadSection(tiff->iifile, (char *)tiff->data, section))
        return NULL;
    } else {
...
int tiff_open_file(char *filename, char *mode, Tf_info *tiff, int anyTifPixel)
{
  ...
  if (tiff->iifile) {
    ...
    strncpy(tiff->iifile->fmode, mode, 3);
    ...
    if (iiTIFFCheck(tiff->iifile)) {
      ...
      /* Just let the library expand a colormap image */
      tiff->PhotometricInterpretation = (tiff->iifile->mode == MRC_MODE_RGB) ? 2 : 1;
