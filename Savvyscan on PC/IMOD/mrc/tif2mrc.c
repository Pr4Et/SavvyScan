...
static void usage(const char *progname) {
  ...
  printf("\t-B #    Write bytes as unsigned (for # 0) or signed (for # 1)\n");
...
int main( int argc, char *argv[])
{
  ...
  for (iarg = 1; iarg < argc - 1 ; iarg++){
    if (argv[iarg][0] == '-'){
      ...
      case 'B': /* Control signed nature of byte output */
        overrideWriteBytes(atof(argv[++iarg]) != 0 ? 1 : 0);
        break;
  ...
  if (replaceFileArgVec((const char ***)&argv, &argc, &iarg, &ydo))
    exit(1);
...
/* Look up each index value in colormap and output r,g,b values 
   This is now relevant only when NOT using the tiff library */
static void expandIndexToRGB(unsigned char **datap, ImodImageFile *iifile, 
                              int section)
{
