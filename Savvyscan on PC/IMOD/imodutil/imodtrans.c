...
static void usage(char *progname)
{
  ...
  printf("\t-R #\tRotate model around X (1 forward, 0 backward\n");
...
int main(int argc, char *argv[])
{
  ...
  if (rotModel && (flipModel || toggleFlip))
    exitError("You cannot enter -R with either -F or -T");
  
  if (mode && rotScaleopt)  
    exitError("You cannot enter -r or -s options with -2 and -3");
  if (transopt && mode == 2 && oneLine < 0 && transz)
  ...
  if (rotModel) {
    ...
    imodRot90X(&model, rotLike3dmod ? 0 : 1);
    ...
    if (modRefp) {
      if (rotLike3dmod) {
        temp = modRefp->ctrans.z;
        modRefp->ctrans.z = model.zmax * modRefp->cscale.y - modRefp->ctrans.y;
        modRefp->ctrans.y = temp;
        modRefp->crot.x -= 90.;
      } else {
        temp = modRefp->ctrans.y;
        modRefp->ctrans.y = model.ymax * modRefp->cscale.z - modRefp->ctrans.z;
        modRefp->ctrans.z = temp;
        modRefp->crot.x += 90.;
      }
      temp = modRefp->cscale.z;
      modRefp->cscale.z = modRefp->cscale.y;
      modRefp->cscale.y = temp;
    }
  }
