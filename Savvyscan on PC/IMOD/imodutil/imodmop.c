...
static int sScat2D = 0, sScat3D = 0;
static int sTubes2D = 0;
static int sUseScans = 0;
static Ival sBkgVal, sMaskVal;
static IloadInfo *sLIp;
static int sNumChan;
static int sMasking = 0;
static int sTaper = 0;
static float sPadding = 0.;
static float sWindowTable[TABLE_SIZE+1];
static int sRetainOutside = 0;
...
int main( int argc, char *argv[])
{
  ...
  int numDiams, numTubes = 0, numLabel = 0, border = -1000;
  int *allsecList;
  int numAllsec = 0;
  ...
  int nxout, nyout, nzout, inside, numNeed, numOptArgs,numNonOptArgs;
  ...
  const char *options[] = {
    ...
    "fc:FillColor:FT:", "mask:MaskValue:F:", "label:LabelMaskList:LI:",
    "retain:RetainOutsideMask:B:", "mode:ModeToOutput:I:", "pad:PaddingSize:F:",
    "taper:TaperOverPad:I:", "objects:ObjectsToDo:LI:", "2dscat:2DScatteredPoints:B:",
    "3dscat:3DScatteredPoints:B:", "tube:TubeObjects:LI:", "diam:DiameterForTubes:F:",
    "planar:PlanarTubes:B:", "allsec:AllSectionObjects:LI:", "color:ColorOutput:B:",
    ...
    "fast:FastLegacyMethod:B:", "help:usage:B:"};
  ...
  sMasking = 1 - PipGetFloat("MaskValue", &mask);
  PipGetFloat("PaddingSize", &sPadding);
  PipGetInteger("TaperOverPad", &sTaper);
  if (sTaper > 1)
    for (i = 0; i <= TABLE_SIZE; i++) {
      size = (TABLE_SIZE - i) * 2.5 / TABLE_SIZE;
      sWindowTable[i] = (float)exp(-size * size / 2.);
    }
  ...
  PipGetBoolean("FastLegacyMethod", &sUseScans);
  PipGetBoolean("2DScatteredPoints", &sScat2D);
  PipGetBoolean("3DScatteredPoints", &sScat3D);
  PipGetBoolean("PlanarTubes", &sTubes2D);
  if (sScat2D && sScat3D)
  ...
  if (!PipGetString("LabelMaskList", &listString)) {
    if (sMasking)
      exitError("You cannot enter both -mask and -label options");
    sMasking = 1;
    labelList = parselist(listString, &numLabel);
    numNeed = numObj > 0 ? numObj : imod->objsize;
    if (!labelList) {

      /* Make sure it is a / entry and allocate and make default list */
      if (numLabel != -1)
        exitError("Bad entry in list of label mask values");
      labelList = B3DMALLOC(int, numNeed);
      if (!labelList)
        exitError("Failed to allocate default label mask list");
      for (i = 0; i < numNeed; i++)
        labelList[i] = numObj > 0 ? objList[i] : i + 1;
      numLabel = numNeed;
    } else if (numNeed != numLabel) {
      exitError("The label mask list has %d values and %d are needed", numLabel, numNeed);
    }
  }
  PipGetBoolean("RetainOutsideMask", &sRetainOutside);
  if (sRetainOutside && !sMasking)
    exitError("You can use -retain only when outputting a mask");
  if (sRetainOutside && (rgbOut || invert))
    exitError("You cannot use -retain with -invert or -color");
    ...
    for (i = 0; i < numAllsec; i++) {
      ...
      if (iobjOpen(imod->obj[objnum].flags))
        exitError("You cannot include an open contour object (%d) on an all-section list",
                  objnum + 1);
      if (iobjScat(imod->obj[objnum].flags) && (sScat3D || !sScat2D))
  ...
  if (!PipGetInteger("ModeToOutput", &outMode)) {
    ...
    if (hdata.mode == MRC_MODE_RGB && !sMasking)
  ...
  if (sMasking) {
    for (i = 0; i < B3DMAX(1, numLabel); i++) {
      ...
      if (((outMode == MRC_MODE_RGB || outMode == MRC_MODE_BYTE) &&
        ...
        exitError("Mask value %g is outside allowed range for output data mode", mask);
      if (mask == bkgFill || (outMode != MRC_MODE_FLOAT && (int)mask == (int)bkgFill))
        exitError("Mask value %g is indistinguishable from fill value %g", mask, bkgFill);
    }
  }

  if (hdata.mode == MRC_MODE_RGB || rgbOut) {
    sBkgVal[0] = bkgRed * bkgFill;
    sBkgVal[1] = bkgGreen * bkgFill;
    sBkgVal[2] = bkgBlue * bkgFill;
  } else 
    sBkgVal[0] = sBkgVal[1] = sBkgVal[2] = bkgFill;
        ...
        if (sScat2D || sScat3D) {
          ...
          for (co = 0; co < obj->contsize; co++) {
            for (pt = 0; pt < obj->cont[co].psize; pt++) {
              ...
              minpt.z = B3DMIN(minpt.z, pts->z - (sScat3D ? size : 0));
              maxpt.x = B3DMAX(maxpt.x, pts->x + size);
              maxpt.y = B3DMAX(maxpt.y, pts->y + size);
              maxpt.z = B3DMAX(maxpt.z, pts->z + (sScat3D ? size : 0));
            }
          }
        }
      } else {
  ...
  if (sMasking && outMode == MRC_MODE_BYTE)
  ...
  for (k = li.zmin; k <= li.zmax; k++) {
    ...
    fflush(stdout);
    if ((!sMasking || sRetainOutside) && mrcReadZ(&hdata, &li, islice->data.b, k))
    ...
    if (sRetainOutside) {
      ...
      for (iy = 0; iy < nyout; iy++) {
        for (ix = 0; ix < nxout; ix++) {
          ...
          slicePutVal(pslice[0], ix, iy, val);
        }
      }
    }
    ...
    for (inside = 0; inside < 2; inside++) {
      for (objnum = 0; objnum < imod->objsize; objnum++) {
        ...
        if (numLabel) {
          if (numObj) {
            for (i = 0; i < numObj; i++)
              if (objList[i] == objnum + 1)
                mask = labelList[i];
          } else
            mask = labelList[objnum];
        }
        sMaskVal[0] = sMaskVal[1] = sMaskVal[2] = mask;
        if (hdata.mode == MRC_MODE_COMPLEX_FLOAT || hdata.mode == MRC_MODE_COMPLEX_SHORT)
          sMaskVal[1] = sMaskVal[2] = 0.;
                      ...
                      Islice *pslice[3], int fill)
{
  ...
  for (j = 0; j < cont->psize; j += 2) {
    ...
    if (y < sLIp->ymin || y > sLIp->ymax)
      continue;

    /* And here truncation matches the positions better than taking the nint,
       and running to < xnd instead of to <= xnd.  */
    xst = cont->pts[j].x;
    xst = B3DMAX(xst, sLIp->xmin);
    xnd = cont->pts[j + 1].x;
    xnd = B3DMIN(xnd, sLIp->xmax);
    for (x = xst; x < xnd; x++) {

      /* Get value if filling */
      if (fill) {
        sliceGetVal(islice, x - sLIp->xmin, y - sLIp->ymin, inval);
        putSliceValue(obj, islice, pslice, x, y, inval, 1.);
      } else {

        /* Or output the background value */
        if (sNumChan < 3)
          slicePutVal(pslice[0],  x - sLIp->xmin, y - sLIp->ymin, sBkgVal);
        else {
          
          slicePutVal(pslice[0],  x - sLIp->xmin, y - sLIp->ymin, redval);
          slicePutVal(pslice[1],  x - sLIp->xmin, y - sLIp->ymin, grnval);
          slicePutVal(pslice[2],  x - sLIp->xmin, y - sLIp->ymin, bluval);
        }
      }
    }
  }
                        ...
                        int fill)
{
  ...
  float morePad = B3DMAX(0., fill ? sPadding : -sPadding);
  float dist, taperFrac, absPad;
  int doAllInside = (fill && sPadding >= 0.) || (!fill && sPadding <= 0.);
  int lookOutside = (fill && sPadding > 0.) || (!fill && sPadding < 0.);
  ...
  absPad = (float)fabs((double)sPadding);
  imodContourGetBBox(cont, &pmin, &pmax);
  xst = B3DMAX((int)(pmin.x - 1 - morePad), sLIp->xmin);
  xnd = B3DMIN((int)(pmax.x + 2 + morePad), sLIp->xmax);
  yst = B3DMAX((int)(pmin.y - 1 - morePad), sLIp->ymin);
  ynd = B3DMIN((int)(pmax.y + 2 + morePad), sLIp->ymax);
  pnt.z = 0.;
  for (y = yst; y <= ynd; y++) {
    for (x = xst; x <= xnd; x++) {
      pnt.x = x + 0.5;
...
void paintScatPoints(Iobj *obj, Islice *islice, Islice *pslice[3], int iz, int allsec)
{
  ...
  for (co = 0; co < obj->contsize; co++) {
    ...
    for (pt = 0; pt < cont->psize; pt++) {
      ...
      if (size <= 0. || pnt->x + size < sLIp->xmin || pnt->x - size > sLIp->xmax || 
          pnt->y + size < sLIp->ymin || pnt->y - size > sLIp->ymax)
        continue;
  ...
  for (co = 0; co < obj->contsize; co++) {
    ...
    for (pt = B3DMIN(1, cont->psize - 1); pt < cont->psize; pt++) {
      ...
      if ((!sTubes2D && 
        ...
        xst = B3DMAX(xst, sLIp->xmin);
        xnd = B3DMIN(xnd, sLIp->xmax);
        yst = B3DMAX(yst, sLIp->ymin);
        ynd = B3DMIN(ynd, sLIp->ymax);
        for (y = yst; y <= ynd; y++) {
          for (x = xst; x <= xnd; x++) {
            float dist;
            pix.x = x + 0.5f;
                          ...
                          int x, int y, Ival inval, float taperFrac)
{
  ...
  if (sNumChan < 3 && !sMasking && taperFrac > 0.99999) {
    slicePutVal(pslice[0],  x - sLIp->xmin, y - sLIp->ymin, inval);
    return;
  }
  ...
  if (taperFrac <= 0.9999) {
    if (sTaper > 1)
      taperFrac = sWindowTable[(int)(TABLE_SIZE * taperFrac)];
    useval[0] = taperFrac * useval[0] + (1. - taperFrac) * sBkgVal[0];
    useval[1] = taperFrac * useval[1] + (1. - taperFrac) * sBkgVal[1];
    useval[2] = taperFrac * useval[2] + (1. - taperFrac) * sBkgVal[2];
  }

  if (sNumChan < 3) {
    slicePutVal(pslice[0],  x - sLIp->xmin, y - sLIp->ymin, useval);
  } else {
    
    /* If doing 3 channels, only first element is needed */
    outval[0] = obj->red * useval[0];
    slicePutVal(pslice[0],  x - sLIp->xmin, y - sLIp->ymin, outval);
    outval[0] = obj->green * useval[0];
    slicePutVal(pslice[1],  x - sLIp->xmin, y - sLIp->ymin, outval);
    outval[0] = obj->blue * useval[0];
    slicePutVal(pslice[2],  x - sLIp->xmin, y - sLIp->ymin, outval);
  }
}
