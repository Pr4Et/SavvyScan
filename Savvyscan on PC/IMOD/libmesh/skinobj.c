 ...
 *
 *  Copyright (C) 1995-2018 by the Regents of the University of
...
static Imesh *imeshContourCap(Iobj *obj, Icont *cont, Icont *scanCont, int co, int side,
...
static Icont *connect_orphans(Iobj *obj, Imesh **objMesh, int *meshsize, Icont *cout,
                              int *list, int *used, int *justinl, int njustin,
                              int *inlist, int joindir, int oddeven, int surf, int time,
...
static int mesh_contours(Iobj *obj, Imesh **objMesh, int *meshsize, Icont *bcont,
                         Icont *tcont, int surf, int time, Ipoint *scale, int inside,
                         int bco, int tco);
...
static int robustCenterOfMass(Icont *cont, Icont *scanCont, Ipoint *pt);
...
static double timeval1, timeval2;
...
void skin_report_time(char *string)
{
  ...
  float elapsed;
  if (doReport < 0)
    doReport = (getenv("IMOD_MESH_TIMING") != NULL) ? 1 : 0;
  if (!doReport)
    return;
  timeval2 = wallTime();
  elapsed = (float)(timeval2 - timeval1);
  printf("%s: %.3f\n",string, elapsed);
  timeval1 = timeval2;
}
 ...
 */
int imeshSkinObject(Iobj *obj, Ipoint *scale, double overlap, int doCap,
                    int *capSkipZlist, int capSkipNumZ, int zIncrement,
                    unsigned int flags, int skipPasses, double tubeDiameter,
                    int (*inCallBack)(int))
{
  int coNum, topCoNum, botCoNum, meshInd;
  Imesh *newMesh = NULL;
  Imesh *tempMeshes[MAX_THREADS];
  Icont *cont, *topCont, *botCont, *joinCont;
  Icont **scanConts;
  int zMin, zMax;
  int ind, jnd, maxContPerSec;
  int foundData, forceConnect, topCnctNum, botCnctNum;
  int *zList;
  int zListSize = 0;
  int nextZ, prevZ;
  int direction;
  int foundInside;
  int numThreads, thread, error, totalPoints = 0, dummyShare = 0;
  int tempMeshSize[MAX_THREADS];
  int maxThreads = 8;

  int (*callback)(int);
  int status;

  int *botList[MAX_THREADS], *topList[MAX_THREADS], *contourZ;
  Ipoint *ptMin, *ptMax;
  int numBot, numTop, contInd, listInd;
  int *numContAtZ;
  int **contsAtZ;
  int indZ, indAtZ, jndAtZ;
  int numNests = 0;
  Icont *otherCont;  /* The contour that will be tested    */
  int otherCoNum;       /* for being eaten all up.            */
  float frac1, frac2, dist, minDist;
  Nesting *nest, *nests;
  int *contNestInd;
  int nestInd, zPass;
  Ipoint botCenMass, topCenMass;
  ...
  if (imodContourMakeZTables(obj, zIncrement, CONNECT_BOTH | ICONT_CONNECT_INVERT,
                             &contourZ, &zList, &numContAtZ, &contsAtZ, &zMin, &zMax,
                             &zListSize, &maxContPerSec))
    return -1;


  /* Allocate space for lists of min's max's, and found contours */

  ptMin = B3DMALLOC(Ipoint, obj->contsize);
  ptMax = B3DMALLOC(Ipoint, obj->contsize);
  if (!ptMin || !ptMax)
    return(-1);
  
  for (coNum = 0; coNum < obj->contsize; coNum++)
    if (obj->cont[coNum].psize)
      imodContourGetBBox(&(obj->cont[coNum]), &(ptMin[coNum]), &(ptMax[coNum]));
  
  /* OPEN OBJECT: GO OFF AND DO IT */
  if (!iobjClose(obj->flags))
    return(mesh_open_obj(obj, scale, zIncrement, flags, skipPasses, zMin, zMax,
                         contourZ, zList, zListSize, numContAtZ, contsAtZ, ptMin, ptMax));
  
  /* Tell callback function we are just getting started. */
  if (inCallBack) 
    callback = inCallBack;
  else 
    callback = imeshDefaultCallback;
  status = (*callback)(1);
   ...
   */
  scanConts = B3DMALLOC(Icont *, obj->contsize);
  if (!scanConts) 
    return(-1);
  for (coNum = 0; coNum < obj->contsize; coNum++) {
    scanConts[coNum] = imodel_contour_scan(&(obj->cont[coNum]));
    totalPoints += obj->cont[coNum].psize;
  }

  /* printf("Total points %d\n", totalPoints); */
  /* This gives 2 threads at 6800 points, 6 at 91200, 8 at 170000 */
  numThreads = B3DNINT(sqrt((double)totalPoints) / 55.);
  maxThreads = B3DMIN(maxThreads, zMax + 1 - zMin);
  B3DCLAMP(numThreads, 1, maxThreads);
  numThreads = numOMPthreads(numThreads);
  numThreads = B3DMIN(numThreads, MAX_THREADS);
  for (thread = 0; thread < numThreads; thread++) {
    tempMeshSize[thread] = 0;
    tempMeshes[thread] = NULL;
    botList[thread] = B3DMALLOC(int, obj->contsize);
    topList[thread] = B3DMALLOC(int, obj->contsize);
    if (!botList[thread] || !topList[thread])
      return(-1);
  }
  ...
  contNestInd = B3DMALLOC(int, obj->contsize);
  if (!contNestInd)
    return(-1);
  for (coNum = 0; coNum < obj->contsize; coNum++)
    contNestInd[coNum] = -1;
  ...
  for (indZ = 0; indZ < zMax + 1 - zMin; indZ++) {
    for (jndAtZ = 0; jndAtZ < numContAtZ[indZ] - 1; jndAtZ++) {
      coNum = contsAtZ[indZ][jndAtZ];
      cont = &(obj->cont[coNum]);
      if (!cont->psize)
        cont->flags |= CONNECT_BOTH;
      if (cont->flags & CONNECT_BOTH)
        continue;
      for (indAtZ = jndAtZ + 1; indAtZ < numContAtZ[indZ]; indAtZ++) {
        otherCoNum = contsAtZ[indZ][indAtZ];
        otherCont = &(obj->cont[otherCoNum]);
        if (!otherCont->psize)
          otherCont->flags |= CONNECT_BOTH;
        if (otherCont->flags & CONNECT_BOTH)
          continue;
        if ((flags & IMESH_MK_TIME) && (cont->time != otherCont->time))
          continue;

        if (imodContourCheckNesting(coNum, otherCoNum, scanConts, ptMin, ptMax, &nests,
                                    contNestInd, &numNests, &numWarn))
          return -1;
      }
    }
  }
  ...
  imodContourNestLevels(nests, contNestInd, numNests);
  ...
  for (nestInd = 0; nestInd < numNests; nestInd++) {
    nest = &nests[nestInd];
    if (/*(nest->level % 2) == 1 && */ nest->ninside > 0) {
      nest->inscan = imodContourDup(scanConts[nest->co]);
      for (ind = 0; ind < nest->ninside; ind++)
        subtract_scan_contours(nest->inscan, scanConts[nest->inside[ind]]);
    }
  }
  ...
  status = (*callback)(25); /* quarter of the way done. */
  /*status = (*cb)(1); ? */
  if (status) return(status);
  ...
  for (zPass = 1; zPass <= skipPasses; zPass++) {
    ...
    /* Windows won't take stderr as a shared variable so pass it something innocuous */
#ifdef _WIN32
#define STDERR_SHARE dummyShare
#else
#define STDERR_SHARE stderr
#endif
#pragma omp parallel for default (none) num_threads(numThreads)         \
  shared(zMax, zMin, numContAtZ, contsAtZ, obj, contourZ, zPass, flags, zList, \
         zListSize, zIncrement, contNestInd, nests, scanConts, ptMin, ptMax, overlap, \
         botList, topList, scale, STDERR_SHARE, error, tempMeshes, tempMeshSize) \
  private(indZ, jndAtZ, coNum, cont, botCont, nextZ, listInd, prevZ, foundData, numBot, \
          numTop, foundInside, indAtZ, topCoNum, joinCont, topCnctNum, contInd, \
          botCnctNum, forceConnect, frac1, frac2, topCont, ind, jnd, nest, thread)
    for (indZ = 0; indZ < zMax + 1 - zMin; indZ++) {
      if (error)
        continue;
      thread = b3dOMPthreadNum();
      for (jndAtZ = 0; jndAtZ < numContAtZ[indZ] && !error; jndAtZ++) {
        coNum = contsAtZ[indZ][jndAtZ];
        cont = &(obj->cont[coNum]);
        ...
        /* define z-sections that connections will be made to; step farther
           in Z for each pass */
        nextZ = contourZ[coNum];
        for (listInd = 0; listInd < zPass; listInd++) {
          if (flags & IMESH_MK_SKIP) {
            nextZ = getnextz(zList, zListSize, nextZ);
          }else{
            nextZ += zIncrement;
          }
          if (nextZ > zList[zListSize - 1])
            break;
        }
        prevZ = contourZ[coNum];
        foundData = 1;

        numBot = 1;
        numTop = 0;
        botList[thread][0] = coNum;
        foundInside = 0;
        if (contNestInd[coNum] >= 0) {

          /* If inside/outside, find outermost */
          foundInside = 1;
          add_whole_nest(contNestInd[coNum], obj, nests, contNestInd, CONNECT_TOP,
                         botList[thread], &numBot);
        }
        ...
        while(foundData) {
          ...
          for (indAtZ = 0; indAtZ < numContAtZ[nextZ - zMin]; indAtZ++) {
            topCoNum = contsAtZ[nextZ - zMin][indAtZ];

            /* The current contour we are going to try to join. */
            joinCont = &(obj->cont[topCoNum]);
            ...
            if (!joinCont->psize)
              continue;
            if (connectBottom(joinCont->flags))
              continue;
            if ((flags & IMESH_MK_SURF) && botCont->surf != joinCont->surf)
              continue;
            if ((flags & IMESH_MK_TIME) && botCont->time != joinCont->time)
              continue;

            /* See if it overlaps any contour in bottom list */
            topCnctNum = istoreConnectNumber(obj->store, topCoNum);
            for (listInd = 0; listInd < numBot; listInd++) {
              contInd = botList[thread][listInd];

              /* Skip if they both have connection numbers and don't match, or
                 force connection if they match */
              botCnctNum = istoreConnectNumber(obj->store, contInd);
              if (topCnctNum >= 0 && botCnctNum >= 0 && topCnctNum != botCnctNum)
                continue;
              forceConnect = (topCnctNum >= 0 && botCnctNum >= 0 && 
                              topCnctNum == botCnctNum) ? 1 : 0;

              if (imodel_scans_overlap(scanConts[contInd], ptMin[contInd], ptMax[contInd],
                                       scanConts[topCoNum], ptMin[topCoNum], 
                                       ptMax[topCoNum]) || forceConnect) {
                if (!forceConnect && overlap != 0.0) {
                  /* contours must overlap by given fraction*/
                  imodel_overlap_fractions(&scanConts[contInd], ptMin[contInd],
                                           ptMax[contInd], &scanConts[topCoNum],
                                           ptMin[topCoNum], ptMax[topCoNum], &frac1,
                                           &frac2);
                  if (frac1 <= overlap && frac2 <= overlap)
                    continue;
                }
                foundData = 1;
                joinCont->flags |= CONNECT_BOTTOM;
                topList[thread][numTop++] = topCoNum;
                if (contNestInd[topCoNum] >= 0) {

                  /* If inside/outside, find outermost */
                  foundInside = 1;
                  add_whole_nest(contNestInd[topCoNum], obj, nests, contNestInd,
                                 CONNECT_BOTTOM, topList[thread], &numTop);
                }
                break;
              }
            }
          ...
          if (!foundData)
            break;
          for (indAtZ = 0; indAtZ < numContAtZ[prevZ - zMin]; indAtZ++) {
            topCoNum = contsAtZ[prevZ - zMin][indAtZ];
            ...
            botCnctNum = istoreConnectNumber(obj->store, topCoNum);
            for (listInd = 0; listInd < numTop; listInd++) {
              contInd = topList[thread][listInd];
              topCnctNum = istoreConnectNumber(obj->store, contInd);
              if (topCnctNum >= 0 && botCnctNum >= 0 && topCnctNum != botCnctNum)
                continue;
              forceConnect = (topCnctNum >= 0 && botCnctNum >= 0 && 
                              topCnctNum == botCnctNum) ? 1 : 0;
              ...
              if (imodel_scans_overlap(scanConts[contInd], ptMin[contInd], ptMax[contInd],
                ...
                foundData = 1;
                joinCont->flags |= CONNECT_TOP;
                botList[thread][numBot++] = topCoNum;
                if (contNestInd[topCoNum] >= 0) {

                  /* If inside/outside, find outermost */
                  foundInside = 1;
                  add_whole_nest(contNestInd[topCoNum], obj, nests, contNestInd, 
                                 CONNECT_TOP, botList[thread], &numBot);
                }
                break;
              }
            }
        ...
        if (numTop && !foundInside) {
          botCont = join_all_contours(obj, botList[thread], numBot, numBot, 1, topList[thread],
                                      numTop, numTop, NULL);
          topCont = join_all_contours(obj, topList[thread], numTop, numTop, -1, botList[thread],
                                      numBot, numBot, NULL);
          if (mesh_contours(obj, &tempMeshes[thread], &tempMeshSize[thread], botCont,
                            topCont, cont->surf, cont->time, scale, 0,
                            botList[thread][0], topList[thread][0])) {
            error = 1;
            break;
          }
          ...
          int *topInsideList, *botInsideList, *topJustInList, *botJustInList;
          int *topOutList, *botOutList, *topUsed, *botUsed, *topLevels, *botLevels;
          Icont *botOutCont, *topOutCont;
          Nesting *botNest;
          float fracMax;
          int indMax, jndMax, topLevel, botLevel, numBotJustIn, numTopJustIn, found;
          int numTopOut, numBotOut, numTopInside, numBotInside, indJustIn, coNumJustIn;
          int numTopSame, numBotSame, oddEven, knd;
          Ipoint botPtMin, botPtMax, topPtMin, topPtMax;

          minFrac = B3DMALLOC(float, numTop * numBot);
          maxFrac = B3DMALLOC(float, numTop * numBot);
          topInsideList = B3DMALLOC(int, numTop);
          botInsideList = B3DMALLOC(int, numBot);
          topJustInList = B3DMALLOC(int, numTop);
          botJustInList = B3DMALLOC(int, numBot);
          topOutList = B3DMALLOC(int, numTop);
          botOutList = B3DMALLOC(int, numBot);
          topUsed = B3DMALLOC(int, numTop);
          botUsed = B3DMALLOC(int, numBot);
          topLevels = B3DMALLOC(int, numTop);
          botLevels = B3DMALLOC(int, numBot);

          if (!minFrac || !topInsideList || !botInsideList || !topJustInList || 
              !botJustInList || !topOutList || !botOutList || !topUsed || !botUsed ||
              !topLevels || !botLevels || !maxFrac) {
          ...
          for (ind = 0; ind < numTop; ind++) {
            topUsed[ind] = 0;
            topLevels[ind] = 1;
            topCont = scanConts[topList[thread][ind]];
            topPtMin = ptMin[topList[thread][ind]];
            topPtMax = ptMax[topList[thread][ind]];
            if (contNestInd[topList[thread][ind]] >= 0) {
              nest = &nests[contNestInd[topList[thread][ind]]];
              topLevels[ind] = nest->level;
              /* DNM 12/18/01: Need to include even levels also */
              if (/*nest->level % 2 &&*/ nest->ninside) {
                topCont = nest->inscan;
                imodContourGetBBox(topCont, &topPtMin, &topPtMax);
              }
            }
            for (jnd = 0; jnd < numBot; jnd++) {
              botUsed[jnd] = 0;
              botLevels[jnd] = 1;
              botCont = scanConts[botList[thread][jnd]];
              botPtMin = ptMin[botList[thread][jnd]];
              botPtMax = ptMax[botList[thread][jnd]];
              if (contNestInd[botList[thread][jnd]] >= 0) {
                botNest = &nests[contNestInd[botList[thread][jnd]]];
                botLevels[jnd] = botNest->level;
                if (/*bnest->level % 2 &&*/ botNest->ninside) {
                  botCont = botNest->inscan;
                  imodContourGetBBox(botCont, &botPtMin, &botPtMax);
                }
              }
              imodel_overlap_fractions(&botCont, botPtMin, botPtMax, &topCont,
                                       topPtMin, topPtMax, &frac1, &frac2);
              if (frac1 > frac2) {
                maxFrac[ind * numBot + jnd] = frac1;
                minFrac[ind * numBot + jnd] = frac2;
              } else {
                maxFrac[ind * numBot + jnd] = frac2;
                minFrac[ind * numBot + jnd] = frac1;
              }
            }
          }
          ...
          for (;;) {
            fracMax = -1.;
            for (ind = 0; ind < numTop; ind++) {
              if (!topUsed[ind] && (topLevels[ind] % 2 == oddEven))
                for (jnd = 0; jnd < numBot; jnd++) {
                  if (!botUsed[jnd] &&
                      (botLevels[jnd] % 2 == oddEven)) {
                    frac1 = minFrac[ind * numBot + jnd];
                    if (frac1 > fracMax) {
                      fracMax = frac1;
                      indMax = ind;
                      jndMax = jnd;
            ...
            /* Start new lists with best pair, take them off original
               lists */
            topLevel = topLevels[indMax];
            botLevel = botLevels[jndMax];
            numTopOut = 1;
            numBotOut = 1;
            topOutList[0] = indMax;
            botOutList[0] = jndMax;
            ...
            do {
              ...
              /* check every available top contour of same level
                 ...
                 overlap */
              for (ind = 0; ind < numTop; ind++) {
                if (!topUsed[ind] && topLevels[ind] == topLevel) {
                  for (jnd = 0; jnd < numBotOut; jnd++) {
                    if (maxFrac[ind * numBot + botOutList[jnd]] > overlap) {
                      topOutList[numTopOut++] = ind;
                      topUsed[ind] = 1;
                      foundData = 1;
                      break;
                    }
              ...
              for (jnd = 0; jnd < numBot; jnd++) {
                if (!botUsed[jnd] && botLevels[jnd] == botLevel) {
                  for (ind = 0; ind < numTopOut; ind++) {
                    if (maxFrac[topOutList[ind] * numBot + jnd] > overlap) {
                      botOutList[numBotOut++] = jnd;
                      botUsed[jnd] = 1;
                      foundData = 1;
                      break;
                    }
            ...
            numTopJustIn = 0;
            for (ind = 0; ind < numTop; ind++) {
              if (!topUsed[ind] && topLevels[ind] == topLevel + 1) {
                nest = &nests[contNestInd[topList[thread][ind]]];
                for (knd = 0; knd < nest->noutside; knd++) {
                  found = 0;
                  for (jnd = 0; jnd < numTopOut; jnd++) {
                    if (nest->outside[knd] == topList[thread][topOutList[jnd]]) {
                      found = 1;
                      topJustInList[numTopJustIn++] = ind;
                      break;
                    }
                  }
                  if (found)
            ...
            for (ind = 0; ind < numBot; ind++) {
              if (!botUsed[ind] && botLevels[ind] == botLevel + 1) {
                nest = &nests[contNestInd[botList[thread][ind]]];
                for (knd = 0; knd < nest->noutside; knd++) {
                  found = 0;
                  for (jnd = 0; jnd < numBotOut; jnd++) {
                    if (nest->outside[knd] == botList[thread][botOutList[jnd]]) {
                      found = 1;
                      botJustInList[numBotJustIn++] = ind;
                      break;
                    }
                  }
                  if (found)
            ...
            for (indJustIn = 0; indJustIn < numBotJustIn; indJustIn++) {
              if (!botUsed[botJustInList[indJustIn]]) {
                numTopInside = 0;
                numBotInside = 1;
                botInsideList[0] = botJustInList[indJustIn];
                botUsed[botJustInList[indJustIn]] = 1;
                do {
                  foundData = 0;
                  /* check on top list of just in contours
                     for ones that overlap list of overlapping
                     bottom conts */
                  for (jnd = 0; jnd < numTopJustIn; jnd++) {
                    coNumJustIn = topJustInList[jnd];
                    if (!topUsed[coNumJustIn])
                      for (knd = 0; knd < numBotInside; knd++) {
                        if (maxFrac[coNumJustIn * numBot + botInsideList[knd]] > overlap){
                          foundData = 1;
                          topInsideList[numTopInside++] = coNumJustIn;
                          topUsed[coNumJustIn] = 1;
                          break;
                        }
                      }
                  }
                  /* check on bottom list of just in contours
                     for ones that overlap list of overlapping
                     top conts */
                  for (jnd = 0; jnd < numBotJustIn; jnd++) {
                    coNumJustIn = botJustInList[jnd];
                    if (!botUsed[coNumJustIn])
                      for (knd = 0; knd < numTopInside; knd++) {
                        if (maxFrac[topInsideList[knd] * numBot + coNumJustIn] > overlap){
                          foundData = 1;
                          botInsideList[numBotInside++] = coNumJustIn;
                          botUsed[coNumJustIn] = 1;
                          break;
                        }
                      }
                  }
                ...
                if (numTopInside) {
                  numBotSame = numBotInside;
                  numTopSame = numTopInside;
                  ...
                  do {
                    foundData = 0;
                    for (ind = 0; ind < numTop; ind++) {
                      if (topUsed[ind] || topLevels[ind] != topLevel + 2)
                        continue;
                      ...
                      found = 0;
                      for (knd = 0; knd < nest->noutside; knd++)
                        for (jnd = 0; jnd < numTopInside; jnd++)
                          if (nest->outside[knd] == topList[thread][topInsideList[jnd]])
                            found = 1;
                      if (!found)
                        continue;
                      ...
                      found = 0;
                      for (knd = 0; knd < numBotOut; knd++)
                        if (maxFrac[ind * numBot + botOutList[knd]] > overlap)
                          found = 1;

                      /* see if overlap added ones */
                      for (knd = numBotSame; knd < numBotInside; knd++)
                        if (maxFrac[ind * numBot + botInsideList[knd]] > overlap)
                          found = 1;
                      if (found) {
                        foundData = 1;
                        topInsideList[numTopInside++] = ind;
                        topUsed[ind] = 1;
                      }
                    ...
                    for (ind = 0; ind < numBot; ind++) {
                      if (botUsed[ind] || botLevels[ind] != botLevel + 2)
                        continue;
                      ...
                      found = 0;
                      for (knd = 0; knd < nest->noutside; knd++)
                        for (jnd = 0; jnd < numBotInside; jnd++)
                          if (nest->outside[knd] == botList[thread][botInsideList[jnd]])
                            found = 1;
                      if (!found)
                        continue;
                      ...
                      /* see if overlap outsides */
                      found = 0;
                      for (knd = 0; knd < numTopOut; knd++)
                        if (maxFrac[topOutList[knd] * numBot + ind] > overlap)
                          found = 1;

                      /* see if overlap added ones */
                      for (knd = numTopSame; knd < numTopInside; knd++)
                        if (maxFrac[topInsideList[knd] * numBot +  ind] > overlap)
                          found = 1;
                      if (found) {
                        foundData = 1;
                        botInsideList[numBotInside++] = ind;
                        botUsed[ind] = 1;
                      }
                    }
                  ...
                  for (ind = 0; ind < numTopInside; ind++)
                    topInsideList[ind] = topList[thread][topInsideList[ind]];
                  for (ind = 0; ind < numBotInside; ind++)
                    botInsideList[ind] = botList[thread][botInsideList[ind]];

                  botCont = join_all_contours(obj, botInsideList, numBotInside, 
                                              numBotSame, 1, topInsideList, numTopInside,
                                              numTopSame, NULL);
                  topCont = join_all_contours(obj, topInsideList, numTopInside, 
                                              numTopSame, -1, botInsideList, numBotInside,
                                              numBotSame, NULL);
                  if (mesh_contours(obj, &tempMeshes[thread], &tempMeshSize[thread],
                                    botCont, topCont, cont->surf, cont->time,
                                    scale, oddEven, botInsideList[0], topInsideList[0])) {
                    error = 1;
                    break;
                  }
                } else
                  botUsed[botJustInList[indJustIn]] = 0;
              }
            }
            if (error)
              break;
            ...
            /* There may now be orphan inside contours left.  At this
               ...
               outside contours */
            for (ind = 0; ind < numTopOut; ind++)
              topOutList[ind] = topList[thread][topOutList[ind]];
            for (ind = 0; ind < numBotOut; ind++)
              botOutList[ind] = botList[thread][botOutList[ind]];

            botOutCont = join_all_contours(obj, botOutList, numBotOut, numBotOut, 1,
                                           topOutList, numTopOut, numTopOut, NULL);
            topOutCont = join_all_contours(obj, topOutList, numTopOut, numTopOut, -1,
                                           botOutList, numBotOut, numBotOut, NULL);
            if (!botOutCont || !topOutCont) {
              error = 1;
              break;
            }

            botOutCont = connect_orphans(obj, &tempMeshes[thread], &tempMeshSize[thread],
                                         botOutCont, topList[thread], topUsed, 
                                         topJustInList, numTopJustIn, topInsideList, -1,
                                         oddEven, cont->surf, cont->time,
                                         scale, scanConts, ptMin, ptMax);
            topOutCont = connect_orphans(obj, &tempMeshes[thread], &tempMeshSize[thread],
                                         topOutCont, botList[thread], botUsed,
                                         botJustInList, numBotJustIn, botInsideList,
                                         1, oddEven, cont->surf, cont->time,
                                         scale, scanConts, ptMin, ptMax);
            /* finally ready to mesh the outside contours, possibly
               changed by orphan handling */
            if (mesh_contours(obj, &tempMeshes[thread], &tempMeshSize[thread], 
                              botOutCont, topOutCont, cont->surf, cont->time,
                              scale, 1 - oddEven, botOutList[0], topOutList[0])) {
              error = 1;
              break;
            }
          ...
          for (ind = 0; ind < numBot; ind++)
            if (botUsed[ind] <= 0)
              obj->cont[botList[thread][ind]].flags &= ~CONNECT_TOP;
          for (ind = 0; ind < numTop; ind++)
            if (topUsed[ind] <= 0)
              obj->cont[topList[thread][ind]].flags &= ~CONNECT_BOTTOM;
  ...
  /* Add up number of meshes and do one allocation/reallocation of object mesh 
     then transfer the meshes */
  totalPoints = 0;
  for (thread = 0; thread < numThreads; thread++)
    totalPoints += tempMeshSize[thread];
  if (obj->meshsize)
    B3DREALLOC(obj->mesh, Imesh, obj->meshsize + totalPoints);
  else
    obj->mesh = B3DMALLOC(Imesh, totalPoints);
  if (!obj->mesh)
    return -1;
  for (thread = 0; thread < numThreads; thread++) {
    memcpy(&obj->mesh[obj->meshsize], tempMeshes[thread], 
           tempMeshSize[thread] * sizeof(Imesh));
    obj->meshsize += tempMeshSize[thread];
  }
  ...
  if (flags & IMESH_MK_STRAY) {
    for (indZ = 0; indZ < zMax + 1 - zMin; indZ++) {
      ...
      if (flags & IMESH_MK_SKIP)
        nextZ = getnextz(zList, zListSize, indZ + zMin);
      else
        nextZ = indZ + zMin + zIncrement;
      if (nextZ > zList[zListSize - 1])
        continue;
      ...
      do {
        topCont = NULL;
        for (jndAtZ = 0; jndAtZ < numContAtZ[indZ]; jndAtZ++) {
          coNum = contsAtZ[indZ][jndAtZ];
          cont = &(obj->cont[coNum]);
          if (connectTop(cont->flags))
            continue;

          botCnctNum = istoreConnectNumber(obj->store, coNum);
          robustCenterOfMass(cont, scanConts[coNum], &botCenMass);
          minDist = 1.e30;

          for (indAtZ = 0; indAtZ < numContAtZ[nextZ - zMin]; indAtZ++) {
            otherCoNum = contsAtZ[nextZ - zMin][indAtZ];
            joinCont = &(obj->cont[otherCoNum]);
            if (joinCont->flags & CONNECT_BOTTOM)
              continue;
            if ((flags & IMESH_MK_SURF) && cont->surf != joinCont->surf)
              continue;
            if ((flags & IMESH_MK_TIME) && cont->time != joinCont->time)
              continue;
            if (cont->psize == 1 && joinCont->psize == 1)
              continue;

            /* Do not connect if they have non-matching connection numbers */
            topCnctNum = istoreConnectNumber(obj->store, otherCoNum);
            if (topCnctNum >= 0 && botCnctNum >= 0 && topCnctNum != botCnctNum)
              continue;

            robustCenterOfMass(joinCont, NULL, &topCenMass);
            dist = (topCenMass.x - botCenMass.x) * (topCenMass.x - botCenMass.x) +
              (topCenMass.y - botCenMass.y) * (topCenMass.y - botCenMass.y);
        ...
        if (topCont) {
          ...
          if (obj->flags & IMOD_OBJFLAG_OUT)
            inTemp = 1 - inTemp;
          newMesh = imeshContoursCost(obj, botCont, topCont, scale, inTemp, botCoNum, 
                                      topCoNum);
          if (newMesh) {
            obj->mesh = imodel_mesh_add(newMesh, obj->mesh, &(obj->meshsize));
            free(newMesh);
            if (!obj->mesh)
              return -1;
          }
          botCont->flags |= CONNECT_TOP;
          topCont->flags |= CONNECT_BOTTOM;
        }
  ...
  status = (*callback)(50); /* half way done. */
  status = (*callback)(1);
  if (status) return(status);
  ...
  if (doCap) {
    ...
    for (coNum = 0; coNum < obj->contsize; coNum++) {
      ...
      if ((cont->psize > 2) && (!connectBoth(cont->flags))) {
        ...
        if (contNestInd[coNum] >= 0)
          if (nests[contNestInd[coNum]].level % 2 == 0)
            inside = 1;
        ...
        if (doCap == IMESH_CAP_ALL) {

          if (connectBottom(cont->flags))
            direction = 1;
          if (connectTop(cont->flags))
            direction = -1;
          newMesh = imeshContourCap(obj, cont, scanConts[coNum], coNum, direction, inside,
                                    scale);
          addMeshToObject(obj, cont, newMesh, flags);
          continue;
        }

        if (cont->pts->z == zMin) {
          newMesh = imeshContourCap(obj, cont, scanConts[coNum], coNum, -1, inside,
                                    scale);
          addMeshToObject(obj, cont, newMesh, flags);
        }
        if (cont->pts->z == zMax) {
          newMesh = imeshContourCap(obj, cont, scanConts[coNum], coNum, 1, inside, scale);
          addMeshToObject(obj, cont, newMesh, flags);
        }
      }
    }
  ...
  for (coNum = 0; coNum < obj->contsize; coNum++) {
    if (scanConts[coNum])
      imodContourDelete(scanConts[coNum]);
  }
  free(scanConts);

  skin_report_time("Capped");
  for (meshInd = 0; meshInd < obj->meshsize; meshInd++) {
    if (!(flags & IMESH_MK_SURF))
      obj->mesh[meshInd].surf = 0;
    if (!(flags & IMESH_MK_TIME))
      obj->mesh[meshInd].time = 0;
  }
  ...
  imodContourFreeZTables(numContAtZ, contsAtZ, contourZ, zList, zMin, zMax);
...
static Icont *connect_orphans(Iobj *obj, Imesh **objMesh, int *meshsize, Icont *cout,
                              int *list, int *used, int *justinl, int njustin,
                              int *inlist, int joindir, int oddeven, int surf, int time,
                              Ipoint *scale, Icont **scancont, Ipoint *pmin, Ipoint *pmax)
{
  ...
  for (inl = 0; inl < njustin; inl++) {
    ...
    if (minratio < 1.e20) {
      ...
      if (mesh_contours(obj, objMesh, meshsize, c1, cinmin, surf, time, scale, oddeven,
                        inlist[0], inlist[0]))
        return NULL;
...
static int robustCenterOfMass(Icont *cont, Icont *scanCont, Ipoint *cm)
{
  Ipoint  ll, ur;
  if (imodContourCenterOfMass(scanCont ? scanCont : cont, cm)) {
 ...
 *
 */
static Imesh *imeshContourCap(Iobj *obj, Icont *cont, Icont *scanCont, int coNum,
                              int side, int inside, Ipoint *scale)
{
  int     pt;
  int     meshdir = 0;
  ...
  failedcm = robustCenterOfMass(cont, scanCont, &cm);
                         ...
                         int bco, int tco)
{
  ...
  if (nmesh) {
    nmesh->surf = surf;
    nmesh->time = time;
    *objMesh = imodel_mesh_add(nmesh, *objMesh, meshsize);
    free(nmesh);
    if (!(*objMesh))
      return -1;
  }
