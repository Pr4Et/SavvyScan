...
// On 10.12, it says to include these files but it STILL doesn't see the declaration
#ifdef Q_OS_MACX
#include <sys/types.h>
#include <sys/uio.h>
extern "C" {
 ...
 */
static const int numOptions = 22;
static const char *options[] = {
  ":r:B:", ":s:B:", ":m:B:", ":G:B:", ":O:I:", ":g:B:", ":n:I:", ":w:FN:", ":d:I:",
  ":e:I:", ":C:FT:", ":T:IP:", ":c:FN:", ":q:I:", ":Q:CH:", ":I:CH:", ":D:CH:",
  ":W:I:", ":P:B:", ":v:B:", ":V:CH:", ":help:B:"};
...
void Processchunks::loadParams(int &argc, char **argv) {
  ...
  PipGetBoolean("m", &mMultipleFiles);
  if (mSingleFile && mMultipleFiles)
    exitError("You cannot enter both -s and -m");
  if (mGpuMode || mSingleFile || mMultipleFiles)
    mNumThreads = 0;
  if (mMultipleFiles) {
    mSlowMachineCrit = 0.;
    mMaxChunkErr = 2;
    mMultipleFiles = numNonOptArgs - 1;
  } else if (numNonOptArgs > 2)
    exitError("More than two non-option arguments were entered; use the -m option to "
  ...
  mEnteredMaxChunkErr = 1 - PipGetInteger("e", &mMaxChunkErr);
  ...
  PipGetString("I", &mInitQueue);
  PipGetString("D", &mDeinitQueue);
  int waitSec = mWaitForQueueInit / 1000;
  if (PipGetInteger("W", &waitSec) != 0) {
    char *waitStr = getenv("IMOD_QUEUE_INIT_WAIT");
    if (waitStr)
      waitSec = atoi(waitStr);
  }
  if (waitSec < -1 || waitSec == 0 || waitSec > 2000000)
    exitError("Wait time for queue initialization command must be -1 or a positive "
              "value up to 2000000");
  if (waitSec < 0)
      mWaitForQueueInit = -1;
  else
    mWaitForQueueInit = 1000 * waitSec;
  int returnPid = 0;
...
void Processchunks::setup() {
  ...
  if (machineNameList.size() == 1 && !mEnteredMaxChunkErr)
    mMaxChunkErr = 2;
  setupHostRoot();
  setupComFileJobs();
...
void Processchunks::timerEvent(QTimerEvent */*timerEvent*/) {
  ...
  if ((mSingleFile && mNumDone > 0) || mNumDone + mNumSkipped >= mSizeJobArray) {
    cleanupAndExit();
    return;
  }
}
...
void Processchunks::cleanupAndExit(int exitCode) {
  ...
  i = runGenericQueueCommand(mDeinitQueue, 30000);
  if (i)
    *mOutStream << "WARNING: Command to de-initialize queue returned with status " << i
                << endl;
...
void Processchunks::initMachineList(QStringList &machineNameList, 
  ...
  for (i = 0; i < cpuArray.size(); i++) {
    ...
    if (!machineNameList.contains(machineName, Qt::CaseInsensitive)) {
      ...
      for (j = 0; j < machineNameList.size(); j++) {
        if (QString::compare(machineName, machineNameList[j], Qt::CaseInsensitive) == 0) {
...
void Processchunks::setupMachineList(QStringList &machineNameList,
  ...
  if (mQueue) {
    ...
    mEscapedRemoteDirPath = *mRemoteDir;
    mEscapedRemoteDirPath.replace(QRegExp(" "), "\\ ");
    int status = runGenericQueueCommand(mInitQueue, mWaitForQueueInit);
    if (status)
        exitError("Command to initialize queue returned with error %d", status);
  }
  else {
    mMachineListSize = 0;
...
// Run a generic entered command on the queue with -C:<command>, escaping spaces
int Processchunks::runGenericQueueCommand(const char *command, int waitMsec) {
  if (!command || !mQueue)
    return 0;
  QString queueCom = command;
  //queueCom.replace(QRegExp(" "), "\\ ");
  queueCom = "C:" + queueCom;
  QStringList params = mQueueParamList;
  params  << "-w" << mEscapedRemoteDirPath << "-a" << queueCom;
  QByteArray output;
  QProcess init(this);
  if (mVerbose) {
    *mOutStream << "queue command: " << mQueueCommand << endl << "params:" << endl;
    for (int i = 0; i < params.size(); i++) {
      *mOutStream << i << ": " << params.at(i) << endl;
    }
    *mOutStream << endl;
  }
  return runGenericProcess(output, init, mQueueCommand, params, mVerbose ? 100 : 0, 
                           waitMsec);
}
...
void Processchunks::setupComFileJobs() {
  ...
  const char *regFilters[4] = {"-???", "-????", "-?????", "-??????"};
  //?- is a special character and gets a warning about -trigraphs
  const char *syncFilters[4] = {"-??\?-sync",  "-???\?-sync",  "-????\?-sync",  
                                "-?????\?-sync"};
  char cleanBuf[32];
  if (isVerbose(mDecoratedClassName, __func__)) {
  ...
  if (mSingleFile || mMultipleFiles) {
    for (i = 0; i < B3DMAX(1, mMultipleFiles); i++) {
      PipGetNonOptionArg(i + 1, &argName);
      QString rootName(argName);
      free(argName);

      // If it already ends in .com or .pcm, check that it exists
      if (rootName.endsWith(".com") || rootName.endsWith(".pcm")) {
        if (!mCurrentDir.exists(rootName)) {
          exitError("The %s command file %s does not exist",
                    mSingleFile ? "single" : "", rootName.toLatin1().data());
        }
      }
      else {
        ...
        altName = rootName;
        rootName.append(mComExtension);
        altName.append(mComExtension == ".com" ? ".pcm" : ".com");
        if (!i && mCurrentDir.exists(rootName) && mCurrentDir.exists(altName))
          exitError("You must enter the full command file name with extension because"
                    " both %s and %s exist", rootName.toLatin1().data(),
                      altName.toLatin1().data());
          
        if (!mCurrentDir.exists(rootName)) {
          if (!mCurrentDir.exists(altName)) {
            exitError("The %s command file %s or %s does not exist",
                      mSingleFile ? "single" : "", rootName.toLatin1().data(),
                      altName.toLatin1().data());
          }
          rootName = altName;
      ...
      if (!i) {
        mComExtension = rootName.right(4);
      } else if (rootName.right(4) != mComExtension) {
        exitError("All command files must have the same extension; %s does not match "
                  "the preceding ones", rootName.toLatin1().data());
      }
    ...
    for (int extLoop = 0; extLoop < 2; extLoop++) {
      numNumericCommandFiles = 0;

      // Compose start and finish names for general use
      QString startComFile(mRootName);
      startComFile.append("-start");
      startComFile.append(mComExtension);
      QString finishComFile(mRootName);
      finishComFile.append("-finish");
      finishComFile.append(mComExtension);
      ...
      for (int digits = 3; digits <= 6; digits++) {
        buildFilters(regFilters[digits - 3], syncFilters[digits - 3], filters);
        dit = new QDirIterator(mCurrentDir.path(), filters);
        while (dit->hasNext()) {
          dit->next();
          if (dit->fileName() != startComFile && dit->fileName() != finishComFile) {
            list << dit->fileName();
          }
        }
        delete dit;
        ...
        list.sort();
        sprintf(cleanBuf, "-\\D{%d,%d}(-sync){0,1}\\", digits, digits);
        cleanupList(cleanBuf, list);
      ...
      if (extLoop) {
        exitError("There are no command files matching %s-nnn.com or %s-nnn.pcm",
                  mRootName, mRootName);
      } else {     
        mComExtension = mComExtension == ".com" ? ".pcm" : ".com";
        comFileArray.clear();
      }
    }
  }
  ...
  if (mCurrentDir.exists(mRootName + QString("-1000000") + mComExtension) ||
      mCurrentDir.exists(mRootName + QString("-1000000-sync") + mComExtension)) {
    exitError("Cannot process more than 999999 chunks");
  }
  if (isVerbose(mDecoratedClassName, __func__)) {
  ...
  mComFileJobs = new ComFileJobs(comFileArray, mSingleFile, mComExtension);
  comFileArray.clear();
  for (i = 0; i < mSizeJobArray; i++) {
...
bool Processchunks::handleChunkDone(MachineHandler &machine, ProcessHandler *process,
  ...
  mNumDone++;
  sprintf(elapseBuf, " in %.2f sec", process->getElapsedTime() / 1000.);
...
bool Processchunks::handleError(const QString *errorMess, MachineHandler &machine,
  ...
  if (numErr >= mMaxChunkErr && mMultipleFiles) {
    process->setFlag(CHUNK_TO_SKIP);
    mNumSkipped++;
  }
...
void Processchunks::handleDropOut(bool &noChunks, QString &dropMess,
  ...
  if (needKill) {
    ...
    *mOutStream << process->getComFileName() << " failed on " << machine.getName()
                << (process->getFlag() == CHUNK_TO_SKIP ? " - giving up and skipping it" :
...
bool Processchunks::checkChunk(int &runFlag, bool &noChunks, int &undoneIndex,
  ...
  if (undoneIndex == -1 && runFlag != CHUNK_DONE && runFlag != CHUNK_TO_SKIP) {
...
void Processchunks::runProcess(MachineHandler &machine, ProcessHandler *process,
  ...
  sprintf(timeBuf, " at %02d:%02d:%02d ...", tms->tm_hour, tms->tm_min, tms->tm_sec);
...
void Processchunks::buildFilters(const char *reg, const char *sync, QStringList &filters) {
  ...
  filter1.append(reg);
  filter1.append(mComExtension);
  filters.append(filter1);
  QString filter2(mRootName);
  filter2.append(sync);
  filter2.append(mComExtension);
...
int Processchunks::runGenericProcess(QByteArray &output, QProcess &process,
                                     ...
                                     const int numLinesToPrint, int waitMsec) {
  int i;
  process.start(command, params);
  if (process.waitForFinished(waitMsec)) {
