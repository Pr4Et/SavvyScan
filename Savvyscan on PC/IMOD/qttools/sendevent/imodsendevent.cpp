...
static int sWinID;
static int sDebugOut = 0;
static int sRetryLimit = 0;
static int sRetryCount = 0;
static QString sTimeStr;
static QString sCmdStr;
static QString sLastCBtext;
...
int main(int argc, char **argv)
{
  ...
  for (argIndex = 1; argIndex < argc - 1 ; argIndex++) {
    if (argv[argIndex][0] == '-'){
      ...
      case 'h':
        printf("   Usage: imodsendevent [-t timeout] [-D] Window_ID action "
            "[arguments]\n");
        exit(0);
  ...
  sWinID = strtol(argv[argIndex], &endptr, 10);
  if (endptr - argv[argIndex] < (int)strlen(argv[argIndex])) {
  ...
  sTimeStr.sprintf(" %d ", timeStamp);
  sTimeStr = QString(argv[argIndex]) + sTimeStr; 
  sCmdStr = QString(argv[argIndex + 1]);
  for (; argIndex + 2 < argc; argIndex++)
    sCmdStr += QString(" ") + QString(argv[argIndex + 2]);
  qstr = sTimeStr + sCmdStr;
  ...
  if (interval > 0) {
    ...
    if (SENDEVENT_RETRY_HACK > 0) {
      sRetryLimit = interval / SENDEVENT_RETRY_HACK;
      if (!sRetryLimit)
        sRetryLimit = 1;
...
{
  if (++sRetryCount < sRetryLimit) {
    sTimeStr += " ";
    QString qstr = sTimeStr + sCmdStr;
    QClipboard *cb = QApplication::clipboard();
    //cb->setSelectionMode(false);

    // Check the clipboard and see if it changed
...
void ImodSendEvent::clipboardChanged()
{
  ...
  if (index <= 0)
    return;
  if ((text.left(index)).toUInt() != sWinID) 
    return;
