...
class ComFileJobs {
public:
  ComFileJobs(const QStringList comFileArray, const bool singleFile, const QString 
              comExtension);
  ~ComFileJobs();
  ...
  ;
  //Add the com extension to the root name
  inline const QString getComFileName(const int index) {
    return QString("%1%2").arg(QString(mJobArray[index].root), mComExtension);
  }
  ;
