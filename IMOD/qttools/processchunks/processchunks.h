...
private:
  ...
  int runGenericQueueCommand(const char *command, int waitMsec);
  bool readCheckFile();
  ...
  int mRetain, mJustGo, mNice, mMillisecSleep, mDropCrit, mQueue, mSingleFile,
    mMaxChunkErr, mVerbose, mGpuMode, mNumThreads, mMultipleFiles, mEnteredMaxChunkErr;
  bool mSkipProbe;
  char *mQueueName, *mRootName, *mInitQueue, *mDeinitQueue;
  int mWaitForQueueInit;
  QFile *mCheckFile;
  ...
  QString mHostRoot, mQueueCommand, mDecoratedClassName, mEscapedRemoteDirPath;
  QString mComExtension;
  QStringList mSshOpts, mQueueParamList;
  QDir mCurrentDir;

  //loop
  int mNumDone, mLastNumDone, mHoldCrit, mTimerId, mFirstUndoneIndex, mNextSyncIndex,
    mSyncing, mCheckFileReconnect, mSlowestTime, mSlowTimeCount, mNumSkipped;
