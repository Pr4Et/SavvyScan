...
static void usage() 
{
   printf("Usage: imodqtassist [options] path_to_help_collection_file\n");
   printf("   Options:\n");
   printf("\t-a\tpath_to_help_collection is absolute, not relative to"
          " $IMOD_DIR\n");
   printf("\t-p file\tName of qhc (help collection) file (IMOD.adp \n "
          "\t\t   can be entered instead of IMOD.qhc)\n");
   printf("\t-q pref\tPrefix to use in front of help page name\n");
   printf("\t-b\tPrefix is absolute, not appended to qthelp://bl3demc/IMOD/\n");
   printf("\t-k\tKeep the sidebar (do not hide it)\n");
   printf("\t-t\tReport ideal thread count (number of processors) and exit\n");
   printf("\t-d\tReport resolution of desktop in dots/inch (DPI) and exit\n");
   printf("\t-db\tJust output X resolution of desktop in dots/inch (DPI) and exit\n");
}
...
int main(int argc, char *argv[])
{
  ...
  if (argc == 2 && !strcmp(argv[ind], "-t")) {
    numCoresAndLogicalProcs(&retval, &ind);
    printf("Qt ideal thread count = %d   physical cores = %d   logical processors = %d\n",
           QThread::idealThreadCount(), retval, ind);
    exit(0);
  }
  ...
  if (argc == 2 && strstr(argv[ind], "-d") == argv[ind]) {
    if (!strcmp(argv[ind], "-d"))
