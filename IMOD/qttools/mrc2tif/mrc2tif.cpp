...
static void usage(char *progname)
{
  ...
  printf("    -j         Output jpeg file instead of tiff\n");
  printf("    -p         Output png file instead of tiff\n");
  ...
  printf("    -O #       Override default for parallel tiff compression on whole"
         " image\n");
...
int main(int argc, char *argv[])
{
  ...
  float chunkCriterion = 400.;
  float savecrit, dmin, dmax, smin =0., smax = 0.;
  int xTileSize = 0;
  int doParallel = -1, didParallel = 0;
  float val[3];
  ...
  /* 1 is none, 32773 is PACKBITS, 32946 is DEFLATE (non-Adobe) */
  int legalComp[] = {1, IICOMPRESSION_LZW, IICOMPRESSION_JPEG, IICOMPRESSION_ZIP, 32773,
                     32946};
  const char *extensions[3] = {"tif", "jpg", "png"};
  const char *typeNames[3] = {"TIFF", "JPEG", "PNG"};
  int numLegal = sizeof(legalComp) / sizeof(int);
  ...
  if (!makeQimage) {
    ...
    if (version > 0 && !oldcode && (doParallel <= 0 || compression == 1) &&
  ...
  for (z = zmin; z <= zmax; z++){
    ...
    for (chunk = 0; chunk < numChunks; chunk++) {
      ...
      else if (!doChunks && doParallel) 
        tiferr = tiffParallelWrite(iifile, buf, compression, 0, resolution,
                                   quality, &didParallel);
      else if (!doChunks) 
      ...
      if (tiferr) {
        exitError("Error (%d) writing section %d to %s%s", tiferr, z, iname, 
                  didParallel ? "; you could use option -O 0 to try again without "
