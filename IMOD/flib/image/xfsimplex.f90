...
Program xfsimplex
  ...
  integer*4 ifXminMax, ifYminMax, iAntiFiltType, minChunkLines
...
13  write(*,'(1x,a,i1,a,$)') '0 to search 6 formal variables,'// &
  ...
  if (pipInput) then
    ...
    if (iAntiFiltType > 0) then 
      minChunkLines = 10
      if (ibinning > 32) minChunkLines = 3
      lenTemp = max(nx * ((minChunkLines + 6) * ibinning + 20),  &
          int(min(10000000, int(nx, kind = 8) * ny)))
      ifFiltAfter = 1
    endif
