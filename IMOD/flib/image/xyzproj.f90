...
program xyzproj
  ...
  logical*4 commonLine, fullImage, haveAngles
  real*4 sind, cosd
  ...
  !
  ! fallbacks from ../../manpages/autodoc2man -3 2  xyzproj
  !
  integer numOptions
  parameter (numOptions = 20)
  character*(40 * numOptions) options(1)
  options(1) = &
      'input:InputFile:FN:@output:OutputFile:FN:@axis:AxisToTiltAround:CH:@'// &
      'xminmax:XMinAndMax:IP:@yminmax:YMinAndMax:IP:@zminmax:ZMinAndMax:IP:@'// &
      'angles:StartEndIncAngle:FT:@tiltfile:TiltFile:FN:@tangles:TiltAngles:FAM:@'// &
      'mode:ModeToOutput:I:@width:WidthToOutput:I:@addmult:AddThenMultiply:FP:@'// &
      'fill:FillValue:F:@constant:ConstantScaling:B:@series:InputIsTiltSeries:B:@'// &
  ...
  !
  if (pipInput) then
    ierr = PipGetLogical('InputIsTiltSeries', commonLine)
    ...
    if (iyLoad0 > 0 .and. .not.commonLine) call exitError &
    ...
    if (ixLoad0 + ixLoad1 + iyLoad0 > 0) then
      nview = 0
      if (commonLine) nview = nzIn
      call get_tilt_angles(nview, 3, tiltAngles, LIMPROJ, 1)
      if (commonLine .and. nview .ne. nzIn) call exitError( &
          'There must be a tilt angle for each view')
      if (commonLine) xyz = 'Z'
      if (.not.commonLine) nzOut = nview
      haveAngles = .true.
    endif
    ...
    if (haveAngles .and. ierr == 0 .and. .not.commonLine) call exitError &
        ('You can enter only one of StartEndIncAngle, TiltAngles, and TiltFile')
  else
    write(*,'(1x,a,$)') 'Axis to tilt around (enter X, Y or Z): '
    read(*,'(a)') xyz
    !
    write(*,'(1x,a,$)') 'Starting, ending, increment tilt angles: '
    read(*,*) tiltStart, tiltEnd, tiltInc
  endif
  if (commonLine .or. .not. haveAngles) then
...
301 format('XYZPROJ: x',2i5,', y',2i5,', z ',2i5,' about ',a1,t57,a9,2X,a8)
  ...
  if (nxout * nzOut > LIMRAY) call exitError('Too many projections for output this wide')
  ...
  do iproj = 0, nzOut - 1
    angle = invertAng * (tiltStart + iproj * tiltInc)
    if (.not. commonLine .and. haveAngles) angle = invertAng * tiltAngles(iproj + 1)
