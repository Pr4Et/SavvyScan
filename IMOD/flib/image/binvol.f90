...
program binvol
  ...
  integer*4 iredBinIntoSlice(3), itempBase, nxEquiv, ifVerbose, nxBinIntoSlice
  ...
  integer*4 lastZfinished, iyBinned, ioutBase, lastZadded, numZafter, ifXYantiAlias
  logical*4 fullOutSlices, spreadZ
  data nxyzst/0, 0, 0/
  !
  ...
  options(1) = &
      ...
      'spread:SpreadSlicesInZ:B:@memory:MemoryLimit:I:@verbose:VerboseOutput:I:@'// &
      'help:usage:B:'
  !
  iredAll = 2
  limit = 1000
  ...
  ierr = PipGetInteger('VerboseOutput', ifVerbose)
  ...
  enddo
  nxBinIntoSlice = nx
  iredBinIntoSlice(:) = iredFac(:)
  if (ifiltType > 1 .and. ibinX == ibinY .and. ibinX > 1) then
    print *,'Antialiasing is being applied in X and Y as well as Z'
    ifXYantiAlias = 1
    iredBinIntoSlice(1:2) = 1
    nxBinIntoSlice = nxBin
  endif
  !
  ...
  if (ifiltType > 0) then
    ...
    if (spreadZ .and. numExtra > 0) then
      ...
      call iiuAltOrigin(3, xOrigin, yOrigin, zOrigin)
    endif
    !
  ...
  fullOutSlices = .false.
  nxEquiv = nx + ifXYantiAlias * ((nxBin + ibinY - 1) / ibinY)
  if (float(nxBin) * nybin * numSliceUsedIn < limDim) then
    inBase = nxBin * nyBin * numSliceUsedIn
    maxLines = ibinY * (((limDim - inBase) / nxEquiv) / ibinY)
  ...
  if (.not. fullOutSlices) then
    ...
    maxLines = ibinY * ((limDim / (nxEquiv + (nxBin + ibinY - 1) / ibinY)) / ibinY - 1)
  ...
  itempBase = (nxEquiv - nx) * maxLines + inBase
  allocate(array(nxEquiv * maxLines + inBase), stat = ierr)
  call memoryError(ierr, 'large array for data')

  call iiuAltSize(3, nxyzBin, nxyzst)
  call iiuAltSample(3, nxyzBin)
  ...
  dmin = 1.e30
  pixSum = 0.
  if (ifVerbose > 0) print *,'fullOutSlices', fullOutSlices, ' num before, after', &
      numZbefore, numZafter, ' num used in, cen off, half', numSliceUsedIn, izCenOffset, &
      halfOffset

  numChunks = (ny + maxLines - 1) / maxLines
  if (ifVerbose > 0) print *,numChunks, ' chunks, maximum lines', maxLines
  if (fullOutSlices) then
    ...
    do inz = 0, nz - 1
      ...
      do iChunk = 1, numChunks
        ...
        if (ifXYantiAlias > 0) then
          call irdReduced(1, inz, array(inBase + 1), nxBin, 0., float(iy), float(ibinX), &
              nxBin, numLines / ibinY, ifiltType - 1, array(itempBase + 1), nx * &
              maxLines, ierr)
          if (ierr .ne. 0) go to 99
        else
          call iiuSetPosition(1, inz, iy)
        ...
        do izOut = lastZfinished + 1, nzBin - 1
          ...
          if (inz >= inputStarts(izOut)) then
            ...
            if (iringPos(izOut) == 0) then
              ...
              if (ifVerbose > 1) print *,'Assigning ', izOUt, ' to slot ', iringEnd
            endif
            !
            ...
            if (ifVerbose > 1) print *,'Adding', inz, ' into', izOut, ', weight', &
                zweight, ioutBase
            call binIntoSlice(array(inBase + 1), nxBinIntoSlice, array(ioutBase), nxBin, &
                numBinLines, iredBinIntoSlice(1), iredBinIntoSlice(2), zWeight)
          endif
          lastZadded = izOut
        enddo
      ...
      do izOut = lastZfinished + 1, lastZadded
        if (inz == inputEnds(izOut)) then
          ...
          call iclden(array(ioutBase), nxBin, nyBin, 1, nxBin, 1, nyBin, dmin2, dmax2, &
            dmean2)
          call iiuWriteSection(3, array(ioutBase))
          if (ifVerbose > 1) print *,'Wrote ', izOut, ioutBase
          !
          dmax = max(dmax, dmax2)
          dmin = min(dmin, dmin2)
    ...
    do izOut = 0, nzBin - 1
      ...
      do iChunk = 1, numChunks
        ...
        do inz = max(0, izCen - numZbefore), min(izCen + numZafter, nz - 1)
          if (ifXYantiAlias > 0) then
            call irdReduced(1, inz, array(inBase + 1), nxBin, 0., float(iy), &
                float(ibinX), nxBin, numLines / ibinY, ifiltType - 1, &
                array(itempBase + 1), nx * maxLines, ierr)
            if (ierr .ne. 0) go to 99
          else
            call iiuSetPosition(1, inz, iy)
            call irdsecl(1, array(inBase + 1), numLines, *99)
          ...
          call sliceWeighting(inz, zWeight)
          call binIntoSlice(array(inBase + 1), nxBinIntoSlice, array, nxBin, &
              numBinLines, iredBinIntoSlice(1), iredBinIntoSlice(2), zWeight)
        enddo
        iy = iy + numLines
        ...
        call iclden(array, nxBin, numBinLines, 1, nxBin, 1, numBinLines, dmin2, dmax2, &
            dmean2)
        call iiuWriteLines(3, array, numBinLines)
        !
        dmax = max(dmax, dmax2)
        dmin = min(dmin, dmin2)
...
1500 format('BINVOL: Volume binned down by factors', 3i4, t57,a9,2X,a8)
  ...
  call iiuWriteHeaderStr(3, titlech, 1, dmin, dmax, dmean)
  call iiuClose(3)
  call iiuClose(1)
  !
  write(6, 500)
