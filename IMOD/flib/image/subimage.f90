...
program subimage
  ...
  equivalence (nxyz, nx)
  common//nx, ny, nz
  character*320 afile, bfile, cfile, textFile
  ...
  integer*4 modeOut, ifZeroMean, ifLowThresh, ifHighThresh, maxUnitOut
  ...
  options(1) = &
      ...
      'minmax:ErrorMinMaxLimit:F:@sdlimit:ErrorSDLimit:F:@param:ParameterFile:PF:@'// &
      'help:usage:B:'
  ...
  if (pipInput) then
    ...
    if (PipGetString('DifferenceTextFile', textFile) == 0) then
      call dopen(7, textFile, 'new', 'f')
      maxUnitOut = 7
    endif
  endif
  ...
  realMax = -realMin
  do i = 6, maxUnitOut
    write(i, '(a)')' Section      Min            Max            Mean           S.D.'
  enddo
  maxLines = limarr / nx
  ...
  do isec = 1, numAsec
    ...
    call sums_to_avgsd8(sum, sumSq, maxXstat + 1 - minXstat, maxYstat + 1 - minYstat, &
        tmean, sd)
    do i = 6, maxUnitOut
      write(i, 4000) asec, tmin, tmax, tmean, sd
    enddo
    sdSum = sdSum + sum
  ...
  if (numAsec > 1) then
    do i = 6, maxUnitOut
      write(i, 5000) dmin, dmax, dmean, sd
    enddo
  endif
