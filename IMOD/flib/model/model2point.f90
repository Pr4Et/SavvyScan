...
program model2point
  ...
  integer*4 nobjTot, imodObj, getImodObjSize, getContValue, getPointValue, imodGetenv
  !
  include 'model.inc90'
  ...
  if (imodGetenv('MODEL2POINT_INTEGERS', pointFile) == 0) floating = .false.
  !
  ...
  if (PipGetLogical('IntegerOutput', exist) == 0) then
    if (ierr == 0) call exitError('You cannot enter both -float and -integers')
    floating = .not. exist
  elseif (ierr == 0 .and. exist .and. floating) then
    print *,'The -float entry is no longer needed'
  endif
