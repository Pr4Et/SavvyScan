...
program tiltxcorr
  ...
  real*4 xpeakList(LIMPEAKS), ypeakList(LIMPEAKS), peakList(LIMPEAKS)
  integer*4 indPeakSort(LIMPEAKS), numXcorrPeaks
  ...
  integer*4 ixStart, ixEnd, iyStart, iyEnd, ivCur, maxUserBinning, iz, ivSkip
  ...
  real*4 angleOffset, cumXcenter, cumYcenter, xModOffset, yModOffset, xrot, yrot
  ...
  integer*4 numControl, numBoundAll, idim2, iunitRef, minChunkLines, ivFirstNotSkipped
  integer*4 nxUnali, nyUnali, numAllViews, ivPairOffset, ifFindWarp, limitedBinSize
  integer*4 nxUBpad, nyUBpad, nxUBtaper, magView, maxAutoBinning
  ...
  integer*4 numRotSteps, indBestRot, izInRefFile, nxSecPeakBox, nySecPeakBox
  ...
  real*4 secXpeak, secYpeak, unstretchSecDx, unstretchSecDy, secXshift, secYshift
  ...
  logical*4 searchedForMag, searchMag, reusePrevArrays, useRefFile, secPeakInBox
  logical*4 inputPass0, inputPass90, inputPassMin90, invPass0, invPass90, invPassMin90
...
101 format(a)
  ...
  if (pipinput) then
    ...
    ierr = PipGetTwoIntegers('SecondPeakBoxSize', nxSecPeakBox, nySecPeakBox)
    ...
    if (PipGetInteger('BinningToApply', nbinning) == 0) then
      if (nbinning <= 0 .or. nbinning > maxUserBinning) call exitError &
    ...
    lenTemp = max(lenTemp, maxUserBinning * nx)
    if (iAntiFiltType > 0) then
      lenTemp = max(lenTemp, (16 * maxUserBinning + 20) * nx)
      binRedText = ' Reduction is'
    endif
    lenTemp = min(int(lenTemp, kind = 8), int(nx, kind = 8) * ny)
...
1100 format(' Sigma1, Sigma2, Radius1, Radius2: ',$)
        ...
        if (iv >= 1 .and. iv <= numAllViews) then
          ...
          do ipt = 1, npt_in_obj(iobj)
            ...
            if (ifFindWarp == 0) then
              call adjustCoord(tilt(iv), 0., x0 - nx / 2., y0 - ny / 2., x0, y0)
              x0 = x0 + nx / 2.
              y0 = y0 + ny / 2.
            endif
            xbound(indBound(ix) + ipt) = x0
            ybound(indBound(ix) + ipt) = y0
            boundXmin = min(boundXmin, x0)
            boundXmax = max(boundXmax, x0)
            boundYmin = min(boundYmin, y0)
            boundYmax = max(boundYmax, y0)
          enddo
        endif
      enddo
  ...
  write(*,'(/,a,i3,a,i6,a,i6)') binRedText, nbinning, &
  ...
  enddo
  if (ifNoStretch == 0 .and. useMax > cosStrMaxTilt) then
    if (pipInput) then
      if (PipGetFloat('FirstTiltAngle', xpeak) == 0 .and. &
          PipGetFloat('TiltIncrement', ypeak) == 0) then
        !
        ! Check if they screwed up the increment and give a better message
        call anglesPassThrough(xpeak, ypeak, nz, inputPass0, inputPass90, inputPassMin90)
        call anglesPassThrough(xpeak, -ypeak, nz, invPass0, invPass90, invPassMin90)
        if (invPass0 .and. .not.invPass90 .and. .not.invPassMin90 .and. &
            ...
            call exitError('Tilt angles pass though 90 or -90 degrees (too high for '// &
            'cosine stretching) and not through 0; does the starting angle or '// &
            'increment have the wrong sign?')
      endif
    endif
    call exitError('Maximum tilt angle is too high to use cosine stretching')
  endif
  !
  do kk = 1, nz
  ...
  if (tracking) then
    ...
    allocate(xmodel(numPatchesAll, izStart:izEnd), ymodel(numPatchesAll, izStart:izEnd), &
        stat = ierr)
  ...
  do iloop = 1, numLoops
    ...
    do iview = ivStart, ivEnd, loopDir
      ...
      if (ivFirstNotSkipped < 0) ivFirstNotSkipped = iview
      !
      ...
      do ipatch = 1, numPatches
        ...
        baseTilt = 0.
        if (iview == ivFirstNotSkipped) then
          ixBoxStart = ixBoxRef
          iyBoxStart = iyBoxRef
        endif
        !
        ...
        if (tracking) then
          if ((iloop == 1 .and. iview == ivFirstNotSkipped) .or. ifFindWarp .ne. 0) then
        ...
        if (ifScanRotation > 0) then
          if (numRotSteps > 0) then
            ...
            do iter = 1, numRotSteps
              ...
              if (nbinning > 1 .and. ifExclude > 0) then
        ...
        secYpeak = 0.
        secPeakInBox = .false.
        if (nySecPeakBox > 0 .and. numXcorrPeaks > 1) then
          xpeakTmp = xpeakList(indPeakSort(2)) - xpeak
          ypeakTmp = ypeakList(indPeakSort(2)) - ypeak
          xrot = xpeakTmp * cosRotAngle - ypeakTmp * sinRotAngle
          yrot = xpeakTmp * sinRotAngle + ypeakTmp * cosRotAngle
          if (xrot * nbinning <= nxSecPeakBox .and. yrot * nbinning <= nySecPeakBox) then
            secXpeak = xpeakList(indPeakSort(2))
            secYpeak = ypeakList(indPeakSort(2))
            secPeakInBox = .true.
          endif
        endif
        !
        ...
        if (idir > 0) then
          ...
          call xfapply(fsInv, 0., 0., secXpeak, secYpeak, unstretchSecDx, unstretchSecDy)
        else
          unstretchDx = xpeak
          unstretchDy = ypeak
          unstretchSecDx = secXpeak
          unstretchSecDy = secYpeak
        endif
        !
        ...
        if (searchedForMag) then
          ...
          if (magView == ivRef) then
            ...
            unstretchSecDx = unstretchSecDx * foundMag
            unstretchSecDy = unstretchSecDy * foundMag
          endif
        ...
        secXshift = idir * nbinning * unstretchSecDx + ixBoxForAdj - ixBoxCur
        secYshift = idir * nbinning * unstretchSecDy + iyBoxForAdj - iyBoxCur
        !
        ! It is time to print these peaks if dual output is wanted
        if (nySecPeakBox > 0) then
          if (secPeakInBox) then 
            write(*,111) 'View', iview, ', shifts', xshift, yshift, '      peak',  &
                peakVal, secXshift, secYshift, peakList(indPeakSort(2))
          else
            write(*,111) 'View', iview, ', shifts', xshift, yshift, '      peak', peakVal
          endif
        endif
        ...
        if (.not.tracking .and. nySecPeakBox <= 0) write(*,111) &
            'View', iview, ', shifts', xshift, yshift, '      peak', peakVal
111     format(a,i4,a,2f10.2,a,g15.6,2f10.2,g15.6)
        if (ifScanRotation > 0) then
          if (numRotSteps > 0) then
            !
...
CONTAINS
  ...
  subroutine correlateAndFindPeaks(useCCC, ubArray, ubBrray, ifDoImOut)
    ...
    integer*4 indFirstOut, indSecondOut, nxInterp, nyInterp
    ...
    if (ifExclude > 0 .and. numXcorrPeaks > 1) then
      ...
      if (indFirstOut > 0 .and. inStreak(ind) .and. &
        ...
        if (ifUBpeakIsSharp < 0) then
          ...
          ifUBpeakIsSharp = 0
          if (ubPeakList(2) > 0 .and. sqrt(ubXpeaks(1)**2 + ubYpeaks(1)**2) < 0.2 .and. &
              widths(1) <= centralPeakMaxWidth .and. &
  ...
  subroutine maskOutsideBoundaries(arrayMOB, nxMOB, nyMOB)
    ...
    integer*4 taperAtFill, numOMPthreads
    !
    !$OMP PARALLEL do num_threads(numOMPthreads(8)) &
...
subroutine anglesPassThrough(first, testInc, nz, pass0, pass90, passMin90)
  implicit none
  real*4 first, testInc, tilt, tiltLast
  logical*4 pass0, pass90, passMin90
  integer*4 nz, ind
  pass0 = .false.
  pass90 = .false.
  passMin90 = .false.
  tiltLast = first
  do ind = 1, nz - 1
    tilt = first + ind * testInc
    if (min(tiltLast, tilt) <= 0 .and. max(tilt, tiltLast) > 0) pass0 = .true.
    if (min(tiltLast, tilt) <= 90 .and. max(tilt, tiltLast) > 90) pass90 = .true.
    if (min(tiltLast, tilt) <= -90 .and. max(tilt, tiltLast) > -90) passMin90 = .true.
    tiltLast = tilt
  enddo
  return
end subroutine anglesPassThrough
