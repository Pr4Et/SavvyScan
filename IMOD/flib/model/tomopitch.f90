...
program tomopitch
  ...
  if (pipInput) then
    ...
    if (numFiles == 0) call exitError('No model file entered')
  ...
  do while(ifile <= numFiles)
    ...
    if (.not.useTimes .and. numVertical == 0 .and. numHoriz == 0) then
      ...
      if (numFiles == 1) then
        ...
        if (maxTime > 0) then
          ...
          ySample = -deltaY * (numFiles - 1.) / 2.
          if (minTime == 0) then
            write(*,'(/,a)') 'ERROR: TOMOPITCH - The model file has '// &
                'multiple times but has some contours with no time index.'// &
                ' Either contours with time or ones without should be eliminated'
            call exit(1)
          endif
        else
          !
    ...
    if (numFiles * numPatch > LIMTOT) call exitError('Too many total patches for arrays')
    !
