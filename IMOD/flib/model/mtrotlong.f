...
c
      ...
      character*320 modelfile,pointfile,secode,objcode,intout, comfile
...
10    write(*,'(1x,a,/,a,$)')
     &    'object #, contour #, do start(0), end(1), or mid(-1), rotate end',
     &    '  to bot(0) or top(1), # of points to fit: '
      read(5,*)iobjin,icont,ifend,iftop,npt
      if(iobjin.eq.0)then
        close(3)
        write(*,'(//,a,a,a)')'The command file ',
     &      trim(comfile),' is ready to run'
        call exit(0)
      endif
...
102   format(2i4,2i3,/,i4,2f6.1,/,3f7.1)
     ...
     &      secode(max(0,ifend)+1:max(0,ifend)+1)//'.st'
        write(3,104)'$rotatevol'
104     format(a)
        write(3,104)trim(modelfile)
        if(ifcommand.eq.1)then
          write(3,104)trim(pointfile)
        else
          write(3,104)'mtl.st1'
        endif
        write(3,104)
...
105     format(i6,2f8.1,/,3f7.1)
        if (ifcommand.ne.1)then
          ...
          write(3,104)'$\\mv mtl.sth '//trim(pointfile)
        endif

        write(3,104)'$alterheader '//trim(pointfile)
        write(3,104)'org'
        write(3,104)'0,0,0'
        write(3,104)'tlt'
        write(3,104)'0,0,0'
