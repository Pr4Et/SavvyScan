...
module ccdvars
  ...
  parameter (MAXDEV = 300, LIMPATCH = 40000, LIMPTOUT = 25 * LIMPATCHOUT)
  integer*4 numPixBorder, iorder, ifIncludeAdj, iScanSize, numEdgePixels, ifVerbose
  integer*4 matSize, maxInDiffPatch, nxAllocScan, nyAllocScan, noiseExtraBord
  ...
  logical*4, allocatable :: scanAreaChanged(:,:)
  real*4, allocatable :: vicinity(:,:), vicinitySqr(:,:), sdArray(:)
  logical*4 fillWithNoise
end module ccdvars
...
program ccderaser
  ...
  real*4 annulusWidth, radSq, sizeMax, size, xcen, ycen, dist, roughMean
  integer*4 ifPeakSearch, numPatch, numPixels, ifTouch, numSearchIterations
  integer*4 ifTrialMode, maxObjectsOut, ifGrew, numPatchTmp, numPixelsTmp
  ...
  newYoverlap = 0
  modelOut = ' '
  numSearchIterations = 3  ! Default in adoc
  nxAllocScan = 0
  nyAllocScan = 0
  call b3dDate(dat)
  call time(tim)
  ...
  if (pipInput) then
    ...
    ierr = PipGetInteger('SearchIterations', numSearchIterations)
    numSearchIterations = max(1, numSearchIterations)
  ...
  itype = 2
  if (fillWithNoise) itype = 2 * (MAXDEV + noiseExtraBord)
  allocate(vicinity(itype, itype), vicinitySqr(itype, itype), sdArray(itype**2), &
      stat = ierr)
  call memoryError(ierr, 'arrays for local noise measurement')
...
101 format('Section',i5,' -',$)
    ...
    if (nx > 10 .and. ny > 10) then
      ...
      do iy1 = 1, 8
        iy2 = ny * iy1 / 10 - 1
        do ix1 = 1, 8
          roughMean = roughMean + array((nx * ix1) / 10 + nx * iy2) / 64
        enddo
      enddo
    endif
    ...
    numPatchOut = 0
    numPtOut = 0
    if (ifPeakSearch > 0) then
      do iobj = 1, numSearchIterations
        numPatchTmp = 0
        call searchPeaks(array, diffArr, izSect, numPatchTmp, numPixelsTmp, numPtOut, &
            numPatchOut, iobj, roughMean)
        numPatch = numPatch + numPatchTmp
        numPixels = numPixels + numPixelsTmp
        if (numPatchTmp == 0) then
          if (ifVerbose > 0) print *,'Done after:', iobj - 1
          exit
        endif
      enddo
    endif
...
subroutine searchPeaks(array, diffArr, izSect, numPatch, numPixels, numPtOut, &
    numPatchOut, iterNum, roughMean)
  use ccdvars
  implicit none
  integer LIMLIST
  parameter (LIMLIST = 400)
  integer*4 izSect, iterNum
  ...
  real*4 dmin, dmax, psum, sumsq, scanAvg, scanSd, polarity, scanCrit, roughMean
  ...
  if (numScanX > nxAllocScan .or. numScanY > nyAllocScan) then
    if (nxAllocScan > 0) deallocate(scanAreaChanged, stat = ix)
    allocate(scanAreaChanged(numScanX, numScanY), stat = ix)
    call memoryError(ix, 'array for track scan area changes')
    nxAllocScan = numScanX
    nyAllocScan = numScanY
  endif
  if (iterNum == 1) scanAreaChanged(:,:) = .true.
  !
  ...
  do iScanY = 1, numScanY
    ...
    do iScanX = 1, numScanX
      if (.not. scanAreaChanged(iScanX, iScanY)) then
        cycle
      endif
      scanAreaChanged(iScanX, iScanY) = .false.
        ...
        do ix = ixStart, ixEnd
          if (abs(array(ix, iy) - scanAvg) > scanCrit) then
            ...
            call checkAndErasePeak(radiusMax, outerRadius, critMain, .false., aboveCrit, &
                'P', roughMean)
            if (aboveCrit) scanAreaChanged(iScanX, iScanY) = .true.
            if (critGiant > 0. .and. .not. aboveCrit .and.  &
              ...
              call checkAndErasePeak(giantRadius, giantRadius + outerRadius - radiusMax, &
                  critGiant, .true., aboveCrit, 'Giant p', roughMean)
              if (aboveCrit) scanAreaChanged(iScanX, iScanY) = .true.
            endif
          endif
        enddo
...
104           format(/,'Diff peak at',2i6,' = ',f8.0,', diff =',f8.0, &
              ...
              scanAreaChanged(iScanX, iScanY) = .true.
            else
              numPtOut = numPtSave
...
CONTAINS
  ...
  subroutine checkAndErasePeak(radMax, outerRad, critPeak, doingBig, aboveCrit, bigText, &
      roughMean)
    real*4 radMax, outerRad, critPeak, roughMean
    ...
    do jy = jys, jyn
      do jx = jxs, jxn
        ...
        if (radSq > radInnerSq .and. radSq <= outerSq) then
          numSum = numSum + 1
          sum8 = sum8 + (array(jx, jy) - roughMean)
          sumsq8 = sumsq8 + (array(jx, jy) - roughMean)**2
        endif
      enddo
    enddo
    call sums_to_avgsd8(sum8, sumsq8, numSum, 1, ringAvg, ringSd)
    ringAvg = ringAvg + roughMean
    !
    ! Make sure the peak passes the main criterion
    !
    sdDiff = 0.
    if (ringSd > 0) sdDiff = abs(array(ixPeak, iyPeak) - ringAvg) / ringSd
...
subroutine cleanArea(array, numInObj, numGrowIter)
  ...
  integer*4 numPatPix, numAdded, numSDpoints, minSDsForMedian, jx, jy
  ...
  integer*4 ixExtraLow, ixExtraHigh, iyExtraLow, iyExtraHigh
  real*4 c1, rsq, fra, xsum, patchSum, polarity, cutoff, percentile, adjMedian, pctile
  real*4 gauss, bigSD, bigMean, xmean, xsqSum
...
104 format(/,i5,' points to fix at',2i6,',',i4,' points being fit')
  ...
  xmean = xsum / numPoints
  !
  ! If filling with noise, get enlarged area and first scan the area marking points
  ! to include
  if (fillWithNoise) then
    numSDpoints = 0
    ixExtraLow = max(1, ixBordLow - noiseExtraBord)
    ixExtraHigh = min(nx, ixBordHigh + noiseExtraBord)
    iyExtraLow = max(1, iyBordLow - noiseExtraBord)
    iyExtraHigh = min(ny, iyBordHigh + noiseExtraBord)
    do iy = iyExtraLow, iyExtraHigh
      do ix = ixExtraLow, ixExtraHigh
        jx = ix + 1 - ixExtraLow
        jy = iy + 1 - iyExtraLow
        ixOffset = ix - ixCen
        iyOffset = iy - iyCen
        nearEdge = ix - ixBordLow < numBordM1 .or. ixBordHigh - ix < numBordM1 .or. &
            iy - iyBordLow < numBordM1 .or. iyBordHigh - iy < numBordM1
        if (.not.inList(ixOffset, iyOffset) .and. &
            (nearEdge .or. adjacent(ixOffset, iyOffset) >= minAdjacent)) then
          vicinity(jx, jy) = array(ix, iy)
          vicinitySqr(jx, jy) = array(ix, iy)**2
        else
          vicinitySqr(jx, jy) = -1.
        endif
      enddo
    ...
    do iy = 2, (iyExtraHigh - iyExtraLow) - 1
      do ix = 2, (ixExtraHigh - ixExtraLow) - 1
        ixl = 0
        xsum = 0.
        xsqSum = 0.
        do jy = iy - 1, iy + 1
          do jx = ix - 1, ix + 1
            if (vicinitySqr(jx, jy) >= 0.) then
              xsum = xsum + vicinity(jx, jy)
              xsqSum = xsqSum + vicinitySqr(jx, jy)
              ixl = ixl + 1
            endif
          enddo
        enddo
        if (ixl >= 6) then
          numSDpoints = numSDpoints + 1
          call sums_to_avgsd(xsum, xsqSum, ixl, bigMean, sdArray(numSDpoints))
        endif
      enddo
    enddo
    !
    ! If there are enough points, take the median as the SD to use
    if (numSDpoints >= minSDsForMedian) &
        call rsFastMedianInPlace(sdArray, numSDpoints, bigSD)
  endif
  !
    ...
    do ix = ixBordLow, ixBordHigh
      ...
      if (inList(ixOffset, iyOffset)) then
        if (nindep > 0 .and. numPoints <= LIMPATCH) then
          ...
          xmean = c1
          do i = 1, nindep
            xmean = xmean + b1(i) * vector(i)
          enddo
        endif
        if (fillWithNoise .and. numSDpoints >= minSDsForMedian) then
          call gaussianDeviate(gauss, 0)
          array(ix, iy) = xmean + gauss * bigSD
        else
          array(ix, iy) = xmean
        endif
      endif
