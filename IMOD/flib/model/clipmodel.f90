...
program clipmodel
  ...
  integer*4 iobjBound(LIMOBJ), numObjBound, numBoundCont, numBoundPoint
  ...
  integer*4, allocatable :: numPtInBound(:), indBoundCont(:), izBound(:), mapZbound(:)
  real*4, allocatable :: xBound(:), yBound(:)
  integer*4 i, ibase, icl, ierr, ifCut, ifExclude, ifFlip, ifKeepAll, ifLoop, ifPoint
  integer*4 ifStartCut, ifTreat, il, imodObj, inList, indY, indZ, iobj, minZbound, iz
  integer*4 iobjTest, ipt, iptEndBest, iptInside, iptStart, iptStartBest, ixMax, ixMin
  integer*4 ixx, iyMax, iyMin, iyy, izMax, izMin, izz, jpnt, lpnt, modFlag, maxZbound
  ...
  integer*4 ifValRange, ioptValues, iptOut, iptDelSize, numToDelete, maxNumPts, minDiff
  ...
  logical*4 keepEmpty, updateMM, coplanarBoundary
  integer*4 getImodFlags, getImodHead, getImodScales, getImodObjSize, findAddMinMax1Value
  integer*4 getObjValueThresh, getContValue, getPointValue, deleteImodPoint, numberInList
  logical pipinput
  ...
  if (pipInput) then
    ...
    loop = 1 - PipGetString('BoundaryObjects', listString)
    if (loop > 0) call parseList2(listString, iobjBound, numObjBound, LIMOBJ)
    if (iobj > 0 .and. loop > 0) call exitError( &
        'You cannot use a boundary object when clipping from starts and ends') 
    if (abs(ioptValues) > 1 .and. loop > 0) call exitError( &
        'You cannot use a boundary object when clipping points by value')
    if (noActions .ne. 0 .and. iobj == 0 .and. ioptValues == 0 .and. ixyzSum == 0 .and.  &
        loop == 0)  &
  ...
  if (numObjBound > 0) then
    ierr = PipGetBoolean('CoplanarBoundary', coplanarBoundary)
    numBoundCont = 0
    numBoundPoint = 0
    minZbound = 100000000
    maxZbound = -minZbound
    do loop = 1, numObjBound
      imodObj = iobjBound(loop)
      if (mod(iflags(imodObj), 4) .ne. 0) call exitError( &
          'Only closed contour objects can be used for boundary objects')
      call loadModelObject()
      do iobj = 1, max_mod_obj
        numBoundPoint = numBoundPoint + npt_in_obj(iobj)
        iz = nint(p_coord(indZ, abs(object(1 + ibase_obj(iobj)))))
        minZbound = min(minZbound, iz)
        maxZbound = max(maxZbound, iz)
      enddo
      numBoundCont = numBoundCont + max_mod_obj
    enddo
    if (numBoundCont == 0 .or. numBoundPoint == 0)  &
        call exitError('The boundary objects have no contours with points')
    allocate(xBound(numBoundPoint), yBound(numBoundPoint), numPtInBound(numBoundCont), &
        indBoundCont(numBoundCont), izBound(numBoundCont),  &
        mapZbound(maxZbound + 4 - minZbound), stat = ierr)
    ...
    do loop = 1, numObjBound
      ...
      do iobj = 1, max_mod_obj
        ...
        if (numInObj > 0) then
          numBoundCont = numBoundCont + 1
          numPtInBound(numBoundCont) = numInObj
          indBoundCont(numBoundCont) = numBoundPoint + 1
          ibase = ibase_obj(iobj)
          izBound(numBoundCont) = nint(p_coord(indZ, abs(object(1 + ibase))))
          do ipt = 1, numInObj
            jpnt = abs(object(ipt + ibase))
            numBoundPoint = numBoundPoint + 1
            xBound(numBoundPoint) = p_coord(1, jpnt)
            yBound(numBoundPoint) = p_coord(indY, jpnt)
          enddo
        endif
      enddo
    enddo
    ...
    do iz = minZbound, maxZbound
      mapZbound(iz + 1 - minZbound) = iz
    enddo
    if (.not.coplanarBoundary) then
      do iz = minZbound, maxZbound
        minDiff = 10000000
        do iobj = 1, numBoundCont
          ipt = abs(izBound(iobj) - iz)
          if (ipt < minDiff .or. (ipt == minDiff .and. iz > izBound(iobj))) then
            minDiff = ipt
            mapZbound(iz + 1 - minZbound) = izBound(iobj)
          endif
        enddo
      enddo
    endif
...
105     format(/,' Selected volume is',g15.6,' cubic ',a)
    ...
    do imodObj = 1, numObjTot
      if (numberInList(imodObj, iobjBound, numObjBound, 0) > 0) cycle
      call loadModelObject()
      !
      maxNumPts = 0
      do iobj = 1, max_mod_obj
      ...
      if (maxNumPts > iptDelSize) then
        ...
        call memoryError(ierr, 'array for deleted point list')
      endif
            ...
            if (ifKeepAll == 0 .and. mod(iflags(imodObj), 4) .ne. 2) then
              ...
              do ipt = 1, numInObj
                ...
                inside = (xx >= xmin .and. xx <= xmax) .and. &
                    ...
                    (zz >= zmin .and. zz <= zmax)
                if (inside .and. numObjBound > 0 .and. loop == 1)  &
                    call testInsideBoundary(inside)
...
CONTAINS
  ...
  ! Load one model object, give error message, scale it
  subroutine loadModelObject()
    if (.not.getModelObjectRange(imodObj, imodObj)) then
      write(*,'(/,a,i6)') 'ERROR: CLIPMODEL - LOADING DATA FOR OBJECT #', imodObj
      call exit(1)
    endif
    call scale_model(0)
  end subroutine loadModelObject

  ! Test for whether the current xx, yy, zz is inside any of the boundary contours
  ! on the Z value that it maps to (nearest one with contours, or same Z value)
  subroutine testInsideBoundary(insideBounds)
    integer*4 ibound, izpt
    logical*4 insideBounds
    logical inside
    insideBounds = .false.
    izpt = nint(zz)
    if (izpt < minZbound .or. izpt > maxZbound) return
    insideBounds = .true.
    izpt = mapZbound(izpt + 1 - minZbound)
    do ibound = 1, numBoundCont
      if (izpt == izBound(ibound)) then
        if (inside(xBound(indBoundCont(ibound)), yBound(indBoundCont(ibound)),  &
            numPtInBound(ibound), xx, yy)) return
      endif
    enddo
    insideBounds = .false.
    return
  end subroutine testInsideBoundary
