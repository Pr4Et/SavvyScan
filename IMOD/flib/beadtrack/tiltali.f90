...
!
subroutine tiltali(ifDidAlign, ifAlignDone, resMean, iview, aliMeanRes)
  use tltcntrl
  implicit none
  ...
  if (nview >= minViewsTiltAli) then
    ...
    if (tiltRange < 0.4) then
      write(errString, '(a, f5.1, a, i4, a)')'Insufficient tilt range to do tilt '// &
          'alignment (', tiltRange, ' deg over', nview, &
      ' views) - increase minimum # of views for tilt alignment'
      call errorExit(errString, 0)
    endif
    ifRotFix = 0
    ...
    if (iv > maxH) then
      ...
      call memoryErrorUC(iv, 'ARRAY FOR H MATRIX')
    endif
    !
    ...
    do i = 1, nrealPt
      ...
      aliMeanRes = aliMeanRes + rsum * scaleXY
      ifMapTilt = ifMapTilt + irealStr(i + 1) - irealStr(i)
    enddo
    aliMeanRes = aliMeanRes / ifMapTilt
    !
    write(6, 98) nview, kount, sqrt(fFinal * rmsScale), aliMeanRes
98  format(i4,' views,',i5,' cycles, F =',f13.6,', mean residual =', f8.2)
    !
    ! Error reports:
    if (ier .NE. 0) then
      print *,'tiltalign error going to view', iview, ' even after varying step factor'
    endif
    !
    do iv = 1, nview
