...
subroutine nextPos(iobj, ipNear, idir, izNext, tilt, maxFit, minFit, axisRot, tiltMin, &
    izExclude, numExclude, xnext, ynext, minTiltInd, minzDelzNearZero, maxDelzNearZero, &
    izDelToNear)
  implicit none
  ...
  integer*4 minTiltInd, minzDelzNearZero, maxDelzNearZero, izDelToNear
  ...
  if (mfit < minFit) then
    ...
    if (mfit == 0 .or. (mfit == 1 .and. minzDelzNearZero > 0 .and. &
        abs((izNext + 1) - minTiltInd) <= minzDelzNearZero .and.  &
        abs(izNext - nint(p_coord(3, iptNear))) <= maxDelzNearZero)) then
      mfit = 1
      xnext = p_coord(1, iptNear)
      ynext = p_coord(2, iptNear)
      izDelToNear = izNext - nint(p_coord(3, iptNear))
    else
      xsum = 0.
      ysum = 0.
      do i = 1, mfit
...
subroutine findPiece(ixPclist, iyPclist, izPclist, nPclist, nx, ny, nxBox, nyBox, &
    xnext, ynext, izNext, ix0, ix1, iy0, iy1, ipcz, ifXfs, prexf, needTaper, needFill)
  implicit none
  ...
  logical*4 needTaper, needFill, xOK, yOK
  integer*4 indx0, indx1, indy0, indy1, ipc, indGood0, indGood1, nxGood, nyGood, ipcAtZ
  !
  critNonBlank = 0.75
  ...
  ipcAtZ = 0
  do ipc = 1, nPclist
    if (izNext == izPclist(ipc)) then
      if (ipcAtZ > 0) return
      xOK = indx0 >= ixPclist(ipc) .and. indx1 < ixPclist(ipc) + nx
      yOK = indy0 >= iyPclist(ipc) .and. indy1 < iyPclist(ipc) + ny
      needFill = .true.

      ! One direction must be OK, so there is missing data just on one side
      if (yOK .and. indx0 < ixPclist(ipc) .and. indx1 > ixPclist(ipc)) then
        nxGood = nxGood - (ixPclist(ipc) - indx0)
      else if (yOK .and. indx1 >= ixPclist(ipc) + nx .and. indx0 < ixPclist(ipc) + nx)  &
          then
        nxGood = nxGood - (indx1 - (ixPclist(ipc) + nx - 1))
      else if (xOK .and. indy0 < iyPclist(ipc) .and. indy1 > iyPclist(ipc)) then
        nyGood = nyGood - (iyPclist(ipc) - indy0)
      else if (xOK .and. indy1 >= iyPclist(ipc) + ny .and. indy0 < iyPclist(ipc) + ny)  &
          then
        nyGood = nyGood - (indy1 - (iyPclist(ipc) + ny - 1))
      else
        needFill = .false.
      endif
      if (.not.needFill .or. nxGood * nyGood < critNonBlank * nxBox * nyBox) return
      ipcAtZ = ipc
      ix0 = max(0, indx0 - ixPclist(ipc))
      ix1 = min(nx - 1, indx1 - ixPclist(ipc))
      iy0 = max(0, indy0 - iyPclist(ipc))
      iy1 = min(ny - 1, indy1 - iyPclist(ipc))
    endif
  enddo
  ipcz = ipcAtZ - 1
  return
...
! dimensioned nxBox x nyBox, WHICH MUST BE THE BASIC nxBox, nyBox IN MAIN because 
! that is what other elong arrays are dimensioned to.
! The nominal center is in xpeak, ypeak
!
...
subroutine adjustXYZinAreas(iobjLists, listSize, indObjList, ninObjList, xyzAll, &
  ...
  do loop = 1, 2
    ...
    if (loop == 1) then
      ...
      call memoryErrorUC(iseq, 'ARRAY FOR INDICES IN PREVIOUS AREAS')
    endif
  enddo
  !
