...
! $Id$
!
subroutine input_model(imodObj, imodCont, numProjPt,  iwhichOut, xcen, ycen, delta, &
    listz, maxZlist, modelFile, residualFile, pointFile, iuAngle, iuXtilt, &
    xorig, yorig, zorig, pipinput)
!
  use alivar
  implicit none
  real*4 xcen, ycen
  ...
  real*4 xdelt, ydelt, dmin, dmax, dmean
      ...
      do ipt = 1, numInObj
        ...
        if (numberInList(iz, listz, numInZlist, 0) == 0) then
          ...
          if (numinzlist > maxzlist) &
              call errorexit('Too many Z values in model for temporary arrays', 0)
          listz(numInZlist) = iz
        endif
      enddo
    endif
