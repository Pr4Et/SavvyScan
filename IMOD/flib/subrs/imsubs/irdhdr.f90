...
subroutine irdhdr(iunit, nxyz, mxyz, imode, dmin, dmax, dmean)
  ...
  character*19 originText, minMaxText, meanText
  !
  ...
  integer*4 lensnum, nd1, nd2, iflags, imodFlags, ifBrief, nVersion, ifileType
  ...
  call iiuFileInfo(iunit, i, ifileType, iflags)
...
1007 format(/,'  Assuming that the header entry for 128 integers per section is an error')
  !
  if (btest(iflags, 5) .and. doExtra) then 
    if (ifileType == 1) then
      write(6, 1011)'  This file is actually a 4-bit TIFF file'
    else
      write(6, 1011)'  This file actually has 4-bit values and is MRC mode 101'
1011  format(/,a)
    endif
  endif
  if (btest(iflags, 6) .and. doExtra) write(6, 1012) nxyz(1) / 2
1012 format(/,'  This file actually has 4-bit values packed in bytes, with ',i4, &
         ' bytes in X')
  !
  if (btest(iflags, 7) .and. doExtra) &
    write(6, '(/,9x,a)') 'This MRC file is apparently inverted in Y'
  !
  ...
  minMaxText = '...................'
  meanText = '...................'
  if ((nVersion > 0 .or. (isIMOD > 0 .and. iand(imodFlags, 4) .ne. 0)) .and. &
      (xorig .ne. 0. .or. yorig .ne. 0. .or. zorig .ne. 0.)) &
      originText = '(inverted_in_file_)'
  if (dmin > dmax) minMaxText =  '...(undetermined)...'
  if (dmean < min(dmin, dmax)) meanText =  '...(undetermined)...'
  minLabel = ' '
  maxLabel = ' '
  meanLabel = ' '
...
1010 format(' (',g13.5,' in file)')
  ...
  if (doExtra) then
    write(6, 1000) nxyz, imode, modeLabel, nxyzst, mxyz, delta, (cell(k), k = 4, 6), &
        (lxyz(mapcrs(k)), k = 1, 3), originText, xorig, yorig, zorig, minMaxText, dmin, &
        trim(minLabel), minMaxText, dmax, trim(maxLabel), meanText, dmean, trim(meanLabel)
...
1000 format(/, &
      ...
      1x,'Origin on x,y,z ..',a              ,'..',1x,3g12.4,/, &
      1x,'Minimum density ..',a              ,'..',g13.5,a,/, &
      1x,'Maximum density ..',a              ,'..',g13.5,a,/, &
      1x,'Mean density .....',a              ,'..',g13.5,a)
1019 format(&
