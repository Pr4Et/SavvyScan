...
module psparams
  ...
  character*320 filename/'gmeta.ps'/
end module psparams

subroutine psSetup(ithickSet, widthSet, upiSet, safeSet, ifset)
  use psparams
  implicit none
  integer*4 ithickSet, ifset
  real*4 widthSet, upiSet, safeSet
  integer*4 ierr
  real*4 fThick
...
subroutine psSetFilename(newName)
  use psparams
  implicit none
  character*(*) newName
  call psPakOff()
  filename = newName
end subroutine psSetFilename
...
subroutine psVectAbs(x, y)
  use psparams
  implicit none
  real*4 x, y, trnc, ddx, ddy, ddlen, xinc, yinc, xs, xe, ys, ye
  if (nThick <= 1) then
...
subroutine pltout(metaScreen)
  use psparams
  implicit none
  integer*4 metaScreen
  character*120 comstr
  character*80 imodPath
