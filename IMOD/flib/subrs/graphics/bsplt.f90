...
subroutine bsplt(namex, xx, yy, igroupNum, numPoints, isymbol, numGroups, irecx, irecy,  &
  ...
  integer*4 numLines, numXticks, numYticks, numDash
  ...
  if (ifTerm == 0) then
    ...
    if (zeroLineDashLen > 0 .and. ylow < 0. .and. ylow + 10. * dy > 0.) then
      iy = yadd
      numDash = max(1., 0.5 / zeroLineDashLen)
      do i = 1, numDash
        xleft = xlow + 10. * dx * (i - 1) * 2 * zeroLineDashLen
        xright = min(xlow + 10. * dx, xleft + 10. * dx * zeroLineDashLen)
        ix = xscale * xleft + xadd
        call scrnMoveAbs(ix, iy)
        ix = xscale * xright + xadd
