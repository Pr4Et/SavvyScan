...
static int argc;
static char **argv;
static char sComFile[FSTRING_LEN];
...
static void draw();
static void diaLimitWindowPos(int neww, int newh, int &newdx, int &newdy);
...
extern "C" {
  ...
  void realgraphicsmain_(char *string, int len);
}
...
void PlaxThread::run()
{
  realgraphicsmain_(sComFile, FSTRING_LEN);
  ::exit(0);
}
...
void plax_initialize(char *string, int strsize)
{
  ...
  c2fString(" ", sComFile, FSTRING_LEN);
...
static int startPlaxApp()
{
  ...
  diaLimitWindowPos(sPlaxWidth, sPlaxHeight, sPlaxLeft, sPlaxTop);
