...
! parallelHDF        Flag that output is being done to an HDF file in parallel
...
! numExactCycles     Cycle of exact filter weighting function to use (1 since no sinc)
! exactSamples       Number of elements in the weightfunction table
! exactObjSize       Object size parameter for exact filters
! numFakeSIRTiter    # of iterations for fake SIRT bp weighting function
...
module tiltvars
  ...
  logical*4 rotateBy90, parallelHDF
  !
  ...
  real*4, allocatable ::  stretchOffset(:), wgtAngles(:), exactTable(:)
  !
  integer*4 numTiltIncWgt, numWgtAngles, numExactCycles, numFakeSIRTiter
  real*4 tiltIncWgts(20), exactSamples, exactObjSize
  !
