...
static void LineOfValuesError(const char *fullStr, const char *format, ...);
...
{
  char *argCopy;
  int err, i, lenarg, newNum, ifAlloc, noMatch;
  char **newArgs;
  char ch;
  FILE *paramFile;
  int indStart;
  ...
  newArgs = expandArgList(&argString, 1, &newNum, &ifAlloc, &noMatch);
  if (!newArgs) {
    PipSetError("Memory allocation failed when expanding non-option arguments");
    return -1;
  }
  for (i = 0; i < newNum; i++) {
    if (!ifAlloc) {
      ...
      argCopy = newArgs[i];
    err = AddValueString(sNonOptInd, argCopy);
    if (err)
      return err;
  }
  return 0;
...
/*!
 * Parses a line of values from the string in [strPtr] and returns them into [array], 
 * whose size is given by [arraySize].  The type is indicated in [valType] as PIP_INTEGER
 * (1) for integer, PIP_FLOAT (2) for float, or PIP_DOUBLE (3) for double.
                       ...
                       int *numToGet, int arraySize)
{
  ...
  while (strlen(strPtr)) {
    ...
    if (!sepPtr) {
      ...
      if (strPtr[0] == '/') {
        ...
        LineOfValuesError(fullStr, "Default entry with a / is not allowed"
      ...
      if (strPtr[0] == ',') {
        ...
        if (gotComma) {
          if (sAllowDefaults && *numToGet) {
            ...
            LineOfValuesError(fullStr, "Default entries with commas are not allowed"
                    " in value entry:  %s  ", option);
            return -1;
          }
        }
    ...
    if (numGot >= arraySize) {
      LineOfValuesError(fullStr, "Too many values for input array in value entry:  %s  ",
              option);
      return -1;
    }
    ...
    if (valType == PIP_INTEGER)
      ...
      darray[numGot++] = (float)strtod(strPtr, &invalid);

    /* If invalid character is before end character, it is an error */
    if (invalid != endPtr) {
      LineOfValuesError(fullStr, "Illegal character in value entry:  %s  ", option);
      return -1;
    }
  ...
  if (numGot < *numToGet) {
    LineOfValuesError(fullStr, "%d values expected but only %d values found in value "
...
/*
 * PipLineOfValues can be called without initializing PIP, so this function is needed
 * to allocate sTempStr if needed.  It handles appending the input string to the error.
 */
static void LineOfValuesError(const char *fullStr, const char *format, ...)
{
  va_list args;
