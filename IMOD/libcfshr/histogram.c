 ...
 * (1-(x/h)^2)^3 / (0.9143 * h) is added at each point.  Use 0 for a standard binned 
...
{
  float dxbin, delta, val, scale;
  int i, j, ist, ind, optimal = 1, threadNum, maxThreads = 8, numThreads = 1;
  ...
  if (h != 0.) {
    ...
    B3DCLAMP(optimal, 1, maxThreads);
    numThreads = numOMPthreads(optimal);
    numThreads = B3DMIN(numThreads, MAX_HIST_THREADS);
