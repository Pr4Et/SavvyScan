 ...
 *
 * Copyright (C) 2009-2019 by the Regents of the University of 
...
float gaussianDeviate(int seed)
{
  ...
  static float savedVal;
  unsigned int useSeed;
  float val1, val2, value, fac, radSq =2.;
  if (valSet < 0 || seed != lastSeed) {
    if (seed > 0)
      useSeed = (unsigned int)seed;
    else
      useSeed = (unsigned int)time(NULL);
    srand(useSeed);
    valSet = 0;
    lastSeed = seed;
  }
  if (valSet > 0) {
    ...
    while (radSq >= 1. || radSq <= 0.) {
      val1 = (2. * rand()) / RAND_MAX - 1.;
      val2 = (2. * rand()) / RAND_MAX - 1.;
      radSq = val1 * val1 + val2 * val2;
    } 
    fac = sqrt(-2. * log(radSq) / radSq);
    value = val1 * fac;
    savedVal = val2 * fac;
    valSet = 1;
  }
