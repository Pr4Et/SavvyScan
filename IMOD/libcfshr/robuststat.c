...
#define rspercentileofsorted RSPERCENTILEOFSORTED
#else
...
#define rspercentileofsorted rspercentileofsorted_
#endif
...
void rspercentileofsorted(float *x, int *n, float *fraction, float *pctile)
{
  rsPercentileOfSorted(x, *n, *fraction, pctile);
}
 ...
 * [tmp] is returned with sorted values of the absolute deviations.
 */
 ...
 * positive direction from the median, 0 otherwise.  A typical value for [kcrit] 
 * would be 2.24, the square root of the 0.975 quantile of a chi-square distribution
 * with one degree of freedom (Wilcox, P. 101, see @@rsMADN@).
 */
void rsMadMedianOutliers(float *x, int n, float kcrit, float *out)
{
  ...
  for (i = 0; i < n; i++) {
    if (madn && fabs((double)(x[i] - median)) / madn > kcrit)
