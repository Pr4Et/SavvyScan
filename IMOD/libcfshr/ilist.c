 ...
 *
 *  Copyright (C) 1995-2004 by the Regents of the University of 
...
/*! Appends [data] to the end of [list], where the argument should be a pointer to the 
  item to be copied into the list. Returns 1 if memory error, but retains previous 
  state. */
int ilistAppend(Ilist *list, void *data)
{
  ...
  if (list->store <= list->size){
    if (list->store)
      newData = realloc(list->data, list->dsize * (list->size + list->quantum));
    else
      newData = malloc(list->dsize * list->quantum);
    if (!newData)
      return 1;
    list->data = newData;
...
/*!
 * Inserts an item pointed to by [data] into [list] at the position given by [element];
 * returns 1 if error 
 */
