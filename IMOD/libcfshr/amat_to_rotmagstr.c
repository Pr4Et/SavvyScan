...
#define amat_to_rotmag AMAT_TO_ROTMAG
#define rotmag_to_amat ROTMAG_TO_AMAT
#else
#ifdef G77__HACK
#define amat_to_rotmagstr amat_to_rotmagstr__
#define rotmagstr_to_amat rotmagstr_to_amat__
#define rotmag_to_amat rotmag_to_amat__
#define amat_to_rotmag amat_to_rotmag__
#else
#define amat_to_rotmagstr amat_to_rotmagstr_
#define rotmagstr_to_amat rotmagstr_to_amat_
#define rotmag_to_amat rotmag_to_amat_
#define amat_to_rotmag amat_to_rotmag_
#endif
#endif
...
/*!
 * Converts a 2 by 2 transformation matrix into four "semi-natural" parameters of 
 * image transformation.  The transformation is specified by [a11], [a12],
 * [a21], and [a22], where
 * ^   x' = a11 * x + a12 * y
 * ^   y' = a21 * x + a22 * y   ^
 * In the converted transformation, [theta] is overall rotation, [smag] is overall
 * magnification, [ydtheta] is the rotation of the Y axis minus the rotation of the X 
 * axis, and [ydmag] is the stretch along the Y axis minus the stretch along the X axis.
 */
void amatToRotMag(float a11, float a12, float a21, float a22, float *theta, 
                  float *ydtheta, float *smag, float *ydmag)
{
  float xmag, ymag, xtheta, ytheta;
  xmag = sqrt(a11 * a11 + a21 * a21);
  xtheta = atan2(a21, a11) / RADIANS_PER_DEGREE;
  ymag =sqrt(a12 * a12 + a22 * a22);
  ytheta = atan2(-a12, a22) / RADIANS_PER_DEGREE;
  *smag = 0.5 * (xmag + ymag);
  *ydmag = ymag - xmag;
...
/*!
 * Fortran wrapper to @amatToRotMag, where [amat] is dimensioned (2,*)
 * or otherwise has elements in the order [a11], [a21], [a12], [a22].
 */
void amat_to_rotmag(float *amat, float *theta, float *ydtheta, float *smag, float *ydmag)
{
  amatToRotMag(amat[0], amat[2], amat[1], amat[3], theta, ydtheta, smag, ydmag);
}

/*!
 * Converts axial rotation and stretch specifications into a transformation matrix in
 * [a11], [a12], [a21], and [a22].  [theta] is overall rotation, [smag] is overall
 * magnification, [ydtheta] is the rotation of the Y axis minus the rotation of the X
 * axis, and [ydmag] is the stretch along the Y axis minus the stretch along the X axis.
 */
                  ...
                  float *a12, float *a21, float *a22)
{
  ...
  *a11 = xmag * cos(xtheta * RADIANS_PER_DEGREE);
  *a21 = xmag * sin(xtheta * RADIANS_PER_DEGREE);
  *a12 = -ymag * sin(ytheta * RADIANS_PER_DEGREE);
  *a22 = ymag * cos(ytheta * RADIANS_PER_DEGREE);
}

/*!
 * Fortran wrapper to @rotMagToAmat, where [amat] is dimensioned (2,*)
 * or otherwise has elements in the order [a11], [a21], [a12], [a22].
 */
void rotmag_to_amat(float *theta, float *ydtheta, float *smag, float *ydmag, float *amat)
{
  rotMagToAmat(*theta, *ydtheta, *smag, *ydmag, amat, amat + 2, amat + 1, amat + 3);
}
