...
#include "livewire/Weights.h"					// jeff's livewire code for weighting images
...
public:
  DrawingTools(DockingDialog *parent, const char *name = NULL);
  ~DrawingTools() {};
...
struct DrawingToolsData   // contains all local plugin data
{
  ...
  int    lwOpt;									// user presets for livewire (see: livewireopt)
	...
	Livewire::Weights::Settings lwSettings;		// current settings used by the
																											//  livewire image "weights" thread
	
	Livewire::Weights *weights;			// the thread which inputs an area of image
