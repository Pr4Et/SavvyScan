 ...
 *
 *  This version has been modified to be a DockingDialog, which is suitable for
 *  a dialog having no expandable parts (e.g. tables).  See IMOD 4.9 source for
 *  the prior version which illustrates a stand-alone dialog
 *
 *  $Id$
 */
...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Bead Fixer2", "plughelp/beadfix2.html#TOP", '9',
                              Qt::Window);
  plug->window  = new BeadFixer2(sTopWin, "bead fixer");
...
// This is how buttons would be added with tooltips at the bottom with tool tips,
// but Done and Help are not needed with a DockingDialog container.  The 0 would be a
// 2 to add two buttons.
static const char *buttonLabels[] = {"Done", "Help"};
static const char *buttonTips[] = {"Close Bead Fixer", "Open help window"};

BeadFixer2::BeadFixer2(DockingDialog *parent, const char *name)
  : DialogFrame(parent, 0, buttonLabels, buttonTips, true, "Bead Fixer", "", name)
{
  QPushButton *button;
  QCheckBox *box;
  QString qstr;
...
// Respond to button presses (but there are no bottom buttons here)
void BeadFixer2::buttonPressed(int which)
{
}

// The window is closing, remove from manager
void BeadFixer2::topCloseEvent ( QCloseEvent * e )
{
  PlugData *plug = &thisPlug;
...
void BeadFixer2::topChangeEvent(QEvent *e)
{
