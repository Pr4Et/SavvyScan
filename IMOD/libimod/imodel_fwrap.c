...
#define imodwriteaswimp IMODWRITEASWIMP
...
#define imodwriteaswimp imodwriteaswimp_
...
void imodopenerror(char *error, int errlen)
{
  if (sLastOpenError > -1|| sLastOpenError <= FWRAP_PAST_ERRORS)
    c2fString("", error, errlen);
  else
    c2fString(sErrorStrings[-1 - sLastOpenError], error, errlen);
}
...
int deleteimodmeshes(int *ob)
{
  ...
  if (err)
    return err;
  if (sObj->mesh)
    imodMeshesDelete(sObj->mesh, sObj->meshsize);
  sObj->mesh = NULL;
  sObj->meshsize = 0;
  return FWRAP_NOERROR;
}
...
/*!
 * Adds a point with coordinates [x], [y], [z] to contour [co] of object [ob] at either
 * an index or a Z value given in [ptOrZ].  If [ifByZ] is 0, then [ptOrZ] should have
 * the floating point value of the index at which to add the point (numbered from 1).
 * If [ifByZ] is non-zero, the point will be added before the first existing point with
 * a Z value greater than or equal to [ptOrZ].  The point must be in the same scaled 
 * coordinates as in the model returned by this module.  The point is added using
 * @imodel.html#imodPointAdd,  which will adjust sizes, labels, and general storage 
 * items in the contour, if any.
 */
int addimodpoint(int *ob, int *co, float *ptOrZ, int *ifByZ, float *x, float *y, float *z)
{
  int pt = B3DNINT(*ptOrZ) - 1;
  Ipoint point;
  int err = checkAssignObjCont(ob, co);
  if (err)
    return err;
  ...
  if (*ifByZ) {
    for (pt = 0; pt < sCont->psize; pt++)
      if (sCont->pts[pt].z >= *ptOrZ)
        break;
  }
  pt = B3DMIN(pt, sCont->psize);
  point.x = *x;
  point.y = *y;
  point.z = *z;
  if (!imodPointAdd(sCont, &point, pt))
    return(FWRAP_ERROR_FROM_CALL);
  return FWRAP_NOERROR;
}
...
int writeimod(char *fname, int fsize)
{
  ...
  if (sWriteAsWimp) {
    retcode = imod_to_wmod(sImod, sImod->file, filename);
    fclose(sImod->file);
    sWriteAsWimp = 0;
  } else {
...
/*!
 * Writes the current model as an ascii WIMP model file with the same library
 * function used by 3dmod 
 */
int imodwriteaswimp(char *fname, int fsize)
{
  sWriteAsWimp = 1;
  return writeimod(fname, fsize);
}
