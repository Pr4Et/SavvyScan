...
int imodPointIntersect(Ipoint *a, Ipoint *b, Ipoint *c, Ipoint *d)
{
  ...
  float abMin, abMax, cdMin, cdMax, epsilon = 1.e-2;

  /* First test whether bounding boxes of the two segments overlap in X and Y */
  abMin = B3DMIN(a->x, b->x);
  abMax = B3DMAX(a->x, b->x);
  cdMin = B3DMIN(c->x, d->x);
  cdMax = B3DMAX(c->x, d->x);
  if (abMin > cdMax + epsilon || cdMin > abMax + epsilon)
    return 0;
  abMin = B3DMIN(a->y, b->y);
  abMax = B3DMAX(a->y, b->y);
  cdMin = B3DMIN(c->y, d->y);
  cdMax = B3DMAX(c->y, d->y);
  if (abMin > cdMax + epsilon || cdMin > abMax + epsilon)
    return 0;
