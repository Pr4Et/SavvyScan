...
void istoreDump(Ilist *list)
{
  ...
  char *types[] = {"COLOR", "FCOLOR", "TRANS", "GAP", "CONNECT", "3DWIDTH", "2DWIDTH", 
                   ...
                   "VALUE6", "MINMAX6", "ISOPARAM", "ISOTHRESH"};
  printf(" %d items in list:\n", ilistSize(list));
  for (i = 0; i < ilistSize(list); i++) {
    store = istoreItem(list, i);
    printf("%6d-", store->type);
    if (store->type > 0 && store->type <= sizeof(types) / sizeof(char *))
      printf("%s", types[store->type - 1]);
    printf("  %6o-", store->flags);
    dtype = 0;
    j = (store->type == ISO_STORE_THRESH || store->type == ISO_STORE_PARAMS) ? 1 : 0;
    if (store->flags & GEN_STORE_NOINDEX) {
      printf("NOIND");
      dtype = 1;
    }
    if (!j && (store->flags & GEN_STORE_REVERT)) {
      printf("%sREVERT", dtype ? "|" : "");
      dtype = 1;
    }
    if (!j && (store->flags & GEN_STORE_SURFACE)) {
      printf("%sSURF", dtype ? "|" : "");
      dtype = 1;
    }
    if (!j && (store->flags & GEN_STORE_ONEPOINT)) {
      printf("%sONEPT", dtype ? "|" : "");
      dtype = 1;
    }

    if (j && (store->flags & ISO_STORE_CAP)) {
      printf("%sCAP", dtype ? "|" : "");
      dtype = 1;
    }
    if (j && (store->flags & ISO_STORE_DELETE)) {
      printf("%sDEL", dtype ? "|" : "");
      dtype = 1;
    }
    if (j && (store->flags & ISO_STORE_HAS_OUTER))
      printf("%sOUTER", dtype ? "|" : "");
    dtype = store->flags & 3;
    item = &store->index;
...
int istoreBreakContour(Icont *cont, Icont *ncont, int p1, int p2) 
{
  ...
  if (p2 < 0)
    p2 = cont->psize - 1;
  if (!ncont || !cont->psize || p1 < 0 || p1 >= cont->psize ||
      p2 >= cont->psize || p2 < p1)
    return 1;
