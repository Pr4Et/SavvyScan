...
/*!
 * Returns the current mesh surface number for model [imod] if the current object is legal
 * and has meshes but no contours, and if the stored surface number is in range for this
 * object; otherwise returns -1.
 */
int imodGetCurMeshSurf(Imod *imod)
{
  ...
  if (!obj->contsize && obj->meshsize > 0 && imod->curMeshSurf <= obj->surfsize)
    return imod->curMeshSurf;
  return -1;
}

/*!
 * Set the current mesh surface number for model [imod] to [surf] if the current object 
 * is legal and has meshes but no contours, and if [surf] is in range for this object;
...
void imodSetCurMeshSurf(Imod *imod, int surf)
{
  ...
  if (imod->cindex.object < 0 || imod->cindex.object >= imod->objsize)
    return;
  obj = &imod->obj[imod->cindex.object];
  if (!obj->contsize && obj->meshsize > 0 && surf <= obj->surfsize)
    imod->curMeshSurf = surf;
}
...
void imodCleanSurf(Imod *imod)
{
  ...
  for (ob = 0; ob < imod->objsize; ob++){
    ...
    maxsurf = 0;
    for (co = 0; co < obj->contsize; co++)
      ACCUM_MAX(maxsurf, obj->cont[co].surf);
    for (co = 0; co < obj->meshsize; co++)
      ACCUM_MAX(maxsurf, obj->mesh[co].surf);
