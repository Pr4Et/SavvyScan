 ...
 * routines at the end.  Also fastEigen was added as an argument to eed and diff_tensor.
 *
 *  $Id$
 */
...
void diff_tensor 
     ...
     (float    lambda,/* edge enhancing diffusion coefficient */
      ...
      float    sigma,    /* Noise scale */
      int      fastEigen, /* Flag to use fast eigenvector routine */
...
diff_tensor (lambda, nx, ny, nz, hx, hy, hz, sigma, fastEigen, grd, dxx, dxy, dxz, dyy, dyz, dzz);
...
int main (int argc, char **argv)
{
  ...
  float  vari;                   /* variance */
  int    fastEigen = 0;          /* Flag to use faster eigenvector call */
  Islice *sl;
  int sliceMode;
  char *progname = imodProgName(argv[0]);
  char *endptr;
  int nWrite = 0;
  int doWrite, iarg, nzout, kst, knd, writeArg, maxCopy;
  int* writeList;
  ...
  for (p=1; p<=pmax; p++) {
    ...
    eed (ht, nx, ny, nz, 1.0, 1.0, 1.0, sigma, lambda, fastEigen, u);
    ...
    if (doWrite || p == pmax) {
      ...
      maxCopy = STRING_MAX - 10;
      if (nWrite && !oneSlice && extArgvInd > 0)
        maxCopy -= strlen(argv[extArgvInd]);
      if (maxCopy < 5)
        exitError("Output filename would be too long for buffer");
      strncpy(outFile, argv[iarg + 1], maxCopy);
      outFile[maxCopy] = 0;
      if (nWrite && !oneSlice) {
        sprintf(&outFile[strlen(outFile)], "-%03d", p);
        if (extArgvInd > 0) {
          strcat(outFile, ".");
          strcat(outFile, argv[extArgvInd]);
        }
      }
