...
{     
  ...
  printf("\t-d      Divide unsigned shorts by 2 instead of subtracting 32767, \n"
         "\t          or doubles by 2\n");
  printf("\t-u      Save unsigned shorts or doubles in unsigned file mode (6) \n");
  printf("\t-c      Save long, ulong, or double input as 16-bit"
          " integers, not floats\n");
...
int main( int argc, char *argv[] )
{
  ...
  int    pixsize = 1;
  float  pixel;
  double pixSpacing = 0., zPixel = 0., tmean = 0.0;
  float  mean = 0.0f, min, max, allmax = -5e29f, allmin= 5e29f;
  b3dByte *sbdata;
  ...
  if (replaceFileArgVec((const char ***)&argv, &argc, &iarg, &i))
    exit(1);

  if (intype == DTYPE_DOUBLE)
    outPixsize = 4;
  ...
  if (convert) {
    if (intype < 0 || intype == DTYPE_LONG) {
      ...
      intype = setintype("double2short", &pixsize, &outtype);
      outPixsize = 2;
    } else
  ...
  if (divide && keepUshort && (intype != DTYPE_DOUBLE2SHORT))
  ...
  for (j = iarg ; j < argc-1 ; j++) {
    ...
    for (k = 0; k < z; k++) {
      ...
      case DTYPE_USHORT:
        ...
        if (divide) 
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)((int)(usdata[ind]) / 2);
        else if (!keepUshort)
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)((int)(usdata[ind]) - 32767);
        break;
      ...
      case DTYPE_ULONG2SHORT:
        ...
        if (divide) 
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)(uldata[ind] / 2);
        else if (keepUshort)
          for (ind = 0; ind < xysize; ind++)
            usdata[ind] = (b3dUInt16)uldata[ind];
        else
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)(uldata[ind] - 32767);
        break;
      ...
      case DTYPE_LONG2SHORT:
        ...
        if (divide) 
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)(ldata[ind] / 2);
        else
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)ldata[ind];
        break;

      case DTYPE_DOUBLE2SHORT:
        ddata = (double *)indata;
        sdata = (b3dInt16 *)indata;
        usdata = (b3dUInt16 *)indata;
        if (divide && keepUshort)
          for (ind = 0; ind < xysize; ind++)
            usdata[ind] = (b3dUInt16)(ddata[ind] / 2.);
        else if (keepUshort)
          for (ind = 0; ind < xysize; ind++)
            usdata[ind] = (b3dUInt16)ddata[ind];
        else if (divide)
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)(ddata[ind] / 2.);
        else
          for (ind = 0; ind < xysize; ind++)
            sdata[ind] = (b3dInt16)ddata[ind];
        break;
