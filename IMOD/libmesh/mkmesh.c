...
static float pointAreaQuick(Ipoint *p1, Ipoint *p2, Ipoint *p3, float zscale);
                                ...
                                float *up, float *down, int openObj)
{
  ...
  Ipoint *bpt, *tpt;
  float zscale = 0.;
  if (fabs(scale->x - scale->y) < 1.e-5 * scale->x)
    ...
    if (zscale) {
      for (k = 0; k < bsize; k++) {
        ...
        if (k == bdim)
          ...
          up[k + lbase] = pointAreaQuick(&tpt[j], &bpt[i], &bpt[ni], zscale);
        if (l == tdim)
          down[k + lbase] = 0.;
        else
          down[k + lbase] = pointAreaQuick(&tpt[j], &tpt[nj], &bpt[i], zscale);
        i = ni;
      }
      ...
      for (k = 0; k < bsize; k++) {
        ...
        if (k == bdim)
          ...
          up[k + lbase] = imodPointAreaScale(&tpt[j], &bpt[i], &bpt[ni], scale);
        if (l == tdim)
          down[k + lbase] = 0.;
        else
          down[k + lbase] = imodPointAreaScale(&tpt[j], &tpt[nj], &bpt[i], scale);
        i = ni;
      }
    }
    j = nj;
...
/*
 * A quicker function for area that uses only the zscale, computes the cross product
 * without a function call, uses sqrtf, and leaves off the factor of 0.5
 */
static float pointAreaQuick(Ipoint *p1, Ipoint *p2, Ipoint *p3, float zscale)
{
  Ipoint n, n1, n2;
