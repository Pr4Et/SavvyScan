...
void PickSeeds::main(int argc, char *argv[])
{
  ...
  Candidate *candp;
  GridDomain *domp;
  std::vector<float> allScores, scoreTemp;
  float scoreMedian, scoreMADN;
  char line[MAXLINE];
  Istore store;
  FILE *fp;
  ...
  for (i = 0; i < ncum; i++) {
    ...
    switch (B3DNINT(xx)) {
      ...
      SET_CONTROL_FLOAT(21, maxMADNsForScore);
      SET_CONTROL_FLOAT(22, maxClustElongScoreMADNs);
    default: break;
    }
  }
  ...
  for (ind = 0; ind < mNumCandidates; ind++) {
    ...
    if (mVerbose)
      printf("%d  tb %d %d -> %d  inc %.3f  res %.3f  miss %d  dev %.3f  score %.6f\n",
             ind, botSum, topSum, candp->topBot, compSum / ncum, resSum / ncum,
             numModels - ncum, candp->meanDeviation, candp->score);
    allScores.push_back(candp->score);
  }

  // Get the median and MADN score and the criterion for outliers to be skipped
  scoreTemp.resize(allScores.size());
  rsFastMedianInPlace(&allScores[0], allScores.size(), &scoreMedian);
  rsMADN(&allScores[0], allScores.size(), scoreMedian, &scoreTemp[0], &scoreMADN);
  mOutlierScore = scoreMedian + maxMADNsForScore * scoreMADN;
  ind = 0;
  ob = 0;
  for (i = 0; i < allScores.size(); i++) {
   if (mCandidates[i].score >= mOutlierScore) {
      ind++;
      if (mCandidates[i].clustered || mCandidates[i].overlapped)
        ob++;
    }
  }
  if (mVerbose)
    printf("Score median %.4f  MADN %.4f,  max allowed %.4f\n", scoreMedian, scoreMADN,
           mOutlierScore);
  if (ind)
    printf("%d candidates excluded by outlier scores (%d clustered or elongated)\n",
           ind, ob);
  ...
  for (phase = 0; phase < numPhase && mNumAccepted[2] < targetNumber; phase++) {
    if (phase == 1)
      mOutlierScore = scoreMedian + maxClustElongScoreMADNs * scoreMADN;
                                   ...
                                   float minSpacing)
{
  ...
  for (j = 0 ; j < numCandidates; j++) {
    ...
    if (!tooClose) {
      if (mVerbose > 1)
        printf("Accepting ranked %d  candidate %d at %.0f %.0f\n", j, mRankIndex[j], 
               candp->pos.x, candp->pos.y);
      ...
      for (ix = ixlo; ix <= ixhi; ix++) {
        ...
        for (i = 0; i < mDomains[idom].numCandidates; i++) {
          ...
          if (!cand->accepted && (cand->overlapped <= overlapThresh) && 
              ...
              (topBot > 1 || cand->topBot == topBot) && cand->score < mOutlierScore &&
...
{
  int idom, gpt, ring, numInside, numSamp, samp, gptInd;
  ...
  for (idom = 0; idom < mNumXdomains * mNumYdomains; idom++) {
    for (gpt = 0; gpt < mDomains[idom].numGridPts; gpt++) {
      areaSum = 0.;
      gptInd = mDomains[idom].gridPtInd[gpt];
      xcen = mGridPoints[gptInd].x;
      ycen = mGridPoints[gptInd].y;
      for (ring = 0; ring < numRings; ring++) {
        rr = (ring + 0.5) * delr;
      ...
      mGridPoints[gptInd].areaFrac = areaSum /maxSum;
    }
  }
}
