...
int main( int argc, char *argv[])
{
    ...
    if (argv[iarg][0] == '-'){
      ...
      case 'h':
        ...
        if (strlen(argv[iarg]) > 2) {
          if (!strcmp(&argv[iarg][1], "help")) {
            imodinfo_usage(progname);
            exit(0);
          }
          fprintf(stderr, "%s: unknown option %s; enter -help for help\n", progname,
                  argv[iarg]);
          exit(1);
        }
        break;
      ...
      case 't':
        sscanf(argv[++iarg], "%d", &useclip);
        scaninside = TRUE;
        break;
               
      case 'x':
        sscanf(argv[++iarg], "%f%*c%f", &ptmin.x, &ptmax.x);
        scaninside = TRUE;
        subarea = TRUE;
        break;

      case 'y':
        sscanf(argv[++iarg], "%f%*c%f", &ptmin.y, &ptmax.y);
        scaninside = TRUE;
        subarea = TRUE;
        break;

      case 'z':
        sscanf(argv[++iarg], "%f%*c%f", &ptmin.z, &ptmax.z);
        subarea = TRUE;
        break;
  ...
  for (; iarg < argc; iarg++){
    ...
    if (!fin){
      fprintf(stderr, "%s: Error opening input file %s%s%s\n", progname, argv[iarg],
    ...
    case MINFO_SPECIAL:
      ...
      imodinfo_special(model, argv[iarg]);
      break;
                             ...
                             Ipoint max, int useclip)
{
  ...
  // Use the proper computation of surface, works for isosurface meshes
  ...
  if (obj->meshsize){
    ...
    double psa;
    Imesh *mesh;
    Ipoint *p1, *p2, *p3, mscale;
    float xx, yy, zz;
    float zs = imod->zscale;
    int maxMeshSurf = 0, maxContSurf = 0;

    msa = (double *)malloc((obj->surfsize + 1) * sizeof(double));
    volInMesh = (double *)malloc((obj->surfsize + 1) * sizeof(double));
    if (!msa || !volInMesh) {
      free(nofc);
      free(vol);
      free(mvol);
    ...
    for (me = 0; me < obj->meshsize; me++) {
      ...
      if (!mesh || !mesh->lsize || 
        ...
        continue;
      if (mesh->surf >= 0 && mesh->surf <= obj->surfsize) {
        mscale.x = imod->xscale;
        mscale.y = imod->yscale;
        mscale.z = imod->zscale;
        volInMesh[mesh->surf] += imeshVolume(mesh, &mscale, NULL) *
          pow((double)imod->pixsize, 3.);
        ACCUM_MAX(maxMeshSurf, mesh->surf);
      }
      for (i = 0; i < mesh->lsize; i++) {
        if (imodMeshPolyNormFactors(mesh->list[i], &listInc, &vertBase, 
          ...
          psa = 0.;
          found = 0;
          if (!obj->contsize && mesh->surf >= 0 && mesh->surf <= obj->surfsize) {
            psurf = mesh->surf;
            found = 1;
          }
          ...
          if (found) {
            ...
            ACCUM_MAX(maxContSurf, psurf);
          }
        }
      }
    }
    if (maxMeshSurf > 0 && !obj->contsize) {
      fprintf(fout, "#Surface : Contours,  Mesh Volume,  Mesh Surface\n");
      for (i = 0; i <= obj->surfsize; i++)
        fprintf(fout, "%7d   %8d   %12.6g  %12.6g\n"
                , i, nofc[i], volInMesh[i], msa[i]);
    } else if (maxContSurf > 0 && maxMeshSurf == 0 && mvol[maxContSurf] > 0.) {
    ...
    } else if (maxContSurf > 0 && maxMeshSurf > 0 && maxContSurf == maxMeshSurf) {
      fprintf(fout, "#Surface : Contours,  Cont. Volume,  Mesh Volume,"
              "  Mesh Surface,  Max Extent\n");
      for (i = 0; i <= obj->surfsize; i++)
        fprintf(fout, "%7d   %8d   %12.6g  %12.6g  %12.6g  %12.6g\n"
                , i, nofc[i], mvol[i], volInMesh[i], msa[i], extent[i]);
    } else {
      fprintf(fout, "#Surface : Contours,  Cyl. Volume,  Cont. Volume,  Mesh Volume,"
              "  Mesh Surface,  Max Extent\n");
      for (i = 0; i <= obj->surfsize; i++)
        fprintf(fout, "%7d   %8d   %12.6g  %12.6g  %12.6g  %12.6g  %12.6g\n"
                , i, nofc[i], vol[i], mvol[i], volInMesh[i], msa[i], extent[i]);
    }
    free(msa);
    free(volInMesh);
  } else {
