...
int main( int argc, char *argv[])
{
  ...
  int sphere = 0, circle = 0, width2d = 0, thickness = 0;
  ...
  int hasValues = 0;
  int contValues = 0;
  int maxesEntered, xmaxIn, ymaxIn, zmaxIn;
  int *red, *green, *blue;
  int directScale = 0;
  float xscale = 1., yscale = 1., zscale = 1., xtrans = 0., ytrans = 0., ztrans = 0.;
  float modelZscale = 1, modPixel = 0.;
  char **names = NULL;
  ...
  const char *options[] = {
    ...
    "width:LineWidthIn2D:I:", "thick:LineThicknessIn3D:I:", "color:ColorOfObject:ITM:",
    "name:NameOfObject:CHM:", "image:ImageForCoordinates:FN:",
    "volume:VolumeSizeXYZ:IT:", "pixel:PixelSpacingOfImage:FT:",
    "origin:OriginOfImage:FT:", "modpix:ModelPixelSize:F:", "zscale:ZScaleOfModel:F:",
    "help:usage:B:"};

  /* Startup with fallback */
  PipReadOrParseOptions(argc, argv, options, numOptions, progname, 
                        2, 1, 1, &numOptArgs, &numNonOptArgs, imodUsageHeader);
  if (!PipGetBoolean("usage", &err)) {
  ...
  err = PipGetFloat("ZScaleOfModel", &modelZscale);
  err = PipGetFloat("ModelPixelSize", &modPixel);
  if (modPixel <= 0. && xscale != 1.f)
    modPixel = xscale / 10.f;

  err = PipGetInteger("PointsPerContour", &numPerCont);
  err = PipGetInteger("SphereRadius", &sphere);
  err = PipGetInteger("CircleSize", &circle);
  err = PipGetInteger("LineWidthIn2D", &width2d);
  err = PipGetInteger("LineThicknessIn3D", &thickness);
  ...
  maxesEntered = 1 - PipGetThreeIntegers("VolumeSizeXYZ", &xmaxIn, &ymaxIn, &zmaxIn);
  if (zFromZero)
    zOffset = 0.5;
  ...
  nvals -= hasValues;
  if ((nvals < 3 && hasValues) || nvals < 2)
    exitError("There must be at least %d values per line", 
              B3DCHOICE(hasValues, 3 + hasValues, 2));
  ...
  imod->ymax = 0.;
  imod->zmax = 0.;
  imod->zscale = modelZscale;
  if (modPixel > 0) {
    imod->pixsize = modPixel;
  ...
  while (1) {
    ...
    if (ob >= imod->objsize) {
      for (i = imod->objsize; i <= ob; i++) {
        ...
        if (width2d > 0)
          imod->obj[i].linewidth2 = width2d;
        if (thickness > 0)
          imod->obj[i].linewidth = thickness;
        if (i < numColors) {
