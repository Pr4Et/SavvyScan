...
void BruteForceSearch::Run()
{
	...
	numThreads = ctfNumOMPthreads(maxThreads);
...
#pragma omp parallel for default(none) num_threads(numThreads)	\
	shared( all_values, all_local_best_values, all_scores)								\
	private(current_iteration, current_values, current_values_for_local_minimization, i, accuracy_for_local_minimization)
		for (current_iteration=0; current_iteration < num_iterations; current_iteration++)
		{
			// what values for the parameters should we try now?
			// DNM: switch to getting from the list
	...
	/* IMOD no progress bar, icl 11 deletes;
