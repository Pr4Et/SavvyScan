...
#define iiutransvalidexttype IIUTRANSVALIDEXTTYPE
#define iiualt4bitmode IIUALT4BITMODE
...
#define iiutransvalidexttype iiutransvalidexttype_
#define iiualt4bitmode iiualt4bitmode_
...
void iiuRetMRCVersion(int iunit, int *version)
{
  ...
  *version = mrcGetStandardVersion(hdr);
}
...
/*!
 * Changes the file mode to or from mode 101 depending on whether direction is > 0 
 * or < 0.  With direction > 0, it changes the mode to 101 if it is not in that mode
 * already, or returns 2 if the file was not already recognized as containing packed 
 * 4-bit data and has signed bytes.  With direction < 0, it changes the mode back to 0 if
 * the mode is 101, or returns 3 if the file has an odd size in X.  Returns 1 if the file
 * is not mode 0 or 101, and -1 if nothing was done.
 */
int iiuAlt4BitMode(int iunit, int direction)
{
  MrcHeader *hdr = iiuMrcHeader(iunit, "iiuAltMode", 1, 0);
  if (hdr->mode != MRC_MODE_BYTE)
    return 1;
  if (direction > 0 && hdr->packed4bits != PACKED_4BIT_MODE) {
    if (!hdr->packed4bits) {
      if (hdr->bytesSigned)
        return 2;
      if (hdr->mx == hdr->nx) {
    ...
    hdr->packed4bits = PACKED_4BIT_MODE;
    setOrClearFlags((b3dUInt32 *)(&hdr->iiuFlags), IIUNIT_4BIT_MODE, 1);
    setOrClearFlags((b3dUInt32 *)(&hdr->iiuFlags), IIUNIT_HALF_XSIZE, 0);
    return 0;
  } else if (direction < 0 && hdr->packed4bits == PACKED_4BIT_MODE) {
    if (hdr->nx % 2)
      return 3;
    hdr->packed4bits = PACKED_HALF_XSIZE;
    setOrClearFlags((b3dUInt32 *)(&hdr->iiuFlags), IIUNIT_4BIT_MODE, 0);
    setOrClearFlags((b3dUInt32 *)(&hdr->iiuFlags), IIUNIT_HALF_XSIZE, 1);
    return 0;
  }
  return -1;
}

int iiualt4bitmode(int *iunit, int *direction){return iiuAlt4BitMode(*iunit, *direction);};
...
/*!
 * Sets the number of bytes in the extended header of unit [iuit] into [numBytes].
 ...
 * size per section for unit [iunit] for headers with an {extType} value of "SERI" or 
 * "AGAR" or with no type indicated there.  For other types is returns the negative of
 * the type in [nintOrBytes] and a version number if appropriate in [nrealOrFlags];
 * currently -3 and 1 for FEI1, and -4 and 0 for an unknown type. Fortran wrappers 
 * iiuRetExtendedType and irtsymtyp.
 */
void iiuRetExtendedType(int iunit, int *nintOrBytes, int *nrealOrFlags)
{
  MrcHeader *hdr = iiuMrcHeader(iunit, "iiRetExtendedType", 1, 0);
  int version, type;
  type = mrcGetExtendedType(hdr, &version);
  if (type == MRC_EXT_TYPE_NONE || type == MRC_EXT_TYPE_SERI || 
      type == MRC_EXT_TYPE_AGAR) { 
    *nintOrBytes = hdr->nint;
    *nrealOrFlags = hdr->nreal;
  } else {
    *nintOrBytes = -type;
    *nrealOrFlags = version;
  }
}

void iiuretextendedtype(int *iunit, int *nintOrBytes, int *nrealOrFlags)
{ iiuRetExtendedType(*iunit, nintOrBytes, nrealOrFlags);}
void irtsymtyp(int *iunit, int *nintOrBytes, int *nrealOrFlags)
{ iiuRetExtendedType(*iunit, nintOrBytes, nrealOrFlags);}

/*!
 * Sets the elements {nint} and {nreal} for unit [iunit].  Fortran wrappers 
...
void iiuAltExtendedType(int iunit, int nintOrBytes, int nrealOrFlags)
{
  ...
  hdr->nint = nintOrBytes;
  hdr->nreal = nrealOrFlags;
}

void iiualtextendedtype(int *iunit, int *nintOrBytes, int *nrealOrFlags)
{ iiuAltExtendedType(*iunit, *nintOrBytes, *nrealOrFlags);}
void ialsymtyp(int *iunit, int *nintOrBytes, int *nrealOrFlags)
{ iiuAltExtendedType(*iunit, *nintOrBytes, *nrealOrFlags);}

/*!
 * Returns a numeric code for the contents of the {extType} field of the header for
 * unit [iunit] into [type] and a version number, if any, in [version], by calling
 * @@mrcfiles.html#mrcGetExtendedType@.  Fortran wrapper iiuRetHeaderExtType.
 */
void iiuRetHeaderExtType(int iunit, int *type, int *version)
{
  MrcHeader *hdr = iiuMrcHeader(iunit, "iiuRetHeaderExtType", 1, 0);
  *type = mrcGetExtendedType(hdr, version);
}

void iiuretheaderexttype(int *iunit, int *type, int *version)
{ iiuRetHeaderExtType(*iunit, type, version);}
...
int iiuRetExtendedData(int iunit, int *numBytes, int *extra)
{
  ...
  *numBytes = hdr->next;
  if (!hdr->next)
    return 0;
  if (mrcReadExtraHeader(hdr, (unsigned char **)(&extra))) {
 ...
 * Returns with no error if the output file is not MRC.
...
int iiuAltExtendedData(int iunit, int numBytes, int *extra)
{
  ...
  if (mrcWriteExtraHeader(hdr, (unsigned char *)extra, numBytes)) {
...
int iiuTransExtendedData(int intoUnit, int iunit)
{
  ...
  mrcCopyValidExtendedType(ihdr, jhdr);
...
/*!
 * Transfers the extType stamp from one MRC file, [iunit], to another, [intoUnit], if it
 * appears to be valid (see @mrcfiles.html#mrcCopyValidExtendedType). Fortran wrapper 
 * iiuTransValidExtType.
 */
void iiuTransValidExtType(int intoUnit, int iunit)
{
  MrcHeader *ihdr, *jhdr;
  if (iiuFileType(intoUnit) != IIFILE_MRC || iiuFileType(iunit) != IIFILE_MRC)
    return;
  ihdr = iiuMrcHeader(iunit, "iiuTransValidExtType", 1, 0);
  jhdr = iiuMrcHeader(intoUnit, "iiuTransValidExtType", 1, 0);
  mrcCopyValidExtendedType(ihdr, jhdr);
}

void iiutransvalidexttype(int *iunit, int *junit)
{ return iiuTransValidExtType(*iunit, *junit);}
