...
#define iitestifhdf IITESTIFHDF
#else
#define iibesttilesize iibesttilesize_
#define iitestifhdf iitestifhdf_
#endif
...
static ImodImageFile *sIITiffs[MAX_TIFF_THREADS];
static int sMaxTiffThreads = 0;
...
{
  if (sQuitCheckFunc != NULL && (*sQuitCheckFunc)(param))
...
ImodImageFile *iiNew()
{
  ...
  ofile->rawPaletteBytes = 0;
  ofile->tiffCompression = 1;  /* 1 actually means no compression */
...
ImodImageFile *iiOpenNew(const char *filename, const char *mode, int fileKind)
{
  ...
  } else if (!err && fileKind == IIFILE_JPEG) {
    err = jpegOpenNew(ofile);
    if (err)
      b3dError(stderr, "ERROR: iiOpenNew - Opening new JPEG file\n");
 } else if (!err) {
...
/*!
 * Makes a copy of the image file structure for [inFile] and opens the file with 
 * @iiReopen to take advantage of the previous analysis of the file properties.  Returns 
 * the opened file copy or NULL for various errors.  For a TIFF file, copies can be used
 * to access a file from multiple threads; this may or may not work with other file types.
 */
ImodImageFile *iiCopyOpen(ImodImageFile *inFile)
{
  ...
  if (!inFile)
    return NULL;
  if (inFile->colormap) {
    b3dError(stderr, "ERROR: iiCopyOpen - Not allowed for a file with a colormap\n");
    return NULL;
  }
  copy = iiNew();
  if (!copy)
    return NULL;
  memcpy(copy, inFile, sizeof(ImodImageFile));

  /* Clear pointers to all allocated items */
  copy->fp = NULL;
  copy->header = NULL;
  copy->filename = NULL;
  ...
  /* Make copy of filename, forget description */
  copy->filename = strdup(inFile->filename);
  if (!copy->filename) {
    iiDelete(copy);
    b3dError(stderr, "ERROR: iiCopyOpen - Memory error copying filename\n");
    return NULL;
  }
  err = iiReopen(copy);
  if (err) {
    iiDelete(copy);
    b3dError(stderr, "ERROR: iiCopyOpen - Error %d calling iiReopen on copy of file\n");
    return NULL;
  }
  return copy;
}
...
/*!
 * Enables parallel reading from a TIFF file open on [inFile], using a fixed array of 
 * ImodImageFile pointers in this module.  Returns 0 if parallel reading is already set 
 * up for a different file, or if the file size is such that only one thread would be
 * used; otherwise returns the number of threads for full-sized reading.  The maximum
 * number of threads to use can be passed in [maxThreads]; if this is 0 the default will
 * used, 16.
 */
int iiUseTiffThreads(ImodImageFile *inFile, int maxThreads)
{
  if (inFile->file != IIFILE_TIFF || sMaxTiffThreads > 0)
    return 0;
  if (maxThreads <= 0)
    maxThreads = MAX_TIFF_THREADS;
  maxThreads = tiffNumReadThreads(inFile->nx, inFile->ny, inFile->tiffCompression, 
                                  maxThreads);
  if (maxThreads > 1) {
    sIITiffs[0] = inFile;
    maxThreads = iiOpenCopiesForThreads(sIITiffs, maxThreads);
  }
  if (maxThreads > 1)
    sMaxTiffThreads = maxThreads;
  return sMaxTiffThreads;
}

/*!
 * Enables parallel reading from a TIFF file using a file pointer [fp] returned after 
 * opening it with @@iiFOpen@.  Calls @iiUseTiffThreads and returns its return value.
 */
int iiUseTiffThreadsForFP(FILE *fp, int maxThreads)
{
  ...
  if (!iiFile)
    return 0;
  return iiUseTiffThreads(iiFile, maxThreads);
}

/*!
 * Closes all the copies of the TIFF file in [inFile] opened by @@iiUseTiffThreads@; has 
 * no effect if no copies were opened or if called with a different file.
 */
void iiCloseTiffCopies(ImodImageFile *inFile)
{
  int ind;
  if (sMaxTiffThreads <= 0 || inFile != sIITiffs[0])
    return;
  for (ind = 1; ind < sMaxTiffThreads; ind++)
    iiDelete(sIITiffs[ind]);
  sMaxTiffThreads = 0;
}

/*!
 * Closes all the copies of a TIFF file opened by calling iiUseTiffThreadsForFP with [fp].
 * Has no effect if no copies were opened or if called with a different file.
 */
void iiCloseTiffCopiesForFP(FILE *fp)
{
 ImodImageFile *iiFile = iiLookupFileFromFP(fp);
  if (iiFile)
    iiCloseTiffCopies(iiFile);
}

/*!
 * Calls @iiCopyOpen [maxThreads] - 1 times with the first element in the array 
 * [fileCopies], putting the additional open files in [fileCopies], thus allowing access
 * from multiple threads.  Returns the number of threads that can be used, which will be
 * less than [maxThreads] if the opening of a copy fails.
 */
int iiOpenCopiesForThreads(ImodImageFile **fileCopies, int maxThreads)
{
  int ind;
  for (ind = 1; ind < maxThreads; ind++) {
    fileCopies[ind] = iiCopyOpen(fileCopies[0]);
    if (!fileCopies[ind])
      return ind;
  }
  return maxThreads;
...
/*!
 * Set up an MRC header with the size, mode, and other critical information
 */
...
/*!
 * Reads the section [inSection] from the file [inFile] into buffer [buf] with conversion
 * specified by [convertTo], which must be one of MRSA_NOPROC, MRSA_BYTE, MRSA_USHORT,
 * and MRSA_FLOAT.  Returns -1 for undefined reading function, failure to reopen file,
 * or invalide [convertTo], otherwise passes along return value of the reading function.
 */
int iiReadSectionAny(ImodImageFile *inFile, char *buf, int inSection, int convertTo)
{
  if (convertTo == MRSA_NOPROC)
    return(iiReadSection(inFile, buf, inSection));
  if (convertTo == MRSA_BYTE)
    return(iiReadSectionByte(inFile, buf, inSection));
  if (convertTo == MRSA_USHORT)
    return(iiReadSectionUShort(inFile, buf, inSection));
  if (convertTo == MRSA_FLOAT)
    return(iiReadSectionFloat(inFile, buf, inSection));
  b3dError(stderr, "ERROR: iiReadSectionAny - Invalid value %d for convertTo parameter\n",
           convertTo);
  return -1;
}
...
static int readWriteSection(ImodImageFile *inFile, char *buf, int inSection, 
                            iiSectionFunc func, const char *mess)
{
  int err, csize, dsize, dataSize = 0, convert;
  if (!func) {
  ...
  if (sMaxTiffThreads > 1 && inFile == sIITiffs[0] && inFile->axis == 3) {
    if (!mrc_getdcsize(inFile->mode, &dsize, &csize)) {
      if (func == inFile->readSection) {
        dataSize = dsize * csize;
        convert = MRSA_NOPROC;
      } else if (func == inFile->readSectionByte) {
        dataSize = 1;
        convert = MRSA_BYTE;
      } else if (func == inFile->readSectionUShort) {
        dataSize = 2;
        convert = MRSA_USHORT;
      } else if (func == inFile->readSectionFloat) {
        dataSize = 4;
        convert = MRSA_FLOAT;
      }
    }
  ...
  iiChangeCallCount(1);
  if (dataSize)
    err = tiffParallelRead(sIITiffs, sMaxTiffThreads, inFile->llx, inFile->urx, 
                           inFile->lly, inFile->ury, dataSize, buf, inSection, convert);
  else
...
/*!
 * Returns the value of a single pixel at [x, y, z] from the image file [inFile].
...
/*!
 * If [ifFloat] is set and the mode of the file [inFile] is not float, it makes a new 
 * buffer for the full contents of [buf] and converts the floats in buf to integer or
 * byte, inverting the lines in Y and setting [inverted] to 1.  Returns the new buffer in 
 * this case, or just returns [buf].  The name of the calling routine should be in 
 * [routine].
 */
                                 ...
                                 int *inverted, const char *routine)
{
  ...
  int nx = inFile->nx;
  int iy, ny = inFile->ny;
  int pixsize = (inFile->mode == MRC_MODE_BYTE) ? 1 : 2;
  int dataSize = (size_t)nx * (size_t)ny * pixsize;
  if (ifFloat && inFile->type != IITYPE_FLOAT) {
    useBuf = B3DMALLOC(char, dataSize);
    if (!useBuf) {
      b3dError(stderr, "ERROR: %s - Allocating array for converting floats\n", routine);
      return NULL;
    }
    *inverted = 1;
    for (iy = 0; iy < ny; iy++) {
      bdata = (unsigned char *)useBuf + nx * pixsize * (size_t)(ny - 1 - iy);
      iiConvertLineOfFloats(fbufp, bdata, nx, inFile->mode, 0, 0);
      fbufp += nx;
    }
  }
  return useBuf;
                           ...
                           int bytesSigned, int pack4bits)
{
  ...
  case MRC_MODE_BYTE:
    if (pack4bits) {
      ...
      /* 4-bit numbers */
      for (i = 0; i < nx / 2; i++) {
        ival = (int)(fbufp[2 * i] + 0.5f);
        B3DCLAMP(ival, 0, 15);
        hval = (int)(fbufp[2 * i + 1] + 0.5f);
        B3DCLAMP(hval, 0, 15);
        bdata[i] = (unsigned char)(ival + (hval << 4));
      }
      if (nx % 2) {
        ival = (int)(fbufp[nx - 1] + 0.5f);
        B3DCLAMP(ival, 0, 15);
...
int iitestifhdf(const char *filename, int namelen)
{
  int retval;
  char *cstr = f2cString(filename, namelen);
  if (!cstr)
    return -1;
  retval = iiTestIfHDF((const char *)cstr);
  free(cstr);
  return retval;
}
...
/*!
 * Adds a dataset for section [cz] to an HDF file open on [inFile] without writing data;
 * [buf] should be defined but need not be more than a few bytes.  Returns error values
 * from the writing routine or 1 if the package does not support HDF.
 */
...
/*!
 * Tests if the file [filename] is an HDF file without opening it extensively; this 
 * function can be used when a file may be open for writing in another process.  Returns
 * -2 if the package does not support HDF, -1 for an error opening the file, 0 if file
 * is not HDF, or 1 if it is.  Fortran wrapper iitestifhdf
 */
...
/* These need to be here because they are globals in hdf_imageio and they are in the 
   libiimod.def file */
