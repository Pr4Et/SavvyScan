                              ...
                              int fromFloat)
{
  ...
  if (fromFloat > 0 && hdata->mode != MRC_MODE_COMPLEX_FLOAT && 
  ...
  if (fromFloat < 0) {
    cleanupTmp(tmpData, -1, NULL, 0, 0, NULL);
    return 0;
  }
  ...
  while (line <= yEnd) {
    ...
    for (; line <= lineEnd; line++) {
      ...
      if (convert) {
        ...
        /* Convert floats */
        iiConvertLineOfFloats(fbufp, bdata, nx, hdata->mode, bytesSigned, 0);
        fbufp += xDimension;
...
static hid_t getDatasetForZ(ImodImageFile *inFile, int cz, int *noData)
{
  ...
  *noData = 0;
  if (inFile->stackSetList) {
    ...
    if (cz >= inFile->zMapSize || inFile->zToDataSetMap[cz] < 0) {
      *noData = 1;
      return 0;
    }
    ...
    if (stackp->isOpen) {
      ...
      /* It gets a new ID when it is opened again! (Which I seemed to know below) */
      dsetID = H5Dopen(fileID, stackp->name, H5P_DEFAULT);
      stackp->dsetID = dsetID;
      stackp->isOpen = 1;
    }
  } else {
