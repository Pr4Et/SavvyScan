...
static  double flyback_us=160.0; //flyback time imposed in ScanMode=1 (shahar).  From 40us to 160us, 5.1.22
const double PI = 3.141592653589793;
 ...
 const int NotifyBlockPerChSamp=2*1024*1024; //should be NotifyBlock / 16
...
int CJediCamera::InitializeCamera(int camNum)
{
  ...
  if (!camNum) {
	...
	sScanInitialized=Initialize_cards ();//Initializ savvyscan camera
  return 0;
}
                                  ...
                                  int *channels, int flags, int *numAcquired)
{
  ...
  //29Mar22: until now exposure was the delay invested per "camera" pixel, now it will include overhead time for hidden pixels (since SerialEM is using this time to ramp the defocus line-by-line) 
  double sampling_exposure; //29Mar22: the exposure time per pixel as we wrote initially in the savvyscan paper, related to the sampling rate. Now is modified according to patern mode
  ...
  if (ScanMode==1)
  {
      scXsize = (int)((*sizeX) * (1.0 +0.414));//was 1+0.414 before sep17 12:00, 
	  scYsize = *sizeY;
  ...
  {
      scXsize = (int)((double)width *sqrt(2.0)); //bruto size
	  scYsize = (int)((double)height*sqrt(2.0));
  ...
  {
	  scXsize = (int)(2*width); //bruto size: width + two half circles of same size on the left and right
	  scYsize = (int)height;
	  //simulate with oversampling=1 to calculate fsize
		int rnbs=(int)(width);//number of circling times, so the right hand of the circle interlaces with left hand, jumping 2 pixels per cycle, drifting 2*width=2*diameters
		int scanr=(int)(height/2); //radius of the circle
		int phnmbs=((int)(2.0*PI*(double)scanr)); ////number of phase points in one round: so the arc steps are 1pixel, oversampling here is 1
  ...
  else if (ScanMode == 4)
  {
      scXsize = (int)width;
      scYsize = (int)height;
      fsize = 3 * scXsize * scXsize;

  }
  else if (ScanMode == 5)
  {
      scXsize = (int)(width * (1 + 0.125)); 
      scYsize = height;
      fsize = height * (width+0.5*width) + height * 4*(int)(flyback_us / exposure); 
  }
  else if (ScanMode == 6)
  {
      width = 128;    //net horizontal(x) size
      height = 128;  //net vertical (y) size
      scTop = 3072;
      scLeft = 3072;//full is 512X512
      binning = 16;
      exposure = 1;//in us per pixel
      scXsize = (int)((double)width * sqrt(2.0)); //bruto size
      scYsize = (int)((double)height * sqrt(2.0));
      //simulate spiral with oversampling=1 to calculate fsize
      int rnbs = (int)(scXsize / 2);//number of radius points
  ...
  if (fsize*8 > 496*1024*1024) fsize = fsize + (NotifyBlockPerChSamp - (fsize % NotifyBlockPerChSamp)); //when more than the memory card is needed it is crucial to use vector length of NotifyBlock multiplicity

  sampling_exposure = exposure * (double)width * height / (double)fsize; //concerned with the sampling rate
  ...
  try {
    ...
    *numAcquired = 0;
	...
	{		
		...
		pass=set_cards_samplerate_andmore(sampling_exposure, fsize, oversampling); //calculate samplerate,oversampling, datasize and set cards accordingly, start timestamp DMA
		...
		fsize =  (width * height); //overwrite previous value, since only this SerialEM can understand
