#!/home/stem/anaconda3/envs/libertem/bin/python
# LiberTEM_passive.py (ver. 2023May5): A LiberTEM-live server for SavvyScan-ver-2b, written by Shahar Seifer (2023), Elbaum lab, Weizmann Institute of Science
    ...
    with conn:
        while True:
                ...
                if message[0:4]==b'EXIT':
                    ...
                    BF_norm = np.int16(np.zeros([2,2]))
                    img = Image.fromarray(BF_norm)
                    img.save("/home/stem/LiberTEM.png") #first replace result with empty file
                    BF_1D=np.ravel(BF_norm, order='C')
                    ...
                    if ptype=="SUM":
                        ...
                        BF_VAL = BF_RES['intensity'].raw_data
                        message=b'OFF\0'
                    elif ptype=="SSB" and nx>128:
                        printf("Not enough resources")
                        BF_VAL = np.int16(np.zeros([2,2]))
                        message=b'OFF\0'
                    elif ptype=="SSB": 
                        mask_params = {
                        ...
                        continue
                    #BF_VAL.shape=(ny,nx)
                    if np.max(BF_VAL)>32766 or np.min(BF_VAL)<0:
                        BF_norm=np.int16(32767 *(BF_VAL-np.min(BF_VAL))/(0.001+np.max(BF_VAL)-np.min(BF_VAL)))
                    else:
                        BF_norm=np.int16(BF_VAL)
                    BF_1D=np.ravel(BF_norm,order='C')
                    message=socket.recv()
                    print("Received request: %s" % message)
                    if message[0:5]==b'DATA?':
                        for y in range (1,ny+1):
                            temp=bytes(BF_1D.astype('int16')[0+(y-1)*nx:y*nx])
                            temp_encode=base64.b64encode(temp+b'/0/0')
                            socket.send_string(temp_encode.decode('utf-8'),flags=zmq.SNDMORE)
                        print("Image transferred to SavvyScan")
                        socket.send_string(reply_txt)
                    BF_norm2=np.int16(32767 *(BF_VAL-np.min(BF_VAL))/(0.001+np.max(BF_VAL)-np.min(BF_VAL)))
                    img = Image.fromarray(BF_norm2)
                    img.save("/home/stem/LiberTEM.png")
                    print("Image saved")
