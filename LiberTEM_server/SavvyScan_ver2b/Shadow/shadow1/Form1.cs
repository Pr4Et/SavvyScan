...
namespace shadow1
{
    public partial class Form1 : Form
    {
       ...
       // }
        ...
        {
            if (chs_blocked.Checked && chosenCH.Value < 7)
            {
                chosenCH.Value = 7;
            }
         ...
         private void set_cella()
        {
            ...
            Program.cella = (int)((11693.0 / magnification) * 10.0 * 8192.0 * ((double)OutputScanAmp.Value / 2000.0)); //size of image in angstorms (goes to mrc file, cella.x and cella.y) 
            Program.LRcella = (int)((11693.0 / LRmagnification) * 10.0 * 8192.0 * ((double)OutputScanAmp.Value / 2000.0)); //size of image in angstorms (goes to mrc file, cella.x and cella.y) 
        ...
        private void send_libertem_Click(object sender, EventArgs e)
        {
            string text2 = "LiberTEM=" + libertem_UI.Text;
            Program.sendrecieve(text2);
            send_libertem.BackColor = Color.Beige;
        }

        private void chs_blocked_CheckedChanged(object sender, EventArgs e)
        {
            string text2;
            if (chs_blocked.Checked)
            {
                text2 = "BFchsBlocked=1";
            }
            else
            {
                text2 = "BFchsBlocked=0";
            }
            Program.sendrecieve(text2);
            if (chs_blocked.Checked && chosenCH.Value<7)
            {
        ...
        private void libertem_UI_SelectedIndexChanged(object sender, EventArgs e)
        {
            send_libertem.BackColor = Color.Red;
        }
