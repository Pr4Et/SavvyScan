...
//#define _AFXDLL 1
//#include <afxinet.h> //added 20Apr23 for connection with FTP site on LiberTEM computer
...
#include <zmq.h> //added 16Apr23 for connection to LiberTEM server 
//#include <wincrypt.h> //base64 communication strings
//#include "CImg.h"
...
void*				remote_data;//for zmq receive data message 
...
//shahar, global vairbales from Shadow GUI to RecRepMulti.cpp and JeolCamera.cpp
 ...
 //added 16Apr23 for ZMQ connection to LiberTEM server:
 extern bool GUI_BF_chs_blocked;
 extern char GUI_LiberTEMcommand[200];
 extern void* context3;
 extern void* requester3;
 extern int rc3;
 int returnsizeb;
 char zmq_msg_buffer3b[200];




 FILE** fpv=new FILE*[9];
 MrcHeader* hdr=new MrcHeader[9];
 ...
 bool FTP_SUCCESS=true;

 extern bool using_LiberTEM;
...
void retrieve_images(const int width, const int height, short * pData)
{
    ...
    // ####   Work on channels 0 to 7 and possibly 8 (LiberTEM) in parallel  #####
	...
	//Load image from LiberTEM if applicable (assuming the process is over, otherwise will get small data structure and skip)
	if (using_LiberTEM)
	{
		//if (strncmp(GUI_LiberTEMcommand, "SSB", 3) == 0)
		//{
		//	Sleep(90000);
		//}
		Sleep(1000);
		...
		FILE* outFile = fopen("A:/SavvyscanData/LiberTEM.bin", "wb");
		...
		for (int yind = 0; yind < height; yind++)
		{
			...
			if (returnsizeb >0)
			{
				returnsizeb= FromBase64Fast((BYTE*)zmq_msg_long, returnsizeb, (char*)long_out, (width + 1) * 2);
				
				fwrite(long_out, 1, (int)width * 2, outFile);
			}
			else
			{
				printf("LiberTEM server did not respond in time");
				using_LiberTEM = false;
				break;
			}
		...
		if (returnsizeb > 0)
		{
			/////using_LiberTEM = vmd_ftpclient( "LiberTEM.bin", "A:\\SavvyscanData\\LiberTEM.bin");
		}
		else
		{
			printf("LiberTEM server did not respond in time");
	...
	//LiberTEM input
	process_channel_parallel(8, width, height, pData);
...
{
	int job_ch = ch;
	if (GUI_BF_chs_blocked && ch >= 0 && ch <= 6)
	{
		chwrite_done[job_ch] = true;
		return;
	}
	if (GUI_chosenCH != 8 && ch == 8 && !using_LiberTEM)
	{
		chwrite_done[job_ch] = true;
		return;
	}
	...
	if (ch == 8)
	{
		if (using_LiberTEM)
		{
			//generate filleresults[] from image information on A, if possible
			//
			int16_t readint16;
			FILE* inFile = fopen("A:/SavvyscanData/LiberTEM.bin", "rb");
			if (inFile == NULL) {
				ch = 7;
			}
			else
			{
				...
				long int size_opened_file = ftell(inFile);
				fseek(inFile, 0L, SEEK_SET);
				if (size_opened_file==(long)(width*height*2))
				{
					for (int j = height - 1; j >= 0; j--)
					{
						for (int i = 0; i < width; i++) {
							fread(&readint16, sizeof(readint16), 1, inFile);
							fillresults[i+j*width] = (double)readint16;
						}
					}
				}
				else
			...
			//alternative
			//::ifstream inFile;
			//size_t size_opened_file = 0;
			//inFile.open("A:/SavvyscanData/LiberTEM.bin", std::ios::in | std::ios::binary | std::ios::ate);
	...
	{
		mrc_head_new(&hdr[job_ch], width, height, GUI_numberOfSlices, MRC_MODE_USHORT);	//hdr.mode=MRC_MODE_USHORT  : we say it is ushort but serialEM expect short values
		mrc_set_scale(&hdr[job_ch], (double)GUI_cella / width, (double)GUI_cella * GUI_aspectratio / height, 0);//double x, double y, double z=0 so will not update, since this is a tilt series. Remember the rotation axis is x so hoppe scheme requires updating the y scale
	}
	if (GUI_tomoIndex == 0) fpv[job_ch] = NULL;
	if (!for_alignment)
	{
		sprintf(multi_fname, "A:/SavvyscanData/CH%d.mrc", job_ch);
		if (GUI_tomoIndex == 0 )
			fpv[job_ch] = fopen(multi_fname, "wb");//write replace
		else //if (GUI_tomoIndex > 0 && !GUI_tomography)
			fpv[job_ch] = fopen(multi_fname, "ab"); // write append. //XXXXsould not happen, I remove since closeing and opening consumes time
		if (true || fpv[job_ch] != NULL) {
			if (GUI_tomoIndex == 0)
				mrc_head_write(fpv[job_ch], &hdr[job_ch]);
			mrc_write_slice((void*)oneimage, fpv[job_ch], &hdr[job_ch], GUI_tomoIndex, 'Z'); //save mrc stack as slice=GUI_tomoIndex
			//if (!GUI_tomography)  fclose(fpv[ch]);
			fclose(fpv[job_ch]);
		}
	}

	if (job_ch == GUI_chosenCH && ref_for_alignment && GUI_tomography && !GUI_SerialEMAligned)
	{
		if (GUI_tiltangle >= -3)
		{
			sprintf(multi_fname, "A:/SavvyscanData/lastHR_positive.mrc");
		...
		if (GUI_tiltangle <= 3)
		{
			sprintf(multi_fname, "A:/SavvyscanData/lastHR_negative.mrc");
			fp_align = fopen(multi_fname, "wb");
			mrc_head_write(fp_align, &hdr_align);
			mrc_write_slice((void*)oneimage, fp_align, &hdr_align, 0, 'Z');
			fclose(fp_align);
	...
	if (job_ch == GUI_chosenCH && for_alignment && !GUI_SerialEMAligned)
	{
		sprintf(multi_fname, "A:/SavvyscanData/newLR4alignment.mrc");
		fp_align = fopen(multi_fname, "wb");
		mrc_head_write(fp_align, &hdr_align);
		mrc_write_slice((void*)oneimage, fp_align, &hdr_align, 0, 'Z');
		fclose(fp_align);
	...
	if (job_ch<=7 && GUI_saveMAT && !for_alignment && GUI_tomoIndex >= 0)
	{
		sprintf(multi_fname, "A:/SavvyscanData/CH%d.mat", job_ch);
		if (GUI_tomoIndex == 0)
			pmat[job_ch] = matOpen(multi_fname, "wz"); //wz opens compressed MAT-file
		else
			pmat[job_ch] = matOpen(multi_fname, "u"); //update compressed MAT-file (append slots)
		if (pmat[job_ch] != NULL) {
			sprintf(varname, "header%d", GUI_tomoIndex);
			matPutVariable(pmat[job_ch], varname, header);
			series[job_ch] = mxCreateDoubleMatrix(1, NumSamplesPerCh, mxREAL);
			memcpy((void*)(mxGetPr(series[job_ch])), (void*)ChannelData, NumSamplesPerCh * sizeof(double));
			sprintf(varname, "series%d", GUI_tomoIndex);
			matPutVariable(pmat[job_ch], varname, series[job_ch]);
			matClose(pmat[job_ch]);
		}

	}

	//for python program
	if (job_ch >= 1 && job_ch <= 4 && save4python_latch)
	{
		sprintf(fname_python, "C:/Users/stem/Lothar/CH%d.mrc", job_ch);
...
int Acquire_scan()
{
	...
	if (!LowRes)
	{
		if (GUI_ArinaON) //Arm Arina detector before each scan
		{
			...
			int number_of_triggers = int(MAT_netXsize) * int(MAT_netYsize);
			char* namestr;
			namestr = GUI_ArinaFileName;
			if (integration_time_us >= 10)
			{
				if (strncmp(GUI_LiberTEMcommand, "SUM", 3) == 0 || strncmp(GUI_LiberTEMcommand, "BF", 2) == 0 || strncmp(GUI_LiberTEMcommand, "SSB", 3) == 0 || strncmp(GUI_LiberTEMcommand, "COM", 3) == 0)
				{
					sprintf(zmq_msg_buffer3b, "START %d,%d", int(MAT_netXsize), int(MAT_netYsize));
					zmq_send(requester3, zmq_msg_buffer3b, strlen(zmq_msg_buffer3b), 0);
					returnsizeb = zmq_recv(requester3, zmq_msg_buffer3b, 2, 0); //should get OK, image will be sent by FTP
					if (returnsizeb > 0)
					{
						printf("\n LiberTEM: START %s \n", GUI_LiberTEMcommand);
						using_LiberTEM = true;
				...
				//the ip address is for the Dectris server of the pixelated camera
				char line2[250];
...
bool Send_ArinaGate_pattern() 
{
	...
	dcb.BaudRate = CBR_115200;     //  baud rate (was CBR_38400 in old box)
...
//----------------------------------------------------
 //For ZMQ image transfer, function written by Dan Rollins (2014)
...
// Utra-Fast base64-decoding, using 128KB lookup table  / Dan Rollins (2014)
